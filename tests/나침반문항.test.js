...
test('모르는 키는 실패다 — 오타 키가 앉으면 그 학생만 조용히 빈칸이 된다', () => {
  ...
  // 시즌 회차에 모르는 키를 섞는 것도 같은 실패다(회차를 위장하는 자리).
  // ⚠ 09-06 부터 why_learning·topik_use 는 시즌 회차의 «덧붙임»으로 «받는» 키다(철학 Ⅱ-4 v1.21 「바꿀래」 ·
  //   조각 20260906000000 · 회귀 = tests/나침반덧붙임.test.js). 그래서 여기서는 진짜 모르는 키로 잰다.
  const 섞임 = 답검사(종류.시즌, { ...시즌답(), season_goal_v2: '섞음' }, true);
  assert.ok(섞임 && 섞임.필드 === 'season_goal_v2', '시즌 회차가 모르는 키를 받았다');
});

...
const L0경로 = path.join(ROOT, 'docs', 'L0_데이터계약.md');
...
/* 산문이 말하는 「현행 cN」. 위 검사는 `contract_ver` **예시**(코드 블록·헤더 표)만 봤고,
 * 사람이 문장으로 적은 판 번호는 **아무도 안 봤다.** 2026-08-09 심문 4판이 전부 같은 자리를
 * 짚었다 — 한 문서 안에서 §0 이 「현행 c6」, §4-1 이 「현행 계약(c9)」, 응답 예시가 `c10` 이라
 * 구현자가 어느 판을 따라야 하는지 결정할 수 없었다. 예시만 고치고 산문을 두고 가는 것이
 * 실제 사고 형태다(위 헤더 표 주석과 같은 무늬).
 *
 * 🔑 **역사 서술은 잡지 않는다.** 이 문서는 「c6 신설」·「c9부터 필수」처럼 개정 이력을 곳곳에
 * 달고 있고 §9 는 통째로 이력 절이다 — 모든 `cN` 을 세면 거짓 경보가 쏟아지고, 거짓 경보를
 * 내는 가드는 곧 꺼진다. 그래서 **「현행」이라고 주장하는 자리만** 본다(사람이 실제로 쓰는 표기).
 * 게으른 수량자라 `현행 계약은 c10 이고 아래는 c4 시점` 에서 **앞의 것만** 집는다. */
const 현행주장 = (text) => [...new Set(
  [...text.matchAll(/현행[^\n]{0,15}?\bc(\d+)\b/g)].map((m) => `c${m[1]}`),
)];

test('탐지력 픽스처 — 산문이 낡은 판을 「현행」이라 부르면 잡는다 (예시만 보는 검사는 이걸 못 봤다)', () => {
  assert.deepEqual(현행주장('`계약.json`(**현행 c6**)'), ['c6']);
  assert.deepEqual(현행주장('payload 이름도 **현행 계약(c9) 목록에서 고른다**'), ['c9']);
  // 이력 서술·먼 거리는 안 잡는다 — 거짓양성이 나면 이 가드는 꺼진다
  assert.deepEqual(현행주장('이 절은 현행 규범이 아니라 이력이다(2026-08-09 명시)'), []);
  assert.deepEqual(현행주장('`task_snapshot` 은 c9부터 필수다'), []);
  // 한 줄에 둘이 있으면 「현행」 쪽만 — 뒤의 이력 판까지 세면 영원히 빨갛다
  assert.deepEqual(현행주장('현행 계약은 **c10**이고 아래는 **c4 시점**의 기술이다'), ['c10']);
});

test('C0 산문의 「현행 cN」이 계약 파일의 현행 판과 같다 (예시는 맞는데 문장이 낡은 것이 실제 사고였다)', () => {
  const md = fs.readFileSync(C0경로, 'utf8');
  const 쓰인 = 현행주장(md);
  assert.ok(쓰인.length > 0,
    'C0 에서 「현행 cN」 표기를 하나도 못 뽑았다 — 표기가 바뀌었거나 이 검사가 죽었다.\n' +
    '  0건은 통과가 아니다(초록은 분모와 함께 읽는다). 표기를 바꿨으면 위 `현행주장` 부터 고친다');
  assert.deepEqual(쓰인.filter((v) => v !== 계약.버전), [],
    `C0 산문이 낡은 판을 「현행」이라 부른다(진짜 현행 ${계약.버전}). 뽑힌 것: ${쓰인.join('·')}\n` +
    '  고치는 법: 계약 파일의 `버전`을 올렸으면 C0 의 「현행 cN」 문장도 같은 커밋에서 함께 올린다.\n' +
    '  ⚠ 개정 이력(「c9부터 필수」)은 고치지 않는다 — 그건 그때의 사실이다');
});

/* ── 라우팅 넓히기: 판을 «선언»하는 문서는 C0 하나가 아니다 ──────────────────
 * 🔴 위 세 검사의 규칙은 옳은데 **들어오는 문서가 C0 하나뿐**이었다. 규칙보다 좁은 라우팅은
 *   그 자체가 구멍이고, 새는 방향은 언제나 「통과」다(CLAUDE.md 가드 ③). 실제로 그 사이
 *   `docs/검수_내부계약.md` 가 생기며 `X-Contract-Ver: c10` 과 응답 봉투 `contract_ver` 를
 *   **자기 문장으로 선언**했는데, 판을 올리는 날 그 문서를 아무도 안 본다 — 그리고 c11 은
 *   바로 그 문서(§5 승인·폐기 물리 4자리)가 쓰기로 예약된 번호다.
 * 🔑 **목록으로 두지 않는다** — 파일 이름을 적어 두면 다음 문서가 생기는 날 이 검사만 그것을
 *   못 보고, 그게 지금 고치는 병 그대로다. 판 표기가 «있는» 문서를 훑어서 대상으로 삼는다.
 * 🚫 판 표기가 없는 문서는 대상이 아니다 — 「모든 문서가 판을 적어야 한다」는 다른 규칙이고,
 *   여기서 그것까지 강제하면 거짓 경보가 쏟아져 이 가드가 곧 꺼진다. */
const 헤더표기 = (text) => [...new Set(
  [...text.matchAll(/X-Contract-Ver`?\s*[|:]\s*`?(c\d+)/g)].map((m) => m[1]))];

/** 한 문서가 선언한 판 전부(예시·헤더·산문) — 정본과 다른 것만 돌려준다. */
const 판어긋남 = (원문, 정본판) =>
  [...뽑기(원문, 'contract_ver'), ...헤더표기(원문), ...현행주장(원문)]
    .filter((v) => v !== 정본판);

/** 판을 선언한 문서만 — 안 적은 문서는 여기 관심 밖이다. */
const 판선언문서들 = () => {
  const dir = path.join(ROOT, 'docs');
  return fs.readdirSync(dir)
    .filter((f) => f.endsWith('.md'))
    .map((f) => ({ 이름: f, 원문: fs.readFileSync(path.join(dir, f), 'utf8') }))
    .filter(({ 원문 }) => 뽑기(원문, 'contract_ver').length || 헤더표기(원문).length || 현행주장(원문).length);
};

test('탐지력 픽스처 — 헤더 표기를 표 꼴·평문 꼴 둘 다 뽑는다 (한 꼴만 보면 다른 꼴이 샌다)', () => {
  assert.deepEqual(헤더표기('| `X-Contract-Ver` | `c10` | 400 |'), ['c10']);
  assert.deepEqual(헤더표기('요청 헤더: X-Contract-Ver: c10'), ['c10']);
  // 값이 아닌 언급은 안 센다 — 거짓 경보를 내는 가드는 곧 꺼진다
  assert.deepEqual(헤더표기('| `X-Contract-Ver` 없음 | 400 `CONTRACT_VER_MISSING` |'), []);
  assert.deepEqual(헤더표기('🔴 **`X-Contract-Ver`를 요구하지 않는다** — §4-1과 다른 판단이다.'), []);
});

test('탐지력 픽스처 — C0 가 아닌 문서가 낡은 판을 선언해도 잡는다 (라우팅 구멍의 실제 모양)', () => {
  const 가짜 = '## §1 공통\n요청 헤더: X-Contract-Ver: c9\n응답 본문: { "contract_ver": "c9", "ok": true }';
  assert.deepEqual(판어긋남(가짜, 'c10'), ['c9', 'c9']);
  assert.deepEqual(판어긋남(가짜, 'c9'), []);          // 자기 처방 — 맞추면 통과한다
  assert.deepEqual(판어긋남('판 표기가 없는 문서', 'c10'), []);
  /* 세 갈래를 **각각** 태운다 — 합치는 함수에서 한 갈래를 빼도 나머지 둘이 초록을 내면
   * 그 갈래는 조용히 검사 밖으로 나간다(변이 시험이 실제로 빠져나간 자리다). */
  assert.deepEqual(판어긋남('§0 — 현행 계약(c9) 기준으로 짓는다', 'c10'), ['c9'], '산문 갈래가 빠졌다');
  assert.deepEqual(판어긋남('요청 헤더: X-Contract-Ver: c9', 'c10'), ['c9'], '헤더 갈래가 빠졌다');
  assert.deepEqual(판어긋남('{ "contract_ver": "c9" }', 'c10'), ['c9'], 'JSON 예시 갈래가 빠졌다');
});

test('분모 — 판을 선언한 문서를 몇 개나 찾았나 (C0 하나만 찾으면 넓히기가 안 된 것이다)', () => {
  const 문서들 = 판선언문서들();
  assert.ok(문서들.some((d) => d.이름 === 'C0_API계약.md'), 'C0 를 대상에서 놓쳤다 — 훑기가 죽었다');
  assert.ok(문서들.length >= 2,
    `판 표기를 가진 문서를 ${문서들.length}개만 찾았다 — 넓히기 전과 같은 상태다(뽑기·헤더표기가 실물 표기를 놓쳤을 수 있다)`);
});

test('🔴 판을 선언한 문서는 전부 계약 파일의 현행 판과 같다 (C0 만 보던 자리가 라우팅 구멍이었다)', () => {
  const 어긋남 = [];
  for (const { 이름, 원문 } of 판선언문서들()) {
    for (const v of 판어긋남(원문, 계약.버전)) 어긋남.push(`${이름}: ${v}`);
  }
  assert.deepEqual(어긋남, [], [
    `현행 판은 ${계약.버전} 인데 다른 판을 선언한 문서가 있다:`,
    ...어긋남.map((s) => `  · ${s}`),
    '  고치는 법: 계약 파일의 `버전`을 올렸으면 **판을 선언한 문서 전부**를 같은 커밋에서 올린다.',
    '  ⚠ 개정 이력(「c9부터 필수」)은 고치지 않는다 — 그건 그때의 사실이고 여기서도 안 잡는다.',
  ].join('\n'));
});
...
test('오디오 업로드 포맷이 C0와 발주서에서 갈라지지 않았다', () => {
  ...
  // ㉢비JS(가드계수 ㉠ 위험 1건은 이 판정): `스펙` 은 마크다운 코드펜스라 JS 렉서(`코드만`)의
  // 대상이 아니고, 굳이 걷으면 펜스 안 예시의 주석만 지워져 «스펙에 적힌 금지 예시»를 놓친다.
...
test('몽골어 해설이 DB에서 학생 눈까지 네 마디로 이어져 있다 (한 마디만 끊겨도 증상은 침묵이다)', () => {
   ...
   *   안 된다. 해설 줄이 몽골어 정본 상수(`몽골어폰트.본문`)를 지나는지 못박는다 — 킷(한글)
   *   폰트 4종에는 키릴 자형이 없다(정본 = src/테마.js 몽골어 절 · 등록은 tests/몽골어폰트.test.js 가 센다). */
  assert.ok(/해설: \{ fontFamily: 몽골어폰트\.본문/.test(화면),
    '해설 줄이 몽골어 정본 폰트(몽골어폰트.본문)를 안 쓴다 — 키릴이 두부로 뜬다');
});
...
/* ── 감시 문서 등록층 — 가드는 로직보다 등록층에서 샌다 ──────────────────────
 * 🔴 2026-08-09 실사고(유호 실측): L0 §3-3 값목록 줄이 「계약 c6 12종」에 멈춰
 *   `content.viewed` 가 빠져 있었다. 위 픽스처는 L0 모양(백틱 없는 한글 나열·SQL·표)까지
 *   못박고 있었는데 정작 그 파일을 안 읽고 있었다 — C0 만 재서 L0 가 통째로 사각.
 * ⚠ 넣는 것은 **문서(파생·설명층)뿐**이다. supabase/L0_스키마.sql·lib/이벤트검증.js 같은
 *   집행층은 전량 나열이 임무라 여기 넣으면 안 된다(그쪽은 계약 동기화 회귀가 따로 잰다). */
const 값목록감시문서 = { C0: C0경로, L0: L0경로 };

test('값목록 감시 등록이 살아 있다 (빠진 문서는 조용히 사각이 된다 — L0 가 그랬다)', () => {
  assert.ok(Object.values(값목록감시문서).includes(L0경로), 'L0 가 등록에서 빠졌다 — 08-09 사각 그대로다');
  for (const [이름, p] of Object.entries(값목록감시문서)) {
    assert.ok(fs.existsSync(p), `${이름} 경로가 죽었다(${p}) — 문서를 옮겼으면 등록도 같이 옮긴다`);
  }
});

test('감시 문서가 값목록을 통째로 복사하지 않았다 (복사본은 계약이 개정되는 날 갈라진다)', () => {
  const 값목록 = 계약.learning_events.값목록;
  const 값전부 = Object.values(값목록).flat();
  for (const [이름, 경로] of Object.entries(값목록감시문서)) {
    const md = fs.readFileSync(경로, 'utf8');
    for (const [키, 값들] of Object.entries(값목록)) {
      assert.ok(창최대값수(md, 키, 값들, 값전부) < 값들.length,
        `${이름}의 ${키} 이름 곁 한 창(±수백 자)에 값 ${값들.length}종이 전부 모여 있다 — 그 자리가 곧 값목록 복사본이고, 계약이 개정되면 이 문서만 낡는다.\n` +
...
/* ── 판·개수 주장 — 낡은 사본이 남기는 유일한 정확 신호 ───────────────────────
 * 전량 가드는 복사본을 **태어나는 날** 잡는다(그날은 전량이니까). 이미 낡은 사본은 계약이
 * 자라며 전량 미달이 되어 빠져나간다 — 08-09 실측: L0 의 12종 나열은 13종 계약 아래서
 * 12/13 이라 위 검사에 안 걸렸다. 낡음이 남기는 흔적은 산문의 「c6 12종」 주장이고, 그건
 * 기계적으로 판정된다: 판이 현행이 아니거나 개수가 어느 값목록과도 안 맞으면 이미 거짓말이다.
 * 🔑 역사 서술은 잡지 않는다(「현행 cN」 검사와 같은 축) — 「c4판 9종」처럼 판과 개수 사이에
 *   글자가 끼면 서술로 본다. 잡는 것은 붙은 모양(cN M종) 하나뿐 — 그 모양이 실사고 모양이다. */
const 판개수주장 = (text) => [...text.matchAll(/c(\d+)\s+(\d+)\s*종/g)]
  .map((m) => ({ 판: `c${m[1]}`, 개수: Number(m[2]), 자리: m[0] }));

test('탐지력 픽스처 — 낡은 「cN M종」 주장을 실제로 잡고 역사 서술은 놓아준다', () => {
  assert.deepEqual(판개수주장('**`event_type` 값목록** — **계약 c6 12종**(정본은 계약 파일이고 여기는 파생)'),
    [{ 판: 'c6', 개수: 12, 자리: 'c6 12종' }], 'L0 실사고 그 모양을 놓쳤다');
  assert.equal(판개수주장('⚠이 줄이 c4 9종에 멈춰 있던 것을 2026-08-06 외부 검토가 잡았다').length, 1,
    '붙은 모양(c4 9종)은 역사여도 잡히는 게 맞다 — 그래서 감시 문서는 역사를 「c4판 9종」으로 띄어 적는다');
  assert.equal(판개수주장('c4판 9종에 멈춘 것을 2026-08-06 외부 검토가 잡았다').length, 0,
    '역사 서술(「c4판 9종」)을 주장으로 셌다 — 그러면 이력 문장이 전부 빨개진다(F272 축)');
});

test('감시 문서의 「cN M종」 주장이 현행 계약과 맞는다 (낡은 사본은 전량 미달이라 위 검사를 빠져나간다)', () => {
  const 개수들 = new Set(Object.values(계약.learning_events.값목록).map((v) => v.length));
  for (const [이름, 경로] of Object.entries(값목록감시문서)) {
    for (const 주장 of 판개수주장(fs.readFileSync(경로, 'utf8'))) {
      assert.equal(주장.판, 계약.버전,
        `${이름}의 「${주장.자리}」 — 판이 현행(${계약.버전})이 아니다. 낡은 판을 현재형으로 주장하는 줄이다.\n` +
        '  고치는 법: 개수·판을 지우고 계약 파일을 가리킨다. 역사 서술이면 「c4판 9종」처럼 띄운다.');
      assert.ok(개수들.has(주장.개수),
        `${이름}의 「${주장.자리}」 — 어느 값목록도 ${주장.개수}종이 아니다(계약과 어긋난 숫자).`);
    }
  }
});

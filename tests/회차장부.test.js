...
// ── 아물었나 — 이상 «이력»과 이상 «상태»를 가른다 (2026-08-24 · F482 처방) ────────
/* 실측 근거: transcribe-batch 가 08-21 새벽 pg_net 타임아웃 1회를 낸 뒤 505회 연속 성공했는데
 * 7일 창 합산 판정이 사흘째 🔴 를 들었다 — 상시 경보는 진짜 경보를 죽인다. 판정은 도구의
 * 순수 함수 하나(`아물었나`)가 지고, 여기는 그 갈래 전수를 값으로 못박는다. */

const { 아물었나, 아묾문턱ms } = require('../tools/회차장부.js');

test('아물었나 — 이상 뒤 발사가 이어졌고 24시간+ 재발 0 이면 true (그 전엔 false)', () => {
  const 지금 = Date.parse('2026-08-24T12:00:00Z');
  const 행 = (마지막이상, 마지막발사) => ({ 타임아웃: 1, 마지막이상, 마지막발사 });
  assert.equal(아물었나(행('2026-08-21T04:30:00Z', '2026-08-24T11:50:00Z'), 지금), true,
    '사흘 전 1회 + 그 뒤 발사 계속 — 아물었는데 생이상으로 든다(상시 경보가 진짜 경보를 죽인다)');
  assert.equal(아물었나(행('2026-08-24T02:00:00Z', '2026-08-24T11:50:00Z'), 지금), false,
    '이상이 24시간 안이다 — 하루 무재발 전에 접으면 간헐 재발이 영영 안 보인다');
  assert.equal(아물었나(행('2026-08-21T04:30:00Z', '2026-08-21T04:30:00Z'), 지금), false,
    '마지막 발사가 곧 마지막 이상이다 — 그 뒤로 안 돈 잡을 아묾으로 읽으면 죽음이 회복으로 보인다');
});

test('아물었나 — 이상 0 이면 null(판정 자체가 없다) · 못 읽는 시각은 생이상 쪽으로 접는다', () => {
  assert.equal(아물었나({ 성공: 5, 마지막발사: '2026-08-24T11:50:00Z' }), null,
    '이상 이력이 없는 잡에 아묾을 말하면 「아문 적 있다」는 거짓이 된다');
  assert.equal(아물었나({ 실패: 1, 마지막이상: null, 마지막발사: '2026-08-24T11:50:00Z' }), false,
    '시각을 못 읽으면 생이상이다 — 새는 방향이 「통과」면 안 되는 자리');
  assert.ok(아묾문턱ms === 24 * 3600 * 1000,
    '문턱이 하루에서 움직였다 — 매일 잡도 최소 한 주기를 재발 없이 보여야 한다는 근거와 함께 옮겨라');
});

test('아묾은 판정 어휘(0·1·2)를 늘리지 않는다 — 형제 rot-check 의 아는판정이 닫힌 목록이다', () => {
  /* 소스 층 못박기: json 통로의 판정 식이 여전히 0/1 이분이고, 아묾은 별도 칸으로 실린다.
   * 원문이 아니라 주석 뺀 판을 본다(소스검사통로 래칫) — 주석이 낱말을 대신 들면 영원히 초록이다. */
  const { 코드만: 정제 } = require('./lib/소스검사.js');
  const 도구코드 = 정제(도구소스);
  assert.match(도구코드, /판정: \(안적힘 > 0 \|\| 이상 > 0\) \? 1 : 0/,
    '판정 식이 바뀌었다 — 새 판정 값은 형제(rot-check `아는판정`)부터 열어야 한다');
  assert.match(도구코드, /안적힘, 이상, 아묾,/, 'json 에 아묾 칸이 없다 — 경보를 접으면 이력은 남겨야 한다');
});

...
test('G3-6 App.js — 겉테 고정 치수 글자 일곱이 전부 자리별 상한 prop 을 지난다', () => {
  const 앱 = 코드만(소스('App.js'));
  assert.match(앱, /import \{[^}]*글자배율상한[^}]*\} from '\.\/src\/테마';/,
    'App.js 가 글자배율상한을 테마에서 안 빌린다 — 값을 따로 적으면 두 곳이 갈린다');
  /* 09-06 — 겉테글 셋 = 답장 · 어제의 나 · 밤 라디오(유튜브 링크 한 장 · 철학 Ⅱ-1 셋째 줄 v1.22). 합 7. */
  for (const [자리, 셈] of [['시스템글', 1], ['미달글', 1], ['겉테글', 3], ['검수문글', 2]]) {
  ...
  assert.equal((앱.match(/maxFontSizeMultiplier=\{글자배율상한\}/g) || []).length, 7,
    '겉테 상한 자리 합이 7이 아니다 — 늘었으면 이 셈을, 줄었으면 빠진 자리를 본다');
});

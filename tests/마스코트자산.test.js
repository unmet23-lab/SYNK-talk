/* 마스코트 자산 회귀 — assets/마스코트/ 펠트 4컷 (그림 정본 = SYNK-appsscript
 * docs/캐릭터/펠트코랄_0815 누끼 · 주인 = 디자인_토큰.json 재질.펠트.정본사진 · 유호 확정 08-19).
 * 옛 「마스코트_렌더」 6컷은 08-23 소각 — 컷 어휘의 몽글_ 접두 봉인은 tests/마스코트생명.test.js
 *   (그 접두는 09-05 에 `재염색_` 에서 옮겨 왔다 — 재염색판이 그날 내려갔다).
 *
...
test('마스코트 자산 — 표정 어휘의 컷이 전부 실WebP 로 있다 (목록은 정본에서 파생)', () => {
  /* 표정컷은 {캐릭터: {표정: 컷}} 중첩이다(08-31 까몽 반입) — 캐릭터 벌을 평탄화해 전량을 센다. */
  const 컷들 = [...new Set(Object.values(표정컷).flatMap((벌) => Object.values(벌)))];
  /* 분모 12 = 몽글 4(본체·놀람·눈웃음·눈감음) + 까몽 8. 깜찍·별눈의 펠트판이 서면 같이 오른다. */
  assert.ok(컷들.length >= 12, `컷 어휘를 못 읽었다(분모 ${컷들.length}) — 통과가 아니라 미실행이다`);
  for (const 컷 of 컷들) {

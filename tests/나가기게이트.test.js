...
test('문구 — 합산 수를 「발화」라고 부르지 않는다(게임 메일·고름도 세는 수다)', () => {
  const 원문 = fs.readFileSync(path.join(ROOT, 'src', '도착확인.js'), 'utf8');
  assert.match(원문, /아직 보내지 못한 것이/, '카드 머리가 없다 — 문구를 바꿨으면 이 검사도 같이');
  assert.ok(!/보내지 못한 발화가/.test(원문), '「발화」 문구가 남았다 — 이제 이 수는 발화만이 아니다');
});

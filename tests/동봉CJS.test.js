...
test('배정 배선 동봉 후보 — 표에 오르기 전에도 CJS 를 유지한다', () => {
  /* 아직 어느 동봉.json 에도 없지만 배정 배선 ④단계가 deliver 표에 올릴 파일들이다
   * (대기열 P0 G1 · §6-8 규칙 4 「배정·제출이 같은 조립」). ESM 로 되돌리면 그 커밋이
   * 위 전량 검사에 닿기 전에 부팅 전사를 밟는다 — 후보 단계부터 잡는다.
   * ④단계가 표에 올리면 이 목록에서 빼도 된다(전량 검사가 이어받는다). */
  for (const 경로 of ['lib/게임스냅샷.js', 'contents/교수멘탈문항.js']) {
    const 소스 = fs.readFileSync(path.join(ROOT, 경로), 'utf8');
    assert.ok(CJS인가(소스), `${경로} — 배정 동봉 후보가 CJS 가 아니다`);
  }
});

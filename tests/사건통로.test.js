...
test('기억이 이미 더 새것이면 왕복 없이 그것을 준다 — 토큰되살리기의 빠른 갈래', async () => {
  /* 08-31 개정 — 옛 판은 「둘째 호출이 낡은 A1 로 나가 401 을 산 뒤 이 갈래로 산다」를 재고
     있었는데, 통로가 첫 발사부터 기억 토큰을 쓰게 되면서(아래 감사 S1-3 회귀) 그 전제가 사라졌다.
     이 갈래 자체는 여전히 실물이다(갱신 경합 직후 등) — 창구에 직접 물어 그 성질만 잰다. */
  const 판 = await 로그인해둔판({ 함수: [만료, 성공], 갱신: 갱신됨 });
  await 판.통로.부르기('progress', 'A1');   // 여기서 A2 로 갱신된다
  const 갱신횟수 = 판.갱신왕복().length;

  const 토큰 = await 판.인증.토큰되살리기('A1'); // 낡은 토큰으로 직접 묻는다
...
/* ─── 08-31 감사 S1-3·G1-4 — 만료 «이후»의 이중 왕복 세금 ─────────────────── */

test('🔑 갱신 뒤 둘째 호출은 401 을 «안 산다» — 첫 발사가 기억 토큰으로 나간다 (감사 S1-3·G1-4)', async () => {
  const 판 = await 로그인해둔판({ 함수: [만료, 성공, 성공], 갱신: 갱신됨 });

  await 판.통로.부르기('progress', 'A1');           // 만료 → 갱신(A2) → 재시도 성공
  const 본문 = await 판.통로.부르기('tasks', 'A1');  // 화면 prop 은 여전히 낡은 A1 이다

  assert.equal(본문.ok, true);
  assert.equal(판.함수왕복().length, 3, '왕복 셋 = 첫 호출(A1) + 재시도(A2) + 둘째 호출(A2 한 방)');
  assert.deepEqual(판.함수왕복().map((r) => r.토큰), ['A1', 'A2', 'A2'],
    '둘째 호출의 «첫 발사»가 이미 A2 여야 한다 — 낡은 prop 으로 401 을 한 번 사고 재시도로 성공하는 이중 왕복은 자기 치유라 영영 안 보이는 세금이었다(「오후만 되면 앱이 느려진다」)');
  assert.equal(판.갱신왕복().length, 1, '갱신은 여전히 한 번뿐이다');
});

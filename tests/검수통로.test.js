...
function 뷰열() {
  ...
  /* 지역명을 §5 쪽 `몸` 과 가른다 — 한 이름이 원문·정제 두 뜻이면 계수기가 정제 쪽까지
   * 원문으로 물들인다(거짓 위험 · 검수큐 뷰열 과 같은 처분). */
  const 정의전문 = 전부[전부.length - 1][1];
  const 셀렉트절 = 정의전문.slice(0, 정의전문.indexOf('from engine.submissions'));
  const 이름 = [];
  for (const 줄 of 셀렉트절.split('\n').map((l) => l.replace(/--.*$/u, '').trim()).filter(Boolean)) {
 ...
 *  🔑 정제판(`코드`)에서 뽑는다 — 배열 안 주석에 적힌 `'event_id'`(「안 싣는다」 설명)가
 *  이름으로 세어지면 아래 부정 단언이 거짓 적색이 된다(가드계수 ㉠ · 앵커는 코드라 정제와 무관). */
function 큐열() {
  /* 지역명을 `m` 과 가른다 — 이 파일 첫 `m`(:60)이 원문 파생이라, 같은 이름이면 계수기가
   * 이 정제 추출까지 원문으로 물들인다(「첫 선언이 이긴다」 근사 · 거짓 위험). */
  const 짝 = /const 큐열 = \[([\s\S]*?)\];/u.exec(코드);
  assert.ok(짝, '`큐열` 목록을 못 찾았다 — 앵커가 낡았다(대조가 통째로 미실행이다)');
  return [...짝[1].matchAll(/'([\w]+)'/gu)].map((x) => x[1]);
}
...
test('승인은 **한 트랜잭션**에 넷을 쓴다 — 하나라도 새면 확정이 반쪽이 된다', () => {
   ...
   * `몸` 은 이미 정제 경유다(`함수본문`→`구간` 이 `코드만` 을 내장) — 여기 위험으로 섰던 것은
   * 뷰열 지역명과의 「한 이름 두 뜻」 충돌(거짓 위험)이었고, 처방은 감싸기가 아니라 이름 가르기였다. */

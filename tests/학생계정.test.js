...
/* 타이핑 편의 — 숫자만 치면 접두가 붙고, 그 밖(직원 원료·미완성 입력)은 그대로다.
 * 숫자만은 직원 번호가 원리상 될 수 없다(직원 번호는 글자가 반드시 든다 — `직원번호입력`). */
test('학생번호접두: 숫자만 3자리부터 접두가 붙고, 나머지는 입력 그대로다', () => {
  assert.equal(학생번호접두('042'), 'SYNK-042', '숫자만 친 학생번호에 접두가 안 붙었다');
  assert.equal(학생번호접두('SYNK-042'), 'SYNK-042', '이미 붙은 값을 다시 만졌다 — 타이핑이 교착한다');
  assert.equal(학생번호접두('L01'), 'L01', '직원 번호 원료(글자 포함)를 만졌다');
  assert.equal(학생번호접두('04'), '04', '아직 치는 중(3자리 미만)인데 만졌다');
});

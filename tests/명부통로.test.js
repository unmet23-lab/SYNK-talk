...
const 붓는곳 = {
  ...
  '시험 시딩(리허설 픽스처)': [
    ...
    'tools/반피드백왕복시험.js',
    /* 생성(§12 픽스처 대군) — §4-1 값 15 전량·마감·구제·tasks 상태 칸을 재려면 갈래 수만큼의
     * 합성 학생이 필요하다(값마다 한 명 — 한 학생을 겹쳐 쓰면 unique(learner_id, assign_date)가
     * 픽스처끼리 엉킨다). 전부 리허설 전용 · 시험 표식(student_code 접두)으로 가른다. */
    'tools/생성왕복시험.js',
    /* 관통(세로줄) — ㉠㉡㉢ 세 층이 «동시에» 찬 학생이 실재해야 잰다. 상태를 다르게 심은 A·B
     * 두 명을 회차마다 새로(같은 날 재실행 함정 우회 · `t{base36}-관통A/B`) — 리허설 전용. */
    'tools/관통왕복시험.js',
    /* 관찰(c14 · 08-31) — 반피드백과 **같은 사유**다: 담당 반 밖 학생이 실재해야 「404 로 막혔다」와
     * 「원래 없다」가 갈린다. 더해 이 시험은 **동의 행이 하나도 없는 학생**을 요구한다 — c14 ③ 의
     * 「운영 기록 계열은 동의 게이트를 안 지난다」가 참인지는 그런 학생을 세워 넣어 봐야만 안다.
     * 고정 코드 둘(RTOB-X1·RTOB-Y1)만 재사용하고 회차마다 늘리지 않는다. */
    'tools/관찰왕복시험.js',
    'tools/검증_마이그레이션.sh',
  ],
};

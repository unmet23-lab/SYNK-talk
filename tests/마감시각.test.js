...
test('계약이 c17 이고 두 이름을 들고 있다', () => {
  /* 판 핀 — c10(due 신설 판) → … → c13(성향 확인 · 08-22) → c14(교실 수집 · 08-31) →
   * c16(접기 어휘 fold_date·promote_ver · 09-02) → c17(첫 주 소급 불가 어휘 ⓐ · 09-11)으로 올렸다.
   * 이 핀의 몫은 「계약 판이 오르면 이 회귀 파일을 열어 due 회귀가 여전히 유효한지 본 사람이 있다」를
   * 남기는 것이다. c17 확인: due 는 첫 주 어휘와 무관하다(payload 키·값목록만 늘었다 · 물리 0).
   * ⚠ c17 ⓐ 는 «어휘만»이라 짝 CHECK 는 아직 `submissions_due_paired_c16` 그대로다 — 접미 교체 조각은
   *   ⓑ(물리 커밋) 몫이고, 그 틈은 tests/계약.test.js·tests/L0스키마.test.js 가 빨갛게 센다(의도된 빨강). */
  assert.equal(계약.버전, 'c17', `계약 버전이 ${계약.버전} 다 — 물리와 갈리면 「c17 계약 + c16 물리」가 초록으로 보인다`);

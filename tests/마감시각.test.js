...
test('계약이 c14 이고 두 이름을 들고 있다', () => {
  /* 판 핀 — c10(due 신설 판) → c11 → c12 → c13(성향 확인 estimate.responded · 08-22) →
   * c14(교실 수집 goal.responded·observation.noted · 08-31)로 올렸다. 이 핀의 몫은 「계약 판이
   * 오르면 이 회귀 파일을 열어 due 회귀가 여전히 유효한지 본 사람이 있다」를 남기는 것이다.
   * c14 확인: due 는 새 사건·목표/관찰 그룹과 무관하고(목표 카드에는 마감이 없다 — 하루 1회
   * 게이트는 due 가 아니라 goal_daily_once 색인이 진다), 짝 CHECK 는
   * `submissions_due_paired_c14` 로 값 그대로 이름만 갈았다(20260831130000). */
  assert.equal(계약.버전, 'c14', `계약 버전이 ${계약.버전} 다 — 물리와 갈리면 「c14 계약 + c13 물리」가 초록으로 보인다`);

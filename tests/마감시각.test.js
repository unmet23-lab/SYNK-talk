...
test('계약이 c12 이고 두 이름을 들고 있다', () => {
  /* 판 핀 — c10(due 신설 판)에서 c11(라디오 소개정), c12(상태기반 과제선택 착지 · 08-21)로
   * 올렸다. 이 핀의 몫은 「계약 판이 오르면 이 회귀 파일을 열어 due 회귀가 여전히 유효한지
   * 본 사람이 있다」를 남기는 것이다. c12 확인: due 는 payload 3칸·화이트리스트와 무관하고
   * 짝 CHECK 는 `submissions_due_paired_c12` 로 값 그대로 이름만 갈았다(20260821060000). */
  assert.equal(계약.버전, 'c12', `계약 버전이 ${계약.버전} 다 — 물리와 갈리면 「c12 계약 + c11 물리」가 초록으로 보인다`);

...
test('계약이 c11 이고 두 이름을 들고 있다', () => {
  /* 판 핀 — c10(due 신설 판)에서 c11(라디오 소개정)로 올렸다. 이 핀의 몫은 「계약 판이 오르면
   * 이 회귀 파일을 열어 due 회귀가 여전히 유효한지 본 사람이 있다」를 남기는 것이다. */
  assert.equal(계약.버전, 'c11', `계약 버전이 ${계약.버전} 다 — 물리와 갈리면 「c11 계약 + c10 물리」가 초록으로 보인다`);

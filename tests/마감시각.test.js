...
test('계약이 c13 이고 두 이름을 들고 있다', () => {
  /* 판 핀 — c10(due 신설 판) → c11 → c12 → c13(성향 확인 estimate.responded · 08-22)으로
   * 올렸다. 이 핀의 몫은 「계약 판이 오르면 이 회귀 파일을 열어 due 회귀가 여전히 유효한지
   * 본 사람이 있다」를 남기는 것이다. c13 확인: due 는 새 사건·확인 그룹과 무관하고
   * 짝 CHECK 는 `submissions_due_paired_c13` 로 값 그대로 이름만 갈았다(20260822150000). */
  assert.equal(계약.버전, 'c13', `계약 버전이 ${계약.버전} 다 — 물리와 갈리면 「c13 계약 + c12 물리」가 초록으로 보인다`);

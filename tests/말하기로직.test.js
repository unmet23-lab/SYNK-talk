...
const { 흐름id, 다음시도번호, 항목추가, 직렬화, 역직렬화, 학습출석, 전송기록, 밀린것, 보낼것, 되듣기기록, 되듣기보낼것, 배달상태 } = require('../lib/제출로그.js');
...
test('말하기화면: 막혔으면 큐를 안 민다 — 두 자리 다', () => {
  ...
  /* 🔑 `되듣기보낼것` 이 이 이름을 **접미로 품는다** — 경계를 안 박으면 되듣기 쓸이 늘 때마다
   *   이 숫자가 저절로 올라 「두 자리가 막힘을 본다」는 이 검사가 조용히 뜻을 잃는다. */
  assert.equal(
    (화면원문.match(/(?<!되듣기)보낼것\(로그참조\.current, 막힘참조\.current\)/g) || []).length, 2,

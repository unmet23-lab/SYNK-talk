...
/* «코드만»으로 읽는다(소스검사통로 래칫 · F401) — 원문이면 낭독 카드 주석의 낱말이 아래
 * 존재 단언을 영원히 초록으로 만들고, 마스코트 부재 단언은 주석 언급 하나로 빨개진다. */
const 화면 = 코드만(fs.readFileSync(require('node:path').join(__dirname, '..', 'src', '말하기화면.js'), 'utf8'));
...
test('🚫 낭독 «카드 안»에 마스코트를 안 얹었다 — 08-14 판정의 남은 몫(08-23 갱신: 화면의 몽글은 연기로만, 카드 층은 여전히 금지)', () => {
  /* 창을 낭독 카드 JSX 의 닫힘(`/>`)까지로 좁힌다 — 08-23 부터 화면 겉테에는 몽글 인트로
   * 연기가 살아서(tests/마스코트배선.test.js), 고정 900자 창은 카드 «뒤» 형제를 오탐한다. */
  const 카드만 = 낭독블록.slice(0, 낭독블록.indexOf('/>') + 2);
  assert.ok(카드만.includes('prompt_id'), '낭독 카드 경계를 못 찾았다 — 아래 단언이 빈 창을 잰다');
  assert.ok(!/마스코트/.test(카드만), '낭독 카드 안에 마스코트가 들어왔다');
});

 ...
 *   ③ **청취 문턱** — 재료가 없을 때 「하한 3초」로 접히는 것이 **사실대로 드러나는가**, 그리고
 *      **재료를 만드는 생산자가 저장소에 남아 있는가**(08-10 방향 전환 — 아래 `쓰기표기` 머리말).
...
/** 「그 열에 **쓴다**」의 표기. 판별식은 여기 한 자리에만 둔다 — 아래 두 검사(실저장소 존재 ·
 *  픽스처 탐지력)가 같은 것을 봐야 한 쪽만 고쳐지는 일이 없다.
 *
 *  ⚠ **08-10 실측: 옛 판별식은 실제 생산자를 못 봤다.** 옛 모양은 `set\s+stt_segments\s*=` 라
 *  `set` 과 열 이름이 **붙어 있는** 표기만 봤는데, 생산자(`supabase/functions/transcribe/index.ts`)
 *  는 `set` 과 떨어진 조각(`` sql`, stt_segments = ${...}` ``)으로 쓴다. 그래서 생산자가 08-09 에
 *  실제로 섰는데도(talk `8bb6f35`) 이 검사는 **계속 초록**이었다 — 사람을 부르라고 놓은 덫이
 *  정확히 그 날 안 울렸다. 가드는 로직이 아니라 **표기**에서 새고, 새는 방향은 언제나 「통과」다.
 *  ⇒ 옛/새 판별식을 같은 저장소에 돌린 실측: 옛 0건 · 새 1건(생산자 그 파일 하나 · 거짓양성 0). */
const 쓰기표기 = /(?:^|[\s,(`])stt_segments\s*=(?!=|>)|stt_segments\s*,[\s\S]{0,400}?\)\s*values/u;

/** 훑기 — 실저장소에서 그 표기를 쓰는 파일 목록. */
function 쓰는곳찾기() {
  ...
  const 걷기 = (디렉터리) => {
    for (const 항목 of fs.readdirSync(디렉터리, { withFileTypes: true })) {
      ...
      if (쓰기표기.test(fs.readFileSync(길, 'utf8'))) 쓰는곳.push(길);
    }
  };
  for (const d of 훑을곳) {
...
test('🔴 재료가 실제로 온다 — `stt_segments` 생산자가 저장소에 있다', () => {
  /* 옛 판(08-09)은 「생산자가 0줄이다」를 지키는 덫이었다. 생산자가 선 지금(08-10) 방향을 뒤집는다:
   * 사라지면 빨개진다. 생산자가 없어지면 게이트 ①은 **조용히** 하한 3초로 되돌아가고
   * (`청취문턱` 이 `재료:false` 로 접힌다) 증상은 「승인이 잘 된다」뿐이라 아무도 안 본다. */
  const 쓰는곳 = 쓰는곳찾기();
  assert.ok(쓰는곳.length > 0,
    '`stt_segments` 에 쓰는 코드가 저장소에서 사라졌다.\n'
    + '  → 게이트 ①의 재료가 없어져 청취 문턱이 언제나 하한 3초로 접힌다(미측정이 통과로 읽힌다).\n'
    + '  생산자를 되살리거나, 게이트 ①을 접겠다는 판정을 먼저 문서에 남겨라(지우지 말 것)');
});

test('🔴 탐지력 — 「쓴다」 표기는 잡고 「읽는다」 표기는 안 잡는다 (양방향 · 픽스처가 진다)', () => {
  /* 실저장소 검사는 「있나 없나」만 본다 — 탐지 능력은 여기서 못박는다(실저장소가 바뀌어도 안 흔들린다). */
  const 잡아야 = [
    'const 조각 = sql`, stt_segments = ${sql.json(x)}, stt_confidence = ${c}`;', // 실제 생산자 표기
    'update submissions set stt_segments = $1 where id = $2',
    'insert into submissions (id, stt_segments, created_at) values ($1, $2, $3)',
  ];
  const 안잡아야 = [
    'select s.stt_segments, s.stt_confidence from submissions s',
    "const 열 = ['stt_segments', 'stt_confidence', 'code_switch_spans'];",
    'const 문턱 = 청취문턱(재료.stt_segments);',
    'if (구간 && 구간.stt_segments.length) 구간실림 += 1;',
    'type 세그먼트값 = { stt_segments: unknown[] };',
  ];
  for (const s of 잡아야) assert.ok(쓰기표기.test(s), `「쓴다」를 놓쳤다: ${s}`);
  for (const s of 안잡아야) assert.ok(!쓰기표기.test(s), `「읽는다」를 「쓴다」로 잡았다(거짓양성): ${s}`);
});
...
test('🔴 문턱을 옮기는 것으로는 안 풀린다 — 08-10 실측을 상수 자리에 못박는다', () => {
  /* 재료가 선 뒤 첫 실측(talk `ccee460`): 59.6초 14조각의 `avg_logprob` 이 서로 다른 값 **3개**뿐이고
   * 경계가 27·56초 — Whisper 30초 디코딩 윈도우다. 같은 윈도우의 조각은 값이 복사돼 온다.
   * ⇒ 30초 미만 발화는 전 조각이 통째로 저신뢰이거나 통째로 고신뢰라, 문턱을 어디로 옮겨도
   *   `청취문턱` 이 낼 수 있는 값은 「발화 전체 길이」 아니면 「하한 3초」 **둘뿐**이다.
   * 이 검사는 값을 옹호하는 것이 아니라, 「튜닝하면 되겠지」로 상수를 만지는 손을 **먼저 멈춰
   * 세우려고** 있다. 진짜 손잡이는 세그먼트별 신뢰도를 실제로 주는 재료 쪽이다.
   * 🚫 실학생 저신뢰 발화를 재기 전에는 이 값을 「검증됐다」로 인용하지 않는다(합성만 0.87~0.90). */
  assert.equal(저신뢰문턱, 0.7,
    '`저신뢰문턱` 을 옮겼다. 08-10 실측은 이것이 **값이 아니라 해상도(30초 윈도우)의 문제**라고\n'
    + '  말한다 — 옮겨도 게이트가 내는 값은 둘뿐이다. 옮길 근거가 생겼다면(세그먼트별 신뢰도를\n'
    + '  주는 재료 확보 · 실학생 저신뢰 구간 실측) 그 실측을 `lib/검수확정.js` 상수 머리말에\n'
    + '  먼저 적고 이 검사를 같이 고쳐라.');
});

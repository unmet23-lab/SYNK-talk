...
test('🔴 값목록을 서버가 다시 적지 않는다 — 두 곳에 적으면 갈라지고 증상은 「어떤 학생만」이다', () => {
  /* 같은 파일을 :94 는 `코드만` 으로 열고 여기만 원문으로 열고 있었다(F401 전환 때 빠진 자리).
     원문이면 auth 의 **주석**이 값을 한 번 말하는 순간 이 검사가 거짓 적색이 된다 — 그리고
     그 주석은 「여기 적지 마라」는 설명일 확률이 가장 높다(설명이 자세할수록 더 잘 먼다).
     🔑 이름을 `src` 에서 가르는 이유: 이 파일엔 `const src` 가 여섯 번 나오는데 어떤 것은
     원문이고 어떤 것은 정제다. `가드계수` 는 파일 하나를 한 스코프로 근사하고 **첫 선언이
     이겨서**(:83 원문) 정제한 자리까지 원문으로 물든다 — 뜻이 둘이면 이름도 둘이어야 한다. */
  const 코드 = 코드만(읽기('supabase/functions/auth/index.ts'));
  for (const 값 of ['study', 'work', 'culture', 'undisclosed']) {
    assert.ok(!new RegExp(`['"]${값}['"]`).test(코드), `🔴 auth 가 ${값} 를 직접 적었다`);
  }
});

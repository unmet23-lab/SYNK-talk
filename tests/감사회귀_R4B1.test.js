...
test('D6-16 「borderRadius: 20, padding: 22」 리터럴이 src 에 0건이다 — 정본은 테마.판눈금 하나', () => {
  ...
  // 09-11 승인된 편지 작업실은 별도 여백을 쓴다. 기존 공통 카드들의 토큰 사용은 계속 확인한다.
  assert.deepEqual(소비처.sort(), [
    '답장화면.js', '라디오화면.js', '막힘카드.js', '말하기화면.js', '목표카드.js',
    '보고서교정화면.js', '생성카드.js', '서류관문화면.js', '알바변명화면.js', '어제의나.js',
  ].sort(), '공통 카드의 판눈금 사용이 달라졌다 — 해당 화면의 현재 디자인과 대조한다');
});

...
test('G2-3 오류경계가 라우팅 묶음을 감싼다 — key={화면} 로 전환 때 경계 상태가 자연 리셋된다', () => {
  ...
  for (const 이름 of ['말하기', '답장', '시스템', '비번변경', '초기화', '검수', '강사',
    '문구감수', '반피드백', '나침반', '회고', '관찰', '학생기록', '어제', '검수문']) {
...
test('G2-1 백버튼 — hardwareBackPress 등록·해제와 지도(직원→시스템 · 나머지→말하기)가 소스에 있다', () => {
  ...
  assert.match(앱, /const 시스템행 = \['검수', '강사', '문구감수', '반피드백', '나침반', '회고', '관찰', '학생기록', '몽글'\];/,
    '직원 9화면 지도가 각 화면의 돌아가기 prop(→시스템)과 갈라졌다');
  assert.match(앱, /<학생기록화면 토큰=\{세션\.access_token\} 돌아가기=\{\(\) => set화면\('시스템'\)\} \/>/,
    '학생 기록의 돌아가기 prop이 백버튼 지도(→시스템)와 갈라졌다');

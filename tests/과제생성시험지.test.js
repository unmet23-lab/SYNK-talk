...
test('E4 ⑤ — 디스크 시험지 == 재조립 바이트(절차 재현) · 풀 불변 · 경로 한 원천', () => {
  ...
  /* 🔴 v2(08-26) — 「불변·추가만」을 «한 번» 깬 판이다(goal_candidate 10종 전부 문장형으로).
   * 이 줄이 그 사실의 자물쇠다: 다음에 또 풀을 고치면 여기서 소리가 나고, 판을 안 올렸다면
   * 그것이 곧 «조용한 표류»라는 뜻이다. 까닭 전문은 풀 파일의 `규칙` 칸. */
  assert.equal(풀.판, '스냅샷풀.v2');
  assert.equal(풀.사례.length, 160, '풀은 불변·추가만 — 줄었다면 옛 시험지 해시가 소급으로 거짓이 된다');
});

/* ── 목표 값의 «모양» — 유호 지적 08-25 「kpop 가사 이해라는 걸 왜 그대로 두느냐」 ──────────
 * 학생이 실제로 받는 질문은 `lib/나침반문항.js` 의 「이번 시즌 목표는 뭔가요?」(유호 확정 09-01)이고
 * 정본은 그 답을 「학생이 **제 말로 적은**」이라 부른다(lib/시즌맥락.js). 명사형 이름표로 되돌아가면
 * 운영에서 안 올 모양으로 시험하는 것이라 여기서 막는다. */
test('시험지 목표는 «학생이 제 말로 적은 문장» 모양이다 — 명사형 이름표로 되돌아가면 빨강', () => {
  const 목표들 = [...new Set(시험지.사례.map((c) => c.goal).filter(Boolean))];
  assert.ok(목표들.length >= 3, `목표 값이 ${목표들.length}종뿐이다`);
  const 한국어목표 = 목표들.filter((g) => /[가-힣]/.test(g));
  for (const g of 한국어목표) {
    assert.match(g, /(어요|아요|여요|싶어요|해요)$/, `명사형 이름표다 — 학생이 적을 말이 아니다: 「${g}」`);
  }
  /* 키릴 하나 — 「맥락의 언어는 출력 언어를 바꾸지 않는다」(프롬프트 v2 부터 있던 규칙)가
   * 과제생성에서 한 번도 시험된 적이 없었다. 실학생은 몽골 학생이라 이 칸에 키릴이 온다. */
  const 키릴 = 목표들.filter((g) => /[А-Яа-яӨөҮү]/.test(g));
  assert.equal(키릴.length, 1, `키릴 목표가 정확히 하나여야 한다(지금 ${키릴.length}) — 언어 안정성 시험 자리`);
  const 키릴사례 = 시험지.사례.filter((c) => c.goal && /[А-Яа-яӨөҮү]/.test(c.goal));
  assert.ok(키릴사례.length >= 3, `키릴 사례가 ${키릴사례.length}건 — 한 번 흔들려도 잡히게 셋 이상`);
});
...
test('풀 순서 그대로 · 앵커 id 고정 · 채점표 판·통과선 동봉', () => {
  ...
  /* 채점표 v3(08-26) — Z4·Z5·A4 증보. 이 리터럴이 「앵커가 바뀌면 판이 오른다」의 자물쇠다. */
  assert.deepEqual(시험지.앵커['0점조건'].map((a) => a.anchor_id), ['Z1', 'Z2', 'Z3', 'Z4', 'Z5']);
  assert.deepEqual(시험지.앵커['1점앵커'].map((a) => a.anchor_id), ['A1', 'A2', 'A3', 'A4']);

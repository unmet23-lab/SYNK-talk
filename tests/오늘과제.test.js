...
const { 몽골날짜, 멱등키, 오늘과제, 따라말하기문장, 따라말하기출처, 화면과제, 제출사건, 열람사건, 되듣기사건,
...
test('AI 자리가 비면 전날 문장으로 내려가고 **강등으로 남는다** (§6-4) — 단 어제가 «진짜 과제»였을 때만', () => {
  const r = 오늘과제({ 날짜, 전날문장: '저는 학교에 갑니다', 전날출처: '생성' });
  assert.equal(호흡(r, 2).문장, '저는 학교에 갑니다');
  assert.equal(r.degraded, true, '강등을 안 적으면 「막힌 것이 통과한 것처럼」 보인다');
});

/* 🔴 08-26 계약 개정(유호 지시) — 「전날」이 어제도 강등이었는지를 «안 봐서» 장애가 사흘이면
 * 같은 문장이 사흘 나갔다. 이제 어제가 생것(생성·교정문)일 때만 전날이고, 그 밖은 예비다.
 * 전문·회귀는 tests/예비과제.test.js. */
test('🔴 어제도 강등이었으면 전날을 되풀이하지 않는다 — 「같은 문장 사흘」을 끊는 자리', () => {
  const r = 오늘과제({
    날짜, 전날문장: '저는 학교에 갑니다', 전날출처: '전날',
    급수: 'Lv5', learner_id: '11111111-2222-3333-4444-555555555555',
  });
  assert.notEqual(호흡(r, 2).문장, '저는 학교에 갑니다', '어제 문장을 또 냈다 — 되풀이가 안 끊겼다');
  assert.equal(r.출처, '예비');
  assert.equal(r.degraded, true, '예비도 강등이다 — 정상 생성과 행에서 갈려야 한다');
});

test('전날 문장도 없고 예비도 못 고르면 고정 도입 과제 — 그래도 강등이다', () => {
  const r = 오늘과제({ 날짜 });
  assert.equal(호흡(r, 2).문장, 도입.따라말하기);
...
test('오늘 스냅샷이 내일의 「전날 문장」이 된다 — 강등 경로가 자기 출력으로 돈다', () => {
  ...
  /* 🔑 v4 부터 «문장»과 «출처»를 한 벌로 넘긴다 — 출처를 빼면 어제가 생것이었는지 알 수 없어
   *   예비로 간다(그게 안전한 방향이다 · tests/예비과제.test.js ⑤). */
  const 오늘 = 오늘과제({
    날짜,
    전날문장: 따라말하기문장(어제.task_snapshot),
    전날출처: 따라말하기출처(어제.task_snapshot),
  });
...
const 결정목록 = () => {
  const 날 = '2026-01-02';
  /* 🔴 v4(08-26) — 「전날」이 `전날출처` 로 갈리고 「예비」 갈래가 생겼다. 그 둘을 안 밟으면
   *   이 지문이 새 갈래를 **원리상 못 지킨다**(바로 아래 ② 검사가 그것을 잰다). */
  const 상황들 = [
    {}, { 첫날: true }, { 첫날: true, 교정문: '고친 문장.' }, { 교정문: '고친 문장.' },
    { 전날문장: '어제 문장.' },                                    // 출처 없음 → 예비/도입
    { 전날문장: '어제 문장.', 전날출처: '생성' },                    // 생것 → 전날
    { 전날문장: '어제 문장.', 전날출처: '교정문' },                  // 생것 → 전날
    { 전날문장: '어제 문장.', 전날출처: '전날' },                    // 어제도 강등 → 예비/도입
    { 전날문장: '어제 문장.', 전날출처: '예비' },                    // 〃
    { 교정문: '고친 문장.', 전날문장: '어제 문장.', 전날출처: '생성' },
    { 첫날: true, 전날문장: '어제 문장.', 전날출처: '생성' },
  ];
  const 급수들 = [null, '', '미정', 'Lv1', 'Lv2', 'Lv3', 'Lv6', '중급', 0, 1, 2, 3, 6];
  /* 예비는 학생마다 다른 자리에서 고르므로 학생 id 가 있어야 밟힌다(고정값 — 지문이 흔들리면 안 된다). */
  const 학생 = '11111111-2222-3333-4444-555555555555';
  const 목록 = [];
  for (const 상황 of 상황들) {
    for (const 급수 of 급수들) {
      const r = 오늘과제({ 날짜: 날, 급수, learner_id: 학생, ...상황 });
      목록.push({
        상황, 급수,

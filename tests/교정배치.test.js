...
/* ── ㉢ 시즌 맥락 배선 (2026-08-20 · 엔진검토 Ⅰ-② · ㉢ 첫 배선) ──────────────────
 * 순수층은 tests/시즌맥락.test.js, 배치 통과는 tests/교정엔진.test.js 가 진다 — 여기는
 * **배선의 실재**만 원문으로 잰다(도달의 정의는 「읽힌 것」 · 조회→조립→두 통로가 이어졌는가). */
test('🔴 ㉢ 시즌 맥락 — 조회가 시즌 답을 싣고, 조립은 한 곳이고, 즉시 통로가 행의 맥락을 낸다', () => {
  assert.match(본체, /as 시즌목표/,
    '조회에 시즌 답 칸이 없다 — ㉢ 첫 배선이 끊겼다(engine.season_compass 를 아무도 안 읽는 상태로 복귀)');
  assert.match(본체, /at time zone \$\{시간대\}/,
    '시즌 판정이 제출 시각 기준이 아니다 — 첨삭은 비동기라 시즌 경계의 밤에 다음 시즌 목표로 어제 문장을 설명한다(시간대는 lib/몽골날짜 정본에서)');
  assert.match(본체, /맥락: 시즌줄\(행\.시즌목표\)/,
    '조립점(한 곳)이 없다 — 즉시·배치가 서로 다른 맥락을 들게 된다');
  assert.match(본체, /급수: 행\.급수, 맥락: 행\.맥락/,
    '즉시 통로가 맥락을 안 싣는다 — 배치로 만든 것과 즉시로 만든 것이 갈린다');
});

...
const 제외사유 = {
  ...
  stt_model: '전사 요청판(`openai:whisper-1:ko`) — **일부러 안 준다**. 검수자는 소리만 듣고 받아쓴다. '
    + '어느 기계가 받아썼는지가 보이면 「이 기계는 원래 잘한다/못한다」가 판정에 스며든다(`due_at` 과 같은 축). '
    + '이 칸의 쓸모는 «나중에 모델을 바꾼 날 옛 행과 새 행을 가르는 것»이라 엔진 쪽 분석에서만 읽는다',
  stt_lang: '벤더가 «들었다고 말한» 언어 — 위와 같은 자리. 「기계가 몽골어로 들었다」가 보이면 '
    + '검수자가 그 판정을 좇아 듣는다(우리가 재려는 것은 학생의 소리지 기계의 짐작이 아니다)',
};

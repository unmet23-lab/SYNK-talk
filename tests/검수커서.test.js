...
test('망가진 반 커서는 400 이 되게 한다 — 기본 커서 꼴도 여기서는 오류다', () => {
  for (const raw of ['어제', '', 'g|2|3|아무때나|9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d',
    'g|2|3|2026-08-14T01:00:00.000Z|짧은id',
    // 기본 커서 꼴(감사|신뢰|시각|id) — 신뢰도 소수점이 반 커서의 정수 칸에 오면 걸려야 한다.
    '1|0.5|2026-08-14T01:00:00.000Z|9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d',
    'g|2|3|2026-08-14T01:00:00.000Z|9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d|덤']) {
    const r = 반커서읽기(raw);
    assert.equal(r.값, null, `${JSON.stringify(raw)} 가 통과했다`);
    assert.match(r.이유, /next_cursor/u);
  }
...
/* 🔴 **두 꼴은 서로소여야 한다**(반박 P1-①). 위 픽스처는 이 못을 못 박았다 — 기본 커서 중
 *   하필 **걸리는 꼴**(`1|0.5|…`)만 골라서, 실제로 **통과하던 꼴**(`1||…` = 신뢰 미측정 ·
 *   현 데이터에 흔하다)을 아무도 안 물었다. 그 한 꼴이 반 모드에서 「조=1·좌석=null」로 읽혀
 *   1조의 좌석 있는 행을 통째로 건너뛰었고, 증상은 「큐가 좀 짧다」뿐이었다.
 *   🔑 그래서 이 검사는 **만들기의 출력을 서로에게 먹인다** — 손으로 적은 꼴을 쓰면 다음 개정에서
 *     꼴이 바뀔 때 이 검사만 낡은 문자열을 지키며 초록으로 남는다. */
test('🔴 두 모드의 커서는 서로를 통과하지 못한다 — 섞이면 200 + 조용히 빠진 목록이다', () => {
  const 반들 = [반행({}), 반행({ group_no: null, seat_no: null }), 반행({ group_no: 1, seat_no: null })]
    .map(반커서만들기);
  const 기본들 = [행({}), 행({ stt_confidence: null }), 행({ is_audit_sample: true, stt_confidence: null })]
    .map(커서만들기);

  // ← 실측으로 샌 방향: 신뢰 미측정(`1||…`)이 반커서읽기를 통과했다.
  assert.ok(기본들.some((s) => /^[01]\|\|/u.test(s)), '「신뢰 미측정」 기본 커서가 픽스처에 없다 — 새는 꼴을 안 물게 된다');

  for (const s of 기본들) {
    const r = 반커서읽기(s);
    assert.equal(r.값, null, `기본 커서 ${s} 가 반커서읽기를 통과했다`);
  }
  for (const s of 반들) {
    const r = 커서읽기(s);
    assert.equal(r.값, null, `반 커서 ${s} 가 커서읽기를 통과했다`);
  }
});

...
/* ── 앱이 선언하는 계약판 ──────────────────────────────────────────
 * 🔑 `src/과제API.js` 는 계약 JSON 을 **import 하지 않는다** — 28KB 짜리 개정 이력이 통째로
 *   번들에 인라인되기 때문이다(앱이 쓰는 것은 두 글자뿐이다). 그 대신 손 상수를 두고
 *   여기서 정본과 묶는다. 갈라진 채 나가면 서버가 426 을 주고, 학생 눈에는 「업데이트하래요」로
 *   보인다 — 원인이 앱 안의 문자열 하나라는 게 그 화면 어디에도 안 적혀 있다. */
test('앱의 계약판 상수가 계약 정본과 같다', () => {
  const src = fs.readFileSync(path.join(ROOT, 'src', '과제API.js'), 'utf8');
  const m = src.match(/export const 계약판 = '([^']+)'/);
  assert.ok(m, 'src/과제API.js 에서 계약판 상수를 못 찾았다 — 이름이 바뀌었으면 이 검사도 같이 고친다');
  assert.equal(m[1], 계약.버전,
    `앱은 ${m[1]} 을 말하는데 계약 정본은 ${계약.버전} 이다 — 올릴 때 같이 올린다`);
});

test('탐지력 — 상수가 어긋나면 위 검사가 잡는다', () => {
  const 가짜 = "export const 계약판 = 'c1';";
  const m = 가짜.match(/export const 계약판 = '([^']+)'/);
  assert.ok(m && m[1] === 'c1' && m[1] !== 계약.버전,

...
test('🔴 맥락이 없으면 v1 과 바이트 동일이다 — 아니면 「맥락 효과」와 「요청이 달라진 것」이 섞인다', () => {
  /* v2 의 맥락 시험은 «같은 문항을 맥락 유/무로 돌려 고친문장·오류태그가 같은가»를 재는데,
   * 맥락이 없을 때의 요청이 v1 과 조금이라도 다르면 그 대조가 무효다(F281: 회차 사이 입력을
   * 얼리지 않으면 합집합은 되지만 대조는 판정을 못 낸다). v1 의 사용자 블록 모양 그대로다. */
  const v1모양 = '학생 급수: Lv2\n학생 문장: 학교에서 갔어요';
  for (const 없음 of [undefined, null, '', '   ', '\n']) {
    assert.equal(요청몸통({ 지시문: 'x', 문장: '학교에서 갔어요', 급수: 'Lv2', 맥락: 없음 }).messages[0].content,
      v1모양, `맥락 ${JSON.stringify(없음)} 에서 v1 과 갈렸다`);
  }

  const 맥락있음 = 요청몸통({
    지시문: 'x', 문장: '학교에서 갔어요', 급수: 'Lv2',
    맥락: '최근 2주 반복 오류: 조사:주격(이/가·은/는) 4회\n목표 트랙: 유학',
  }).messages[0].content;
  assert.match(맥락있음, /^\[학생 맥락\]\n/, '맥락 블록이 라벨 없이 붙었다 — 학생 문장과 안 갈린다');
  assert.ok(맥락있음.endsWith(v1모양),
    '학생 문장이 끝에 안 온다 — 「고칠 것」이 어디인지가 모호해지고, 마지막에 읽은 맥락이 판정을 끈다');

  /* 🔑 맥락은 **user 안에만** 산다 — system 으로 새면 캐시 접두가 학생마다 달라져 캐시가
   *   조용히 죽고(요금으로만 보인다), 맥락이 지시로 읽힐 자리도 생긴다. */
  const 몸 = 요청몸통({ 지시문: '같은지시문', 문장: 'a', 급수: null, 맥락: '반복 오류: 어순 3회' });
  assert.ok(!JSON.stringify(몸.system).includes('어순'), '맥락이 캐시 접두(system) 안으로 들어갔다');
});
...
test('㉢ 배치몸통 — 행의 맥락이 요청몸통까지 실린다(안 실리면 즉시 통로와 배치 통로의 산출이 갈린다 · v7)', () => {
  const 몸 = 배치몸통(
    [{ submission_id: 제출id, 문장: '학교에서 갔어요', 급수: 'Lv2', 맥락: '이번 시즌 목표: 토픽 4급' }],
    '지시문원문', 'v7',
  );
  const 글 = JSON.stringify(몸.requests[0].params);
  assert.ok(글.includes('[학생 맥락]'), '맥락 블록이 배치 요청에 없다 — 배치로 만든 것이 즉시와 갈린다');
  assert.ok(글.includes('이번 시즌 목표: 토픽 4급'), '시즌 줄이 통째로 사라졌다');
  /* 맥락 없는 행은 v1 과 같은 몸통 — 채움말·빈 블록이 생기면 「맥락 없음=바이트 동일」 불변식이 죽는다. */
  const 없이 = JSON.stringify(배치몸통(
    [{ submission_id: 제출id, 문장: '학교에서 갔어요', 급수: 'Lv2' }], '지시문원문', 'v7',
  ).requests[0].params);
  assert.ok(!없이.includes('[학생 맥락]'), '맥락 없는 행에 빈 블록이 생겼다');
});

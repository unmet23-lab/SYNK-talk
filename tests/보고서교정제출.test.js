...
const G2항목 = (문항id, 덧 = {}, 스냅샷덧 = {}) => ({
  task_ref: `task-2026-08-12-${문항id}`,
  task_snapshot: {
    challenge_id: 'g2-보고서교정',
    /* 🔑 문항을 되짚는 열쇠는 **스냅샷 안**이다(G1 과 같은 칸) — 3단계 실측으로 되돌린 자리다.
     *   봉투 «밖» 칸으로 두면 통로 어디에도 그 칸이 없어 `게임재료()` 가 영원히 null 이다. */
    prompt_seed: 문항id,
...
const 재료of = (문항id, 항목덧, 스냅샷덧) => 게임재료(G2항목(문항id, 항목덧, 스냅샷덧));
...
test('확정팩이면 재료가 서고, 문항을 되짚는 열쇠는 스냅샷 «안»에서 온다', () => {
  const 재료 = 재료of(오류id);
  ...
  /* 🔴 열쇠는 `task_snapshot.prompt_seed` 다 — G1 과 **같은 칸**이다(3단계 실측). 봉투 «밖»
   *   (`항목.prompt_seed`)에서 찾으려 들면 통로 어디도 그 칸을 안 실어 영원히 null 이고,
   *   증상은 오류가 아니라 조용한 말하기 폴백이라 어디에도 안 남는다. */
  assert.equal(재료of(오류id, {}, { prompt_seed: '' }), null, '열쇠가 없으면 재료를 만들지 않는다');
  assert.equal(재료of(오류id, {}, { prompt_seed: '없는문항' }), null, '못 펴는 문항 — 지어내지 않는다');
  assert.equal(재료of(오류id, { prompt_seed: '없는문항' }).문항id, 오류id,
    '🔴 봉투 밖 칸은 «무시»한다 — 두 곳을 다 보면 갈라진 쪽이 조용히 이긴다(판정 한 곳)');
});

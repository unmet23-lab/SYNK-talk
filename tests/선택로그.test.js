...
const { 보기세우기, 선택payload, 걸린시간 } = require('../lib/선택로그.js');
...
test('position 은 **고른 것의 자리**다 — 손버릇과 선호를 가르는 유일한 칸', () => {
  ...
  const p = 선택payload({ 보기, 고른것: 'opt_kimchi' });
...
test('latency_ms — 시계가 뒤로 가면 「즉답」이 아니라 「안 쟀다」다', () => {
  ...
  const 보기 = 보기세우기(선택지(), null, 항등);
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi', 시작: 0, 끝: 1500 }).latency_ms, 1500);
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi' }).latency_ms, null, '안 쟀으면 null(0 이 아니다)');
});

test('안 물어본 칸은 null 로 싣는다 — 빈 문자열은 「모름」이 아니다', () => {
  const 보기 = 보기세우기(선택지(), null, 항등);
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi' }).selection_reason, null);
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi', 사유: '' }).selection_reason, null);
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi', 사유: '맛있어서' }).selection_reason, '맛있어서');
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi', 바꾼횟수: 0 }).changed_selection, false);
  assert.equal(선택payload({ 보기, 고른것: 'opt_kimchi', 바꾼횟수: 2 }).changed_selection, true);
});
...
test('🔑 조립한 payload 가 **실계약 검증을 실제로 통과한다**(고름 갈래)', () => {
  ...
  const r = 검증(사건(선택payload({ 보기, 고른것: 'opt_bulgogi', 시작: 0, 끝: 900 })), 계약);
  assert.deepEqual(r.오류들 ?? r.errors ?? [], [], JSON.stringify(r));
});

/* 🔴 **나머지 두 갈래는 여기서 못 건다 — 계약이 막고 있기 때문이다.**
 *   `skipped`·`rejected_all` 은 계약이 택1로 허용해 놓고, 같은 계약이 `payload.position` 을
 *   무조건 필수로 걸어(널허용에도 없다) 안 고른 행을 원리상 거부한다. 즉 **「학생이 안 골랐다」는
 *   관측은 지금 통로가 없다** — 그런데 그건 성향 축에서 「무관심 vs 뚜렷한 거절」을 가르는 신호다.
 *   🚫 여기에 「지금은 거부된다」를 assert 로 못박지 않는다 — 그건 **버그가 아직 있을 것을 요구하는
 *      회귀**가 되어(CLAUDE.md 맹점 ②) 계약이 고쳐지는 날 애먼 데가 빨개진다.
 *   ▶ 처방은 계약 판정 자리다(조건부 필수 = `selected_option` 이 있을 때만 `position` 필수).
 *      기록: 대기열 P1 · 이 파일 머리말 · `lib/선택로그.js position` 주석. */

test('③ 자기 처방 — 검증이 거부한 것을 사유대로 고치면 통과한다', () => {
  const 보기 = 보기세우기(선택지(), 'opt_kimchi', 항등);
  const 온전 = 선택payload({ 보기, 고른것: 'opt_kimchi' });

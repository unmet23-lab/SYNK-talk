...
/** ⑦ 이 쓰는 임시 방 — 호스트 스크립트를 «파일로» 세워야 argv 가 안 밀린다(⑦ 등록층 ⚠). */
const 임시방 = () => fs.mkdtempSync(path.join(require('node:os').tmpdir(), 'q111-'));
...
test('깨진 줄·칸 빠진 줄은 세되 기준이 되지 못한다', () => {
  ...
  const { 줄들, 깨진, 메모 } = 빚.장부파싱(text);
  assert.equal(깨진, 4);
  assert.equal(메모, 0, '메모가 아닌 것을 메모로 세면 「모름으로 샌다」 경고가 조용해진다');
  assert.deepEqual(줄들.map((o) => o.함수), ['tasks']);
});

/* 🔴 이 검사의 급소는 «메모로 세는 것»이 아니라 **메모로 세지 «않는» 쪽**이다 —
 *   판정이 넓으면 「칸 빠진 진짜 기록」이 메모로 숨고, 그 함수는 «모름» 인 채 조용해진다. */
test('일부러 남긴 메모 줄은 «깨짐» 이 아니다 — 단 판정은 좁다', () => {
  const text = [
    줄({ 판: '운영', 함수: 'tasks', head: 'aaa' }),        // 성한 기록
    '{"정정": "앞 줄의 122/122 는 실측 전에 적은 값이었다"}', // 메모 — 실물(장부 332행)과 같은 꼴
    '{"정정": "   "}',                                     // 빈 정정 = 메모 아님(깨짐)
    '{"메모": "정정이라는 칸이 없다"}',                     // 다른 이름 = 메모 아님(깨짐)
    줄({ 판: '운영', 함수: 'deliver', 정정: '기록인데 head 가 없다' }), // 🔴 기록 칸이 있으면 메모로 숨지 못한다
  ].join('\n');
  const { 줄들, 깨진, 메모 } = 빚.장부파싱(text);
  assert.equal(메모, 1);
  assert.equal(깨진, 3, '나머지 셋은 그대로 깨짐으로 남는다');
  assert.deepEqual(줄들.map((o) => o.함수), ['tasks']);

  // 판정 함수 자체도 좁은지 직접 잰다(위 셈이 우연히 맞는 것과 구별한다)
  assert.equal(빚.메모줄인가({ 정정: '한 줄' }), true);
  assert.equal(빚.메모줄인가({ 정정: '한 줄', 함수: 'deliver' }), false, '기록 칸이 하나라도 있으면 메모가 아니다');
  assert.equal(빚.메모줄인가({ 정정: '한 줄', 판: '운영' }), false);
  assert.equal(빚.메모줄인가({ 정정: '한 줄', head: 'aaa' }), false);
  assert.equal(빚.메모줄인가(null), false);
});
...
// ── ⑦ 인자 게이트 — 모르는 낱말을 조용히 삼키던 자리 (대기열 #Q111 · 2026-08-18) ──
/* 발단(2026-08-17 실측): `node tools/배포빚.js --운영` 이 인자 없을 때와 **출력이 한 글자도
 *   다르지 않고 exit 0** 이었다. 그래서 as `작업본소유자.js` 의 갚기 안내가 「운영은 `--운영`」
 *   이라는 거짓을 몇 달 달고 있어도 아무도 못 봤다 — 쳐 보면 「돌긴 도네」였기 때문이다.
 *   F400·F435·F436 과 같은 계열이고, 새는 방향은 언제나 「통과」다.
 *
 * 🔴 왜 형제 `tests/플래그게이트.test.js` 가 이걸 못 잡았나 — 그 검사의 분모가
 *   `자격증명.읽기(` 를 쓰는 도구뿐이다. 이 도구는 **네트워크 0 · 자격증명 0** 이라 그 스캔에
 *   원리상 안 들어온다. 착수 실측: talk `tools/` 에서 argv 를 읽는 도구 30개 중 그 분모 «밖»이
 *   17개, 그중 게이트가 없는 것이 **16개**다. 그 분모를 넓히는 것은 별도 트랙(#Q112)이고,
 *   여기 ⑦은 **이 도구 한 자리**를 못박는다. */

test('⑦ 아는 낱말은 딱 둘이다 — `--운영` 을 «일부러» 안 넣었다 (넣으면 그게 F592 다)', () => {
  /* 🔴 이 검사가 지키는 급소. `공용플래그` 머리말이 「도구는 자기 낱말에 그것을 펴 넣는다」라
   *   말하기 때문에, 다음 사람은 선의로 `[...공용플래그, …]` 로 «고치려» 든다. 그러면 `--운영`
   *   이 다시 통과하고 — 이 도구엔 조준 축이 없으니 — 받은 척하고 아무것도 안 갈아탄다.
   *   그게 정확히 #Q110·F592 가 몇 달 살아남은 모양이다. 문구가 아니라 기계로 막는다. */
  assert.deepEqual(빚.아는플래그, ['--판', '--json']);
  assert.ok(!빚.아는플래그.includes('--운영'),
    '`--운영` 이 아는 낱말로 돌아왔다 — 조준 축이 없는 도구가 조준 낱말을 받으면 그게 F592 다');
});

test('⑦ 조준힌트 — 거절문이 «갈 곳»을 준다 (갈 데 없는 차단은 우회를 정상 통로로 만든다 · F103)', () => {
  for (const 판 of 빚.판들) {
    assert.match(빚.조준힌트([`--${판}`]), new RegExp(`--판 ${판}`),
      `--${판} 을 거절하면서 --판 ${판} 을 안 알려 줬다 — 따를 수 없는 처방이다`);
  }
  assert.equal(빚.조준힌트(['--jsonn']), '', '조준과 무관한 오타에까지 판 힌트가 붙으면 문구가 헛돈다');
  assert.equal(빚.조준힌트([]), '');
});

test('⑦ 자기 처방 — 힌트가 시킨 명령이 실제로 이 게이트를 통과한다 (F103)', () => {
  /* 가드 맹점 ③: 차단 사유가 시키는 명령을 그 가드에 되먹여 통과하는지 본다. */
  const { 모름 } = require('../lib/플래그.js');
  const 힌트 = 빚.조준힌트(['--운영']);
  const 인용된낱말 = (힌트.match(/--[가-힣A-Za-z0-9_-]+/g) || []);
  assert.ok(인용된낱말.length, '힌트가 낱말을 하나도 안 인용했다 — 검사가 장식이 된다');
  for (const f of 인용된낱말) {
    assert.deepEqual(모름([f], 빚.아는플래그), [],
      `힌트가 시킨 ${f} 를 그 게이트가 막는다 — 따를 수 없는 처방이다`);
  }
});

test('⑦ CLI — `--운영` 은 exit 1 이고 stdout 에 보고서가 안 나간다 (그게 F592 의 모양이었다)', () => {
  const r = 띄우기([path.join(ROOT, 'tools', '배포빚.js'), '--운영'],
    { cwd: ROOT, encoding: 'utf8', 통과코드: [1] });
  assert.doesNotMatch(String(r.stdout), /전체 \d+ = 낡음/,
    '거절해 놓고 보고서를 냈다 — 사람은 자기가 부탁한 일이 된 줄 안다');
  assert.match(String(r.stderr), /모르는 플래그 --운영/);
  assert.match(String(r.stderr), /--판 운영/, '갈 곳을 안 알려 주면 차단이 곧 막다른 길이다');
});

test('⑦ CLI — 값 없는 `--판` 은 두 판을 내며 초록으로 끝나지 않는다', () => {
  /* 옛 판은 `args[i+1]` 이 undefined → 고른판 falsy → **두 판 전부 · exit 0** 이었다.
   *   한 판만 봤다고 믿는 사람에게 두 판의 초록이 나가는 자리라, 같은 계열의 결함이다. */
  for (const 인자들 of [['--판'], ['--판', '--json']]) {
    const r = 띄우기([path.join(ROOT, 'tools', '배포빚.js'), ...인자들],
      { cwd: ROOT, encoding: 'utf8', 통과코드: [1] });
    assert.doesNotMatch(String(r.stdout), /전체 \d+ = 낡음/, `${인자들.join(' ')} 가 보고서를 냈다`);
    assert.match(String(r.stderr), /--판 에 값이 없다/);
  }
});

test('⑦ 거짓양성 — 실제로 쓰이는 인자 조합은 전부 그대로 산다 (호출부 전수 · #Q111 착수 감사)', () => {
  /* 이 장치가 틀릴 때의 모습은 **거짓양성**이다(선언에서 낱말 하나를 빠뜨리면 되던 명령이 죽는다).
   *   그래서 「지금 실제로 쓰이는 것」을 여기 못박는다 — 목록의 근거는 착수 때의 호출부 전수 감사다:
   *     · as `tools/작업본소유자.js 형제배포빚()` → `--json`   (세션 시작 화면)
   *     · `.claude/skills/close/SKILL.md` 3-b · `배포대조.js 집계문` · 갚기 안내 → 인자 없음
   *     · 이 파일 ⑥ → `--json` · `--json --판 운영` */
  for (const 인자들 of [[], ['--json'], ['--json', '--판', '운영'], ['--판', '리허설'], ['--판', '운영']]) {
    const r = 띄우기([path.join(ROOT, 'tools', '배포빚.js'), ...인자들],
      { cwd: ROOT, encoding: 'utf8', stdio: ['ignore', 'pipe', 'ignore'] });
    assert.ok(String(r.stdout).trim(), `\`${인자들.join(' ')}\` 가 빈손으로 끝났다 — 게이트가 멀쩡한 통로를 죽였다`);
  }
});

test('🔴 ⑦ 등록층 — 게이트는 `main()` 안에서만 돈다 (모듈층에 두면 알림이 통째로 조용해진다)', () => {
  /* 🔴 **이 회귀가 지키는 급소.** 이 파일을 `require` 하는 쪽이 셋인데(`동봉신호`·`배포대조`·
   *   `원격배포`), `동봉신호.js 빚요약()` 은 그 require 를 **try/catch 로 감싸 실패를 `[]` 로
   *   접는다**. 게이트를 모듈 최상단에 두면 호스트의 argv(`원격배포 … --적용 --운영`)를 읽고
   *   죽고, post-commit 배포 알림이 **소리 없이 사라진다** — 게이트가 막으려던 바로 그 모양이다.
   *   ⚠ 소스 모양이 아니라 **행동**으로 잰다: 적대적 argv 를 달고 require 해 본다.
   *
   *   ⚠ **`node -e` 로 재지 않는다** — 착수 때 그것으로 쟀다가 아래 픽스처에 걸렸다: `-e` 는
   *   남은 첫 인자를 `argv[1]` 로 놓아 `slice(2)` 가 한 칸씩 밀린다(`--적용 --운영` → `["--운영"]`).
   *   재는 층이 값을 깨뜨리면 「멀쩡하다」와 「못 쟀다」가 같은 얼굴로 나온다(CLAUDE.md 원인 판정 ②).
   *   그래서 **진짜 호스트 스크립트 파일**을 세워 `원격배포.js` 가 불리는 모양 그대로 만든다. */
  const 방 = 임시방();
  const 호스트 = path.join(방, '호스트.js');
  fs.writeFileSync(호스트,
    `require(${JSON.stringify(path.join(ROOT, 'tools', '배포빚.js'))});\n`
    + "process.stdout.write('살아있음:' + JSON.stringify(process.argv.slice(2)));\n", 'utf8');
  for (const 인자들 of [['--적용', '--운영'], ['--도장', '--운영'], ['--진단'], ['HEAD']]) {
    const r = 띄우기([호스트, ...인자들], { cwd: ROOT, encoding: 'utf8' });
    assert.equal(String(r.stdout).trim(), `살아있음:${JSON.stringify(인자들)}`,
      `호스트 argv \`${인자들.join(' ')}\` 에서 모듈 로드가 죽었다 — `
      + '`동봉신호.빚요약()` 의 try/catch 가 이것을 `[]` 로 삼켜 배포 알림이 조용히 사라진다');
  }
  fs.rmSync(방, { recursive: true, force: true });
});

test('⑦ 등록층 픽스처 — 모듈층 게이트를 실제로 잡아낸다 (탐지력이 죽으면 위 검사는 장식이다)', () => {
  /* 위 검사는 「지금 안 죽는다」만 본다 — 판정기가 죽어도 초록이라, 탐지력은 여기가 진다.
   *   모듈 최상단에서 argv 를 보고 죽는 **가짜 모듈**을 세워, 그 모양이 정말로 잡히는지 잰다.
   *   🔑 이 픽스처가 실제로 일했다: 앞판의 `-e` 통로를 여기서 잡았다(위 ⚠). */
  const 방 = 임시방();
  const 가짜 = path.join(방, '가짜.js');
  fs.writeFileSync(가짜,
    "if (process.argv.slice(2).some((a) => a.startsWith('--'))) { process.exit(1); }\n"
    + 'module.exports = { 요약문: () => 1 };\n', 'utf8');
  const 호스트 = path.join(방, '호스트.js');
  fs.writeFileSync(호스트,
    `require(${JSON.stringify(가짜)});\nprocess.stdout.write('살아있음');\n`, 'utf8');

  assert.throws(() => 띄우기([호스트, '--적용'], { cwd: ROOT, encoding: 'utf8' }),
    '모듈층에서 죽는 판을 못 잡았다 — 그러면 위 검사는 무엇이든 통과시킨다');
  /* 반대쪽도 본다: 적대적 낱말이 없으면 그 가짜도 산다(늘 빨간 검사는 탐지력이 아니다). */
  assert.equal(String(띄우기([호스트, 'HEAD'], { cwd: ROOT, encoding: 'utf8' }).stdout).trim(), '살아있음');
  fs.rmSync(방, { recursive: true, force: true });
});

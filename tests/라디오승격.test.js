...
/* ── 동봉 격리 — 승격기가 몽골날짜 하나 때문에 오늘과제를 물면 게임 트리 전체가 동봉에 실려
 * 게임 트랙 커밋마다 radio-promote 배포가 밀린다(반박 ⑤ · 3회 실측). 뿌리 둘을 못박는다 —
 * require 를 되살리면 동봉 닫힘 검사가, 동봉을 되살리면 이 회귀가 빨개진다. */
test('radio-promote 동봉이 오늘과제·게임스냅샷 트리를 싣지 않는다 (반박 ⑤)', () => {
  const 동봉 = JSON.parse(fs.readFileSync(
    path.join(__dirname, '..', 'supabase', 'functions', 'radio-promote', '동봉.json'), 'utf8'));
  for (const 뿌리 of ['오늘과제.mjs', '게임스냅샷.mjs']) {
    assert.ok(!(뿌리 in 동봉),
      `radio-promote 동봉에 ${뿌리} 가 돌아왔다 — 날짜 접기는 lib/몽골날짜.js 에서 문다(반박 ⑤)`);
  }
  assert.equal(동봉['몽골날짜.mjs'], 'lib/몽골날짜.js', '몽골날짜 동봉이 빠졌다 — 승격기가 첫 import 에서 죽는다');
});

/* ── 인자 정책 단일 정본 — 「필수 인자 없으면 접수 안 함」의 정본은 명령표 하나다(반박 ②).
 * 승격기에 널 검사를 따로 적으면 정책과 코드가 주석으로만 묶여, 정책을 바꿔도 승격기가
 * 안 따라간다. 변이(정책을 잠시 «선택»으로)로 실시간 읽기를 잰다. */
test('인자없음 제외는 명령표의 인자 정책을 실시간으로 읽는다 (변이 · 반박 ②)', () => {
  const 원래 = 명령표['목표'].인자;
  try {
    명령표['목표'].인자 = '선택';
    const r = 승격계획({ ...기본재료(), 메시지들: [채팅('m1', '목표', null, '2026-08-12T12:01:00Z')] });
    assert.ok(!사유들(r).includes('인자없음'),
      '정책을 «선택»으로 바꿨는데 승격기가 여전히 «필수»로 판정한다 — 널 검사 사본이 생겼다');
  } finally {
    명령표['목표'].인자 = 원래;
  }

...
const { 코드만 } = require('./lib/소스검사.js'); // [2026-08-13 · F401] 사본 → 공용 통로
const 화면코드 = () =>
  코드만(fs.readFileSync(path.join(__dirname, '..', 'src', '답장화면.js'), 'utf8'));
...
test('🔴 말하기화면 공용 배출구(밀린것보내기)가 교정 큐도 민다 — 답장 화면만 밀면 상시 배출구가 없다 (D7-2)', () => {
  /* 게임큐 B2 무늬 그대로 — 몸을 첫 「};」까지로 자르므로 이 블록 안에 그 시퀀스를 만들지 않는다. */
  ...
  assert.match(몸, /교정로그읽기\(\)/, '배출구가 교정 큐를 안 읽는다 — 밀린 교정 사건이 답장 화면을 다시 여는 날까지 안 나간다');
  assert.match(몸, /교정사건보내기\(토큰,\s*항목\.사건\)/,
    '교정 전송이 항목의 «그 사건 객체»를 그대로 안 보낸다 — 멱등키가 갈려 서버가 중복을 못 접는다');
  assert.match(몸, /보낼것\(교정기록,\s*막힘참조\.current\)/,
    '교정 큐가 막힘 실값을 안 본다 — 막힌 학생의 답이 send_final 로 접혀 영영 죽는다');
  assert.match(몸, /교정로그쓰기\(/, '전송 결과가 기기에 안 남는다 — 다음 마운트가 같은 사건을 또 보낸다');
});

...
test('지난날 긍정/부정 비교 — 오늘 답한 키와 축 순서가 부정 효력을 대신하지 않는다', () => {
  const 카드들 = 관통검증.비교답카드들(원행들(), { 기준, 시간대 });
  const 첫 = 카드들[0], 어제 = '2026-09-10';
  assert.equal(첫.shown_key, '여유제출');
  assert.ok(카드들.length >= 2, '축 순서가 부정을 가리는 반례에 다른 후보 축이 필요하다');
  const A = 관통검증.지난응답이력(카드들, 어제, { 조회일: 오늘, 부정쌍: 첫 });
  const B = 관통검증.지난응답이력(카드들, 어제, { 조회일: 오늘 });
  assert.equal(A.오늘답수, 0);
  assert.deepEqual(A.오늘답한키들, []);
  assert.deepEqual(A.축별마지막날, B.축별마지막날);
  const 파생 = (이력) => 관통검증.확인카드파생(원행들(), { 기준, 시간대, 이력 }).카드;
  assert.ok(파생(A) && 파생(B));
  assert.equal(관통검증.같은쌍(파생(B), 첫), true, '긍정 대조군에 목표 키가 안 돌아오면 부정 효과를 분리할 수 없다');
  assert.equal(관통검증.같은쌍(파생(A), 첫), false);
  assert.equal(관통검증.같은쌍(파생({ ...A, 부정키들: [] }), 첫), true,
    '부정 이력 제거 변이가 판정을 바꿔야 한다 — 같은 날 억제가 가리는 옛 시험으로 돌아갔다');
  assert.throws(() => 관통검증.지난응답이력(카드들, 오늘, { 조회일: 오늘 }), /앞서야/);
});

test('사슬 — 부정한 여유제출 지표만 빠지고 같은 축의 제출률·표본과 axes_used 는 보존된다', () => {
  const 행 = 원행들();
  ...
  const 키목록 = 부정키들([사건행]);                            // 도구의 SQL 심기 뒤 deliver 가 별도 부정이력에서 뽑는 그 규칙
  assert.deepEqual(키목록, [부정키(축, 카드.shown_key)], '부정 키가 정확히 하나여야 아래가 실측이다');
  const 뒤 = 과제요약(상태, { 목표: 'study', 부정키들: 키목록 });
  assert.equal(카드.shown_key, '여유제출', '어느 짐작을 부정했는지 고정한 비교 재료다');
  assert.notEqual(관통검증.지표값(앞.요약, 축, '마감여유분_중앙'), null, '분모: 부정할 지표가 실제 입력에 실렸어야 한다');
  assert.equal(관통검증.지표값(뒤.요약, 축, '마감여유분_중앙'), null);
  for (const 이름 of ['제출률', '지각', 'n']) {
    const 이전 = 관통검증.지표값(앞.요약, 축, 이름);
    assert.notEqual(이전, null, `${이름} 보존을 재기 위한 분모가 없다`);
    assert.equal(관통검증.지표값(뒤.요약, 축, 이름), 이전);
  }
  assert.deepEqual(관통검증.축차이(앞.axes_used, 뒤.axes_used), { 빠진: [], 늘어난: [] });
  assert.deepEqual(뒤.제외지표, ['리듬.마감여유분_중앙']);
  assert.deepEqual(뒤.제외키, [부정키(축, 카드.shown_key)]);
...
test('N3 서버 응답 — 확인 카드와 목표 카드는 서로 대체하지 않고 화면 증거는 미실행이다', () => {
  const { 목표카드 } = require('../lib/목표확인.js');
  const { 카드 } = 관통검증.확인카드파생(원행들(), { 기준, 시간대 });
  const body = { ok: true, 오늘의확인: 카드, 오늘의목표: 목표카드({ 오늘답함: false }, 오늘) };
  const 정상 = 관통검증.progress카드검사(body, 오늘);
  assert.equal(정상.확인통과 && 정상.목표통과, true);
  assert.equal(정상.화면검증, '미실행');
  const 확인없음 = 관통검증.progress카드검사({ ...body, 오늘의확인: null }, 오늘);
  assert.equal(확인없음.확인통과, false, '목표 카드로 확인 카드 실패를 가렸다');
  assert.equal(확인없음.목표통과, true);
  assert.equal(관통검증.progress카드검사({ ...body, 오늘의목표: null }, 오늘).목표통과, false);
  assert.equal(관통검증.progress카드검사({ ...body, 오늘의목표: null }, '2026-09-12').목표통과, true, '주말은 목표 카드 null이 정상이다');
  assert.equal(관통검증.progress카드검사({ ok: true, 오늘의확인: 카드 }, '2026-09-12').목표통과, false, '응답 필드 누락은 주말 null과 다르다');
});

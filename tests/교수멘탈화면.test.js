/* 교수멘탈화면 회귀 — G1 화면을 **실제로 그려 본다**(`tests/lib/화면세우기.js` · F268 계열).
 *
 * 재는 것: ① 세 단계가 그리다 죽지 않고 정본 값을 글자로 낸다 ② 숨은 시계(시간 표시 0 ·
 * 숫자/퍼센트 0) ③ 요구문형(모범 문형) 비노출 — 힌트는 빠진 칸 **이름**까지 ④ 즉답 금지
 * ⑤ controlled TextInput 금지(소스층 — 핸들러는 첫 렌더가 못 돈다 · ⑩ 선례) ⑥ 학생 접점
 * 금칙어 0 ⑦ 말하기화면 라우팅 갈래의 실재(소스층 — 갈래는 effect 뒤라 첫 렌더 밖이다).
 * 탐지력(깨진 화면을 정말 잡는가)은 `화면렌더.test.js` ⑤ 픽스처가 이미 못박았다 — 같은 통로다.
 */
'use strict';
const { test } = require('node:test');
const assert = require('node:assert/strict');
const fs = require('node:fs');
const path = require('node:path');
const { 그리기, ROOT } = require('./lib/화면세우기.js');
const { 코드만, 코드만픽스처 } = require('./lib/소스검사.js');

const 소스 = fs.readFileSync(path.join(ROOT, 'src', '교수멘탈화면.js'), 'utf8');

/* 픽스처는 손으로 짓지 않고 정본(팩)에 접게 한다 — 화면이 읽는 모양을 여기 또 적으면 두 벌이다.
 * 재료의 **키 모양**은 `게임제출.test.js` ⓪(「모양이 화면 픽스처와 한 벌」)이 lib 쪽에서 못박는다 —
 * 검수확정 게이트(H4) 때문에 실팩으로는 `게임재료` 가 null 이라, 화면은 확정된 날의 재료 모양을
 * 여기서 그대로 짓는다(키가 갈리면 저 테스트가 빨개진다). */
const { 시드만들기, 펴기 } = require(path.join(ROOT, 'contents', '교수멘탈문항.js'));
const 시드 = 시드만들기('g1t01', 0, 0);
const 문항 = 펴기(시드);
const 재료픽스처 = {
  prompt_seed: 시드,
  문항,
  task_ref: 'task-2026-08-11',
  level_snapshot: 'Lv2',
  goal_snapshot: null,
  retry_of_event_id: null,
};
const 그때 = (시작단계) => 그리기('src/교수멘탈화면.js', { 재료: 재료픽스처, 토큰: 'x', 학생번호: 'SYNK-042', 시작단계 });

/* synk-brand 바닥 검사 — 활용형까지 어간으로(교수멘탈문항.test.js 와 같은 자) */
const 금칙꼴 = [/패배/, /졌다/, /실패/, /불운/, /하락/, /부족/, /안\s?됨/, /늦/];

test('① 전략 단계 — 상황·지시문·전략 3장·오늘의 추천이 화면에 선다', () => {
  const 글 = 그때('전략');
  assert.ok(글.includes(문항.질문), '상황문이 화면에 없다 — 팩 정본이 그리는 재료다');
  assert.ok(글.includes(문항.지시문), '지시문이 없다 — 핵심 어휘 3개가 이 안에 있다(§6-8 규칙 3)');
  for (const 문구 of ['무슨 일이 있었는지 설명해요', '짧게 사과하고 부탁해요', '제가 할 수 있는 일을 말씀드려요']) {
    assert.ok(글.includes(문구), `전략 카드가 빠졌다: ${문구}`);
  }
  assert.match(글, /어떻게 말씀드릴까요/);
  assert.doesNotMatch(글, /솔직한 공책|마음을 담은 봉투|다음 계획 달력/);
  assert.match(글, /오늘의 추천/, '추천 표시가 없다 — 「밀어준 것」과 「선호」를 가르는 재료다');
});

test('② 쓰기 단계 — 게이지 5칸 이름·「문법 점수가 아니라」·받는 사람 고정', () => {
  const 글 = 그때('쓰기');
  assert.match(글, /받는 사람\s+교수님/);
  assert.match(글, /문법 점수가 아니라, 빠진 부분을 알려 주는 안내/, '게이지가 문법 점수가 아닌 빠진 부분 안내라는 뜻이 화면에 없다');
  for (const 이름 of Object.keys(문항.요구문형)) {
    assert.ok(글.includes(이름), `게이지 칸 이름이 없다: ${이름} — 힌트는 빠진 칸 이름까지다`);
  }
  assert.match(글, /아직 비어 있는 칸/, '빠진 칸 안내가 없다');
  assert.match(글, /보내기/, '주행동(신호 1점)이 없다');
});

test('②-b 선택 예문은 쓰기 중 다시 열 수 있고 처음에는 입력 공간을 차지하지 않는다', () => {
  const { 장면만들기 } = require('../contents/교수멘탈장면.js');
  for (const 전략 of 장면만들기({ ...재료픽스처, 캐릭터: '마린' }).전략) {
    const 글 = 그리기('src/교수멘탈화면.js', { 예문: 전략.예문 }, '쓰기말투예시');
    assert.match(글, /말투 예시 다시 보기/);
    assert.ok(!글.includes(전략.예문), '방금 읽은 예문은 처음에 접어 두고 필요할 때 다시 펼친다');
  }
  assert.equal(그리기('src/교수멘탈화면.js', { 예문: null }, '쓰기말투예시'), '',
    '선택하지 않은 예문을 임의로 만들어 보여 주면 안 된다');
});

test('③ 숨은 시계 — 시간·숫자·퍼센트가 화면에 0 이다(유호 확정 · 발주 G1 §6)', () => {
  for (const 단계 of ['전략', '쓰기', '대기']) {
    const 글 = 그때(단계);
    assert.doesNotMatch(글, /\d+\s*초|\d+:\d\d|타이머/, `${단계}: 시계가 걸렸다 — 재려던 구조가 안 재진다`);
    assert.doesNotMatch(글, /%|퍼센트/, `${단계}: 게이지에 숫자·퍼센트 병기 금지(게임층 §4 규격 3)`);
  }
});

test('④ 요구문형(모범 문형)은 화면 비노출이다 — 베껴 채우면 재려던 산출이 안 재진다(팩 §2)', () => {
  const 쓰기 = 그때('쓰기');
  for (const 문형 of Object.values(문항.요구문형)) {
    assert.ok(!쓰기.includes(문형), `모범 문형이 노출됐다: ${문형}`);
  }
  // placeholder·힌트에 문형 조각이 스미는 흔한 자리까지 — 대표 조각 둘로 못박는다
  assert.doesNotMatch(쓰기, /주시면 감사하겠습니다|안녕하십니까 \//);
});

test('⑤ 대기 단계 — 답장을 기다리며 즉답·읽음·사람 검수 완료를 약속하지 않는다', () => {
  const 글 = 그때('대기');
  assert.match(글, /답장을 기다리고 있어요/);
  assert.match(글, /며칠/, '「며칠 걸린다」는 기대 관리가 없다 — 즉답 기대가 첫 주에 깨진다');
  assert.doesNotMatch(글, /답장이 왔|도착했어요/, '보내자마자 답장·도착을 말하면 즉답 연출이다');
  assert.doesNotMatch(글, /교수님이 읽고|선생님이 확인한 뒤|선생님 확인/,
    '답장 목록은 AI 교정도 반환하며 읽음 증거가 없다 — 사람 검수나 읽음을 앞질러 말하면 안 된다');
});

test('⑥ controlled TextInput 금지 — defaultValue 만 쓴다(발주 G1 §4-5 ⚙ · 한글 조합 보호)', () => {
  /* 이 파일의 다른 소스층 검사(:94·:104·:130)는 전부 `코드만` 을 지나는데 여기만 원문이었다.
     `value={` 는 특히 위험한 표기다 — 금지를 설명하는 JSX 주석(`{/* value={…} 금지 *​/}`)이
     그대로 위반으로 잡혀 거짓 적색이 난다(공용 통로 머리말의 `반피드백화면` 실측과 같은 모양). */
  const 코드 = 코드만(소스);
  assert.ok(/defaultValue/.test(코드), 'TextInput 이 defaultValue 를 안 쓴다');
  assert.ok(!/value=\{/.test(코드),
    'TextInput 에 value= 가 있다 — 매 글자 되돌려 넣으면 「안녕」이 「ㅇㅏㄴㄴㅕㅇ」으로 흩어진다');
  /* 게이지 갱신 조건 — 같은판정이 false 일 때만 setState(그 줄이 사라지면 매 글자 리렌더다).
   * 소스층인 이유: 핸들러는 첫 렌더가 못 돈다(⑩ 선례 — 못 재는 층을 비워 두지 않는다). */
  assert.match(코드, /같은판정\(/, '게이지 갱신이 같은판정을 안 지난다 — 매 글자 리렌더 = controlled 와 같은 파괴다');
});

test('⑥-b 입력 칸은 본문 하나다 — 제목 합성이 계측·게이지·body_original 을 오염시켰다(H1)', () => {
  const 코드 = 코드만(소스);
  assert.equal([...코드.matchAll(/<TextInput/g)].length, 1,
    '입력 칸이 하나가 아니다 — 제목 칸을 붙이려면 먼저 제목의 행 자리(계약 판정)가 서야 한다');
  /* 결함 모양 그대로 잡는다 — `제목: ${…}` 합성 템플릿(스타일 키 `제목:` 은 무관하다). */
  assert.ok(!/제목: \$\{/.test(코드),
    '합성 접두(`제목: ${…}`)가 남았다 — 모든 학생의 input_burst_max 가 접두 길이만큼 공짜로 오르고, 그 값은 소급이 없다');
  assert.ok(!/제목참조/.test(코드), '제목 ref 가 남았다 — 계측·게이지가 본문 밖 글자를 잰다');
});

test('⑥-c 죽은 막힘 prop 이 없다 — 막힘 검사 뒤에서만 사는 화면이 실값처럼 들면 오독이다(M7)', () => {
  const 코드 = 코드만(소스);
  assert.ok(!/막힘/.test(코드),
    '화면이 막힘을 받는다 — 이 화면은 말하기의 막힘 검사 뒤에서만 살아 그 값이 늘 null 이고, 실값 보류는 말하기 밀기가 진다');
});

test('⑦ 학생 접점 금칙어 0 — 세 단계 렌더 전량(synk-brand 바닥 검사)', () => {
  for (const 단계 of ['전략', '쓰기', '대기']) {
    const 글 = 그때(단계);
    for (const 꼴 of 금칙꼴) {
      assert.ok(!꼴.test(글), `${단계}: 금칙 「${꼴}」 — 벌이 없는 세계가 이 게임의 축이다`);
    }
  }
});

test('⑧ 재료 없이 그려지면(배선 실수) 정직하게 말한다 — 고정 과제로 둔갑하지 않는다', () => {
  /* 못 펴는 시드·미검수 판은 이제 라우팅(게임재료 → null)이 말하기 폴백으로 내린다(H4·H5) —
   * 이 카드가 서는 것은 재료 없이 그려진 배선 실수뿐이다. 그래도 조용하면 안 된다. */
  const 글 = 그리기('src/교수멘탈화면.js', { 토큰: 'x', 학생번호: 'SYNK-042' });
  assert.match(글, /불러오지 못했어요/, '재료 없는 렌더가 조용하면 배선 실수가 며칠 안 보인다');
});

test('⑩ 소리는 게이트 한 문으로만 — 발송 achieve 1회 · 실패음 0 · expo-audio 직접 금지', () => {
  /* 게임층 §3-1 ⚠ — 화면이 따로 오디오 세션을 잡으면 「녹음 중 소리 0」이 프로즈로 남는다.
   * 소스층인 이유: 재생은 핸들러 안이라 첫 렌더가 못 돈다(⑥과 같은 판정).
   * 주석은 벗기고 본다 — 설명 속 낱말이 코드로 읽히면 이 검사는 영원히 빨갛거나 영원히 초록이다. */
  assert.equal(코드만(코드만픽스처.입력), 코드만픽스처.기대, '주석 제거기가 죽었다');
  const 코드 = 코드만(소스);
  assert.ok(!/expo-audio|expo-haptics/.test(코드), '화면이 오디오·햅틱을 직접 잡았다 — 문은 src/소리.js 하나다');
  const 부름 = [...코드.matchAll(/효과음\(\s*'([^']+)'\s*\)/g)].map((m) => m[1]);
  assert.deepEqual(부름, ['achieve'], `소리 자리는 발송 achieve 하나뿐이다(실패음 없음) — 실제: ${부름}`);
  assert.doesNotMatch(코드, /bgm재생\s*\(/,
    '09-11 사용자 요청: G1 구곡은 제거됐으며 새 음원 반입 전에는 BGM을 자동 재생하지 않는다');
});

test('⑨ 말하기화면 라우팅 — 판정은 게임재료 하나(H4·H5)·갈래는 막힘 검사 뒤(소스층)', () => {
  const 말하기 = 코드만(fs.readFileSync(path.join(ROOT, 'src', '말하기화면.js'), 'utf8'));
  /* 🔑 `게임재료` 로 가른다 — challenge_id 대조·시드 펴기·**검수확정 게이트**가 그 한 곳에
   * 살아서, 못 펴는 시드·미검수 판이면 null = 조용한 말하기 폴백이다(학생이 그날 아무것도
   * 못 하는 상태가 없다). 게이트 없는 판정(게임과제인가)으로 가르면 미검수 판이 화면에 닿는다. */
  assert.match(말하기, /set게임재료값\(게임재료\(항목\)\)/, '라우팅이 게임재료 판정을 안 지난다(H4·H5)');
  assert.doesNotMatch(말하기, /게임과제인가/, '라우팅이 게이트 없는 판정으로 되돌아갔다 — 미검수 판이 학생에게 열린다');
  assert.match(말하기, /<교수멘탈화면/, 'G1 갈래가 화면을 안 그린다 — 판정만 있고 배선이 없다');
  /* 막힘 검사 «뒤»에 선다 — 순서가 바뀌면 막힌 학생에게 게임이 열려 업로드만 죽는다. */
  const 막힘자리 = 말하기.indexOf('if (서버막힘)');
  const 게임자리 = 말하기.indexOf('if (게임재료값)');
  assert.ok(막힘자리 > -1 && 게임자리 > 막힘자리, 'G1 갈래가 막힘 검사보다 앞이다');
});

...
test('㉢ 등록층 — `시크릿들` 의 갈래 낱말 전량이 `아는플래그` 에 있다', () => {
  ...
  /* 🔑 이 한 줄이 **조회가 절대 빗나가지 않는다**는 것을 기계로 만든다.
   * `고른것` 은 `시크릿들` 의 키에서 걸러낸 것이라 늘 키다 — 남은 구멍은 `기본갈래` 뿐이고,
   * 그게 키가 아니면 무플래그 호출이 `undefined` 이름으로 조용히 착지한다(증상 401).
   * 이 줄이 없으면 그 자리를 아무도 안 지켜서 「조회 실패 대비」 코드가 살아난다. */
  assert.ok(Object.keys(착지.시크릿들).includes(착지.기본갈래),
    '기본갈래가 시크릿들의 키가 아니다 — 무플래그 호출이 이름 없이 착지한다');
});

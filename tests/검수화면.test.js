...
/* ── 쪽 경계 (반박 P1-② · 반 모드는 구조적으로 한 쪽을 넘는다) ─────────────
 * 여기가 「틀려도 조용한 자리」의 표본이다: 뒤 쪽이 남았는데 「없어요」를 그리면 교사는 다 봤다고
 * 믿고 일어서고, 큐는 원래 줄어드는 목록이라 **누구도 빠진 것을 못 센다.** */

test('🔴 커서가 살아 있으면 「없어요」가 아니다 — 「쪽끝」과 「없음」은 다른 화면이다', () => {
  const 기본 = { 불러오는중: false, 항목있음: false, 목록수: 0, 다음커서: null };
  assert.equal(빈화면꼴({ ...기본, 다음커서: 'g|1|2|2026-08-14T01:00:00.000Z|x' }), '쪽끝',
    '손에 든 쪽만 비었는데 「없음」으로 접혔다 — 조용한 미검수 경로다');
  assert.equal(빈화면꼴(기본), '없음');
  assert.equal(빈화면꼴({ ...기본, 목록수: 7 }), '조끝', '조 필터가 비운 것을 반 전체가 빈 것으로 읽는다');
  assert.equal(빈화면꼴({ ...기본, 불러오는중: true, 다음커서: 'g|1|2|t|x' }), '읽는중');
  assert.equal(빈화면꼴({ ...기본, 항목있음: true }), null);
  /* 자기 처방(맹점 ③): 「더 받기를 누르라」고 말하는 화면에서는 그 버튼이 실제로 보여야 한다. */
  assert.equal(더받기보임({ 다음커서: 'g|1|2|t|x', 재검수있음: false, 불러오는중: false }), true,
    '「쪽끝」이 시키는 버튼이 같은 상태에서 안 보인다 — 따를 수 없는 처방이다');
});

test('🔴 「다음 쪽 더 받기」는 목록이 비어도 남는다 — 손에 든 쪽을 다 확정한 자리가 그 자리다', () => {
  const 커서 = 'g|1|2|2026-08-14T01:00:00.000Z|9b1deb4d-3b7d-4bad-9bdd-2b0d7b3dcb6d';
  assert.equal(더받기보임({ 다음커서: 커서, 재검수있음: false, 불러오는중: false }), true);
  assert.equal(더받기보임({ 다음커서: null, 재검수있음: false, 불러오는중: false }), false);
  assert.equal(더받기보임({ 다음커서: 커서, 재검수있음: true, 불러오는중: false }), false,
    '재검수 중에 쪽을 더 받으면 손에 든 판정이 밀린다');
  /* 반 전환이 도는 동안 누르면 **옛 모드 커서**가 새 모드 요청에 실린다(P1-①). */
  assert.equal(더받기보임({ 다음커서: 커서, 재검수있음: false, 불러오는중: true }), false,
    '읽는 중에 버튼이 살아 있다 — 그 창에서 두 모드의 커서가 섞인다');
});

test('반 모드만 쪽을 이어 받는다 — 조 칩이 「이 쪽에 실린 조」가 되면 교사가 없는 조를 찾는다', () => {
  const 커서 = 'g|1|2|2026-08-14T01:00:00.000Z|x';
  assert.equal(이어받을까({ 반모드: { id: 'c1' }, 다음커서: 커서, 쪽수: 1 }), true);
  assert.equal(이어받을까({ 반모드: null, 다음커서: 커서, 쪽수: 1 }), false,
    '기본 큐까지 이어 받으면 감사 표본 우선의 앞쪽 의미가 사라진다');
  assert.equal(이어받을까({ 반모드: { id: 'c1' }, 다음커서: null, 쪽수: 1 }), false);
  /* 상한은 **자르는 것이 아니라 멈추는 것**이다 — 커서가 살아남아 「더 받기」가 그대로 보인다. */
  assert.equal(이어받을까({ 반모드: { id: 'c1' }, 다음커서: 커서, 쪽수: 이어읽기상한 }), false);
  assert.equal(더받기보임({ 다음커서: 커서, 재검수있음: false, 불러오는중: false }), true,
    '상한에서 멈춘 뒤 이어받을 길이 없다 — 그건 조용히 자른 것이다');
});

...
const { 검증, 공통필수, 널허용, 이벤트별필수, 문턱없음, 선택필드, 생산자, 엔진도달, 도달0상한, 생산자섰는데도달0상한, 행동층, 행동안바뀜상한, 늘지않음상한, 차단자갈래, 서버칸, 서버사건 } = require('../lib/이벤트검증.js');
...
test('추가 필수가 0인 사건은 그 이유가 적혀 있다 — 결론과 미정은 같은 모양이면 안 된다', () => {
  /* c11 에서 실장부의 빈 목록이 0이 됐다(`preference.stated` 가 규격을 얻었다). 「빈 것이
   * 하나는 있어야 산 검사다」를 실장부에 요구하면 마지막 빈 칸이 채워지는 **정상 진화**가
   * 이 검사를 죽인다 — 탐지력은 픽스처가 지고, 실저장소에는 거짓양성만 잰다(저장소 공통 규칙). */
  const 사유없는빈것 = (필수표, 사유표) => Object.keys(필수표)
    .filter((k) => !필수표[k].length)
    .filter((k) => !String(사유표[k] || '').trim());
  assert.deepEqual(사유없는빈것({ a: [] }, {}), ['a'],
    '사유 없는 빈 목록을 그냥 지나쳤다 — 이 검사가 아무것도 안 재고 있다');
  assert.deepEqual(사유없는빈것({ a: [] }, { a: '정해서 안 거는 이유를 여기 적어 둔 픽스처다 — 서른 자를 넘긴다' }), [],
    '사유를 적었는데도 막힌다 — 따를 수 없는 처방은 우회를 정상 통로로 만든다(F103)');

  const 사유없음 = 사유없는빈것(이벤트별필수, 문턱없음);
...
test('엔진도달 래칫 — 도달 0 인 사건이 늘어나면 빨개진다', () => {
  ...
  assert.equal(도달0상한, 3, '상한을 올렸다 — 래칫은 내리기만 한다(도달을 적으면 준다)');
});
...
/* ── 교차 불변식④ — 「누가 부른다」와 「사람이 안 쳐도 돈다」는 또 다르다 ──────────────
 *
 * 🔴 불변식③ 이 초록인데도 사슬이 끊겨 있었다. `런타임파일들` 이 `tools/` 를 «런타임»으로 세기
 *   때문이다 — `lib/성과회수.js` 를 부르는 것은 `tools/성과계기판.js` 손 CLI 하나뿐이었고,
 *   그건 **사람이 칠 때만 도는 것**이라 유호님 상시 지시(「수집은 엔진 도달까지가 한 벌 ·
 *   자동화로 학습되는 게 아니라면 의미가 없어」)의 «자동» 조건을 충족하지 않는다.
 *   appsscript `docs/엔진도달_설계.md` §2 ⑦행이 이 사각을 이미 글로 적어 두었다:
 *   *"호출부가 손 CLI 뿐(자동 배선 0 — 「사람 손 없이」 축에선 미완이고, 이 갈림은 장부가 못 본다)"*.
 *   장부가 못 보던 그 갈림을 여기서 본다.
 * 🔑 판정 = 「배포되어 요청·스케줄로 도는 층에서 불리는가」. 그 층은 `supabase/functions/` 하나다.
 *   `tools/`·`src/`(앱 화면)는 각각 사람의 타이핑·탭이 있어야 도므로 근거가 못 된다. */
const 자동층 = path.join('supabase', 'functions');
/* 🔑 «거르는 일»을 검사 안에 둔다 — 호출부가 목록을 미리 걸러 넘기면, 픽스처는 거르지 않은
 *   목록으로 통과하고 실저장소만 걸러진 목록으로 도는 갈림이 생긴다(그러면 탐지력을 못 잰다.
 *   실제로 이 검사가 처음 그렇게 쓰여서 ① 이 빨갰다). */
const 손으로만불린다 = (장부, 파일들) => 안불리는소비자(장부, 파일들.filter((f) => f.includes(자동층)));

test('엔진도달 교차 불변식④ — 소비자는 «사람이 안 쳐도 도는 자리»에서 불린다', () => {
  const 파일들 = 런타임파일들(ROOT);
  const 지목 = (소비자) => ({ x: { 도달: [{ 경로: 'A', 부품: 'p', 소비자 }] } });

  // ① 탐지력(픽스처) — 손 CLI 만 부르는 소비자는 «자동» 축에서 잡힌다.
  //    2026-08-12 이전의 `lib/성과회수.js` 가 정확히 이 모양이었다(③ 초록 · ④ 적색).
  assert.deepEqual(손으로만불린다(지목('lib/성과회수.js'), [path.join(ROOT, 'tools', '성과계기판.js')]),
    ['lib/성과회수.js'],
    '손 CLI 하나만 부르는데 통과했다 — 그러면 「사람이 매번 치면 된다」가 도달로 세어진다');

  // ② 거짓양성 아님 — 배치가 부르는 소비자는 통과한다.
  assert.deepEqual(손으로만불린다(지목('lib/학습자상태.js'), 파일들), [],
    '`functions/deliver` 가 동봉본을 import 하는데 못 읽었다');

  // ③ 실저장소 — 거짓양성만. 여기가 빨개지면 소비자가 **손으로만** 불리는 상태다.
  assert.deepEqual(손으로만불린다(엔진도달, 파일들), [],
    '장부가 지목한 소비자를 «자동으로 도는 자리»에서 부르는 코드가 없다.\n'
    + '처방 = 그 lib 을 도는 함수의 `동봉.json` 에 넣고 `index.ts` 가 import 해 부른다'
    + '(손 CLI 를 하나 더 만드는 것은 이 검사를 통과시키지 못한다 — 그게 이 검사의 목적이다).');
});
...
/* ── 교차 불변식⑤ — 「자동으로 돈다」와 「그 결과가 무언가를 바꾼다」는 또 다르다 ──────────
 *
 * 🔴 ③④ 가 초록인데도 사슬이 한 칸 더 뒤에서 끊겨 있었다. ③④ 가 재는 축은 **호출 여부**다 —
 *   `functions/deliver` 가 `학습자상태`·`성과회수` 를 자동층에서 정말로 import 하므로 둘 다
 *   통과한다. 그런데 부르고 나서 하는 일이 **스탬프와 응답 계수**뿐이다: 축 값은 어디에도
 *   안 남고, `결정`(무엇을 줄지)은 상태 없이 이미 서 있다(「상태 계산이 실패해도 그대로
 *   찍힌다」가 그 증거다). 그래서 철학 정본 부록 A-1 ㉡ 행이 **🔴 스탬프까지만 닿는다**고
 *   적혀 있는 동안, 이 파일의 래칫 둘과 불변식 넷은 전부 초록이었다 — 그 불일치를 아무도
 *   안 쟀다. 여기가 그 자리다(장부 = `lib/이벤트검증.행동층`).
 *
 * ⚠ 천장은 장부 머리말이 적은 그대로다 — 「스탬프한다」와 「읽어서 바꾼다」를 기계는 못 가른다.
 *   여기서 지는 몫은 셋뿐이다: ⓐ빠뜨릴 수 없게 ⓑ올릴 수 없게 ⓒ거짓 지목을 못 하게. */

/** 엔진도달 장부에 실린 부품 이름 전부 — 행동층의 키는 **여기서 파생**한다.
 *  같은 목록을 두 곳에 적으면 갈라지고, 갈라진 쪽의 증상은 언제나 「통과」다(가드 맹점 ④). */
const 부품들 = (장부) => [...new Set(Object.values(장부)
  .flatMap((v) => (Array.isArray(v.도달) ? v.도달.map((d) => d.부품) : [])))].sort();

/** 부품 → 그 부품을 내는 소비자 파일들(장부에서 조인). */
const 부품소비자 = (장부, 부품) => [...new Set(Object.values(장부)
  .flatMap((v) => (Array.isArray(v.도달) ? v.도달 : []))
  .filter((d) => d.부품 === 부품)
  .map((d) => d.소비자))];

/** 행동층이 «파일»로 지목했는데 그 파일이 소비자를 **정말로 들여오지는 않는** 부품.
 *
 * 🔴 `낡은지목`·`낡은도달` 과 **규칙이 다르다**: 저쪽은 「원문에 그 문자열이 있나」까지만 잰다.
 *   그 규칙을 여기 그대로 쓰면 `lib/오늘과제.js` 가 소비자 이름을 **주석에만** 갖고 있는 채로
 *   초록이 된다 — 이 장부가 겨눈 바로 그 거짓이 장부 자신을 통과한다. 그래서 import 로 잰다.
 * 🔑 자기 자신은 근거가 못 된다 — 소비자가 소비자를 「읽는 자리」로 지목하면 순환이다. */
const 거짓행동지목 = (행동, 도달장부) => Object.entries(행동)
  .filter(([, v]) => v && v.파일)
  .filter(([부품, v]) => {
    const 소비자들 = 부품소비자(도달장부, 부품);
    if (!소비자들.length) return true;              // 장부에 없는 부품을 지목했다
    if (소비자들.includes(v.파일)) return true;      // 소비자 자기 자신
    const 원 = 원문(path.join(ROOT, v.파일));
    if (!원) return true;                           // 파일이 없다
    return !소비자들.some((c) => {
      const n = path.basename(c).replace(/\.[^.]+$/, '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      /* 세 표기 전부 실재한다 — lib→lib `require('./x.js')` · tools→lib `require('../lib/x.js')` ·
       * Edge Fn `import m from './x.mjs'`(동봉본이라 경로가 저장소 경로와 다르다). */
      return new RegExp(`(?:require\\(|import\\(|from)\\s*['"\`][^'"\`]*${n}[^'"\`]*['"\`]`).test(원);
    });
  })
  .map(([부품]) => 부품)
  .sort();

/** 장부 둘의 어긋남 — ①행동층에 없는 부품 ②엔진도달에 없는 유령 줄.
 *  호출부가 아니라 여기서 «둘 다» 낸다 — 픽스처와 실저장소가 같은 함수를 지나야 탐지력을 잰다. */
const 장부어긋남 = (도달장부, 행동) => ({
  빠짐: 부품들(도달장부).filter((p) => !(p in 행동)),
  유령: Object.keys(행동).filter((p) => !부품들(도달장부).includes(p)),
});

test('행동층 장부 — 엔진도달의 모든 부품이 파일 xor 사유를 가진다', () => {
  const 부품 = 부품들(엔진도달);
  assert.ok(부품.length > 0, '엔진도달 장부에서 부품을 하나도 못 뽑았다 — 이 검사가 아무것도 안 재고 있다');

  /* ① 탐지력(픽스처) — **이 래칫이 실제로 발화하는 방식.** 새 파생층을 세우며 행동층을 안 적는
   *   것이 앞으로 벌어질 일이고, 그때 빨개져야 한다. 실저장소만 보면 이 힘을 못 잰다. */
  const 새부품 = { z: { 도달: [{ 경로: 'A', 부품: '새로운 파생층', 소비자: 'lib/학습자상태.js' }] } };
  assert.deepEqual(장부어긋남(새부품, {}).빠짐, ['새로운 파생층'],
    '엔진도달에 부품을 늘렸는데 행동층이 비어도 조용하다 — 이 검사가 아무것도 안 재고 있다');
  assert.deepEqual(장부어긋남(새부품, { '없어진 부품': { 사유: 'x' }, '새로운 파생층': { 사유: 'y' } }).유령, ['없어진 부품'],
    '엔진도달에서 사라진 부품이 행동층에 남았는데 통과했다 — 낡은 줄이 영원히 산다');

  // ② 실저장소 — 거짓양성만.
  const { 빠짐, 유령 } = 장부어긋남(엔진도달, 행동층);
  assert.deepEqual(빠짐, [], `엔진도달에 있는데 행동층 장부에 없다 — 그 파생을 읽어 무엇을 바꾸는지 아무도 안 정했다: ${빠짐.join(', ')}`);
  assert.deepEqual(유령, [], `행동층에 있는데 엔진도달 부품이 아니다 — 낡은 줄이다: ${유령.join(', ')}`);

  for (const [부품이름, v] of Object.entries(행동층)) {
    const 파일 = typeof v.파일 === 'string' && v.파일.trim();
    const 사유 = typeof v.사유 === 'string' && v.사유.trim();
    assert.ok(Boolean(파일) !== Boolean(사유),
      `${부품이름}: 파일 xor 사유여야 한다 — 둘 다면 어느 쪽이 정본인지, 둘 다 아니면 아무것도 안 정한 것`);
    // 「나중에」는 사유가 아니다 — 무엇이 서면 생기는지를 적게 한다(생산자 장부와 같은 규칙).
    if (사유) assert.ok(v.사유.length >= 30, `${부품이름} 의 사유가 너무 짧다 — 무엇이 서면 그 자리가 생기는지를 적는다`);
    /* 🆕 늘었나(④완결 셋째 칸 · 심문 B[22]) — 파일 지목(닿음 ✓)에만 필수·사유 갈래엔 금지.
     * 맨몸 「✓」를 못 적게 근거 서술을 강제한다 — 진실성은 사람 몫이되 빈 주장은 기계가 막는다. */
    if (파일) {
      assert.ok(typeof v.늘었나 === 'string' && /^[✓✗] — .{20,}/.test(v.늘었나),
        `${부품이름}: 파일 지목엔 늘었나 칸이 필수다 — 「✓ — <실측 근거>」 또는 「✗ — <무엇이 서면 ✓가 되는지>」(근거 20자 이상). 배선이 이어졌다고 자란 것이 아니다(B[22])`);
    } else {
      assert.ok(!('늘었나' in v),
        `${부품이름}: 사유 갈래에 늘었나 칸이 있다 — 닿지도 않은 부품의 「늘었나」는 물음이 성립하지 않는다`);
    }
  }
});

test('행동층 장부 — 파일 지목은 «주석»이 아니라 «import» 로만 선다', () => {
  const 도달픽스처 = { x: { 도달: [{ 경로: 'A', 부품: '학습자 모델', 소비자: 'lib/학습자상태.js' }] } };

  /* ① 탐지력 — **이 장부가 태어난 이유 그 자체.** `lib/오늘과제.js` 는 배정층인데 소비자
   *   이름을 주석 한 줄로만 갖고 있다. 문자열 규칙이면 여기서 초록이 된다. */
  assert.deepEqual(거짓행동지목({ '학습자 모델': { 파일: 'lib/오늘과제.js' } }, 도달픽스처), ['학습자 모델'],
    '주석에만 이름이 있는 파일이 「읽는 자리」로 통과했다 — 문자열로 재고 있다는 뜻이다');

  // ② 없는 파일도 잡는다.
  assert.deepEqual(거짓행동지목({ '학습자 모델': { 파일: 'lib/없는파일.js' } }, 도달픽스처), ['학습자 모델'],
    '없는 파일 지목이 통과했다');

  // ③ 소비자 자기 자신은 근거가 못 된다 — 그러면 모든 부품이 제 손으로 초록이 된다.
  assert.deepEqual(거짓행동지목({ '학습자 모델': { 파일: 'lib/학습자상태.js' } }, 도달픽스처), ['학습자 모델'],
    '소비자가 자기 자신을 「읽는 자리」로 지목했는데 통과했다');

  // ④ 장부에 없는 부품을 지목해도 잡는다(조인이 비면 근거가 0이다).
  assert.deepEqual(거짓행동지목({ '없는부품XYZ': { 파일: 'lib/성과회수.js' } }, 도달픽스처), ['없는부품XYZ'],
    '엔진도달에 없는 부품을 지목했는데 통과했다');

  // ⑤ 거짓양성 아님 — 정말로 require 하는 파일은 통과한다(`성과회수` 가 `학습자상태` 를 들여온다).
  assert.deepEqual(거짓행동지목({ '학습자 모델': { 파일: 'lib/성과회수.js' } }, 도달픽스처), [],
    '실제 require 를 못 읽었다 — 사람이 실제로 쓰는 표기로 검사해야 한다');

  // ⑥ 실저장소 — 거짓양성만. 파일을 적었으면 그 파일이 소비자를 정말로 들여와야 한다.
  assert.deepEqual(거짓행동지목(행동층, 엔진도달), [],
    '행동층이 지목한 파일이 그 소비자를 들여오지 않는다 — 지목만 하고 배선을 안 했다');
});

/* 🔑 유호님 상시 지시(「수집은 엔진 도달까지가 한 벌」)가 **파생층에서 한 번 더** 걸리는 자리다.
 * 오늘 둘 다 0이다(학습자 모델 · AI 게이트웨이). 이 검사는 그 0 을 초록으로 덮지 않고
 * **세어서 고정**한다 — 새 파생층을 세우면서 그것을 읽는 자리를 안 내면 여기서 빨개진다.
 * 내리는 것은 언제나 통과한다(읽는 자리를 적으면 준다). */
test('행동층 래칫 — 결과가 아무것도 안 바꾸는 부품이 늘어나면 빨개진다', () => {
  const 안바뀜 = Object.entries(행동층).filter(([, v]) => !(v && v.파일)).map(([p]) => p).sort();
  assert.ok(안바뀜.length <= 행동안바뀜상한,
    `파생은 섰는데 그 결과가 «다음에 줄 것»을 못 바꾸는 부품이 ${행동안바뀜상한} → ${안바뀜.length} 로 늘었다: ${안바뀜.join(', ')}\n`
    + '새 파생층을 세우려면 그 값을 읽어 무엇을 바꾸는지를 같은 커밋에 적는다(유호 상시 지시 08-08 · 부록 A-1 ㉡ 행).');
  assert.equal(행동안바뀜상한, 1, '상한을 올렸다 — 래칫은 내리기만 한다(읽는 자리를 적으면 준다)');
});

/* 🆕 ④완결 셋째 칸의 래칫(심문 B[22] · 유호 확정 08-22 D4) — 「닿았는데 안 늘었다」가 늘어나면
 * 빨개진다. 배선을 이었으면(파일 ✓) 자라는 증거(✓)까지가 한 벌이고, 그 전에 다음 배선으로
 * 넘어가는 것을 이 수가 센다. 실학생 0 시대엔 ✗ 가 정직값이라 0 을 요구하지 않는다(F207). */
test('늘지않음 래칫 — 닿았는데 안 자라는 부품이 늘어나면 빨개진다', () => {
  const 늘지않음 = Object.entries(행동층)
    .filter(([, v]) => v && v.파일 && typeof v.늘었나 === 'string' && v.늘었나.startsWith('✗'))
    .map(([p]) => p).sort();
  assert.ok(늘지않음.length <= 늘지않음상한,
    `닿았는데(파일 지목) 늘었다는 증거가 없는 부품이 ${늘지않음상한} → ${늘지않음.length} 로 늘었다: ${늘지않음.join(', ')}\n`
    + '배선을 이었으면 자라는 증거까지가 한 벌이다 — 첫 실측이 서면 그 칸을 ✓ 로 바꾸고 이 상한을 내린다.');
  assert.equal(늘지않음상한, 1, '상한을 올렸다 — 래칫은 내리기만 한다(✓ 실측이 서면 준다)');
});

/** 빈 칸(=대장의 «빨강») 전량 — 두 장부에서 같은 규칙으로 뽑는다.
 *  🔑 **픽스처와 실저장소가 같은 함수를 지난다** — 갈라지면 탐지력을 잰 곳과 지키는 곳이 달라진다.
 *  판정 축은 «값의 유무»다: 엔진도달은 `도달` 배열이 비었을 때, 행동층은 `파일` 이 없을 때가 빨강.
 *  ⚠ 사유 문장은 **안 본다** — 문장은 개서되고, 그날 이 검사가 조용히 빈다. */
const 빨간칸들 = (도달장부, 행동) => [
  ...Object.entries(도달장부)
    .filter(([, v]) => !(v && Array.isArray(v.도달) && v.도달.length))
    .map(([이름, v]) => ({ 층: '엔진도달', 이름, 차단자: v && v.차단자 })),
  ...Object.entries(행동)
    .filter(([, v]) => !(v && v.파일))
    .map(([이름, v]) => ({ 층: '행동층', 이름, 차단자: v && v.차단자 })),
];

/* 🔑 유호 지시 2026-08-16 — 「대장의 빨간 칸을 «지금 할 수 있음 / 학생이 와야 함»으로 가른다」.
 * 이 검사가 없으면 F513 이 그대로 재발한다: 빈 칸이 **왜** 비었는지는 사유에 적혀 있는데
 * 「누가 열 수 있나」는 어디에도 구조로 없어서, 화면이 다섯 칸을 같은 빨강으로 냈고 한 세션이
 * 표본 0 짜리 되먹임을 짜러 착수했다(착수 직전에 되돌아왔다 — 그 왕복이 이 필드의 존재 이유다).
 * ⚠ 값의 «정직»은 기계가 못 잰다(도달·사유와 같은 성질의 사람 선언) — 기계 몫은 빠뜨림과 오타뿐. */
test('차단자 — 빈 칸마다 «누가 열 수 있나»가 닫힌 어휘로 적힌다 (유호 지시 08-16 · F513)', () => {
  const 어휘 = Object.keys(차단자갈래);
  assert.deepEqual(어휘, ['지금', '학생', '설계상'],
    '차단자 어휘가 바뀌었다 — 읽는 쪽(appsscript tools/이해대장.js)이 같은 세 갈래를 그린다. 한쪽만 바꾸면 화면이 조용히 빈 갈래를 낸다');

  /* ① 탐지력(픽스처) — 이 검사가 실제로 발화하는 두 방식. 실저장소만 보면 이 힘을 못 잰다. */
  const 도달픽스처 = { z: { 사유: '어떤 사유가 서른 자를 넘도록 길게 적혀 있는 칸이다.' } };
  assert.deepEqual(
    빨간칸들(도달픽스처, {}).filter((c) => !어휘.includes(c.차단자)).map((c) => c.이름), ['z'],
    '차단자를 아예 안 적은 빈 칸이 통과했다 — 그러면 화면이 그 칸을 어느 갈래에도 못 넣는다');
  assert.deepEqual(
    빨간칸들({}, { p: { 차단자: '나중에', 사유: 'x' } }).filter((c) => !어휘.includes(c.차단자)).map((c) => c.이름), ['p'],
    '어휘에 없는 차단자가 통과했다 — 오타 하나가 화면에서 조용히 사라지는 칸이 된다');
  // 채운 칸은 애초에 빨강이 아니다 — 차단자를 요구하지 않는다(거짓양성 방지).
  assert.deepEqual(빨간칸들({ y: { 도달: [{ 경로: 'A', 부품: '학습자 모델', 소비자: 'lib/학습자상태.js' }] } }, {}), [],
    '이미 닿은 칸을 빨강으로 셌다 — 그러면 채운 사람에게 차단자를 적으라고 요구하게 된다');

  /* ② 실저장소 — 거짓양성만. */
  const 빨강 = 빨간칸들(엔진도달, 행동층);
  assert.ok(빨강.length > 0, '빨간 칸을 하나도 못 뽑았다 — 이 검사가 아무것도 안 재고 있다');
  const 미표기 = 빨강.filter((c) => !어휘.includes(c.차단자));
  assert.deepEqual(미표기.map((c) => `${c.층}/${c.이름}`), [],
    `빈 칸인데 차단자가 없거나 어휘 밖이다 — 「누가 열 수 있나」를 안 정하면 대장에서 다섯 칸이 같은 빨강이 된다(F513): ${미표기.map((c) => c.이름).join(', ')}`);

  /* ③ 분모를 못박는다 — 갈래별 수가 바뀌면 대장 화면의 결론도 바뀐다(0은 분모와 함께 · 유호 확정 08-14).
   *   ⚠ 래칫이 아니다: 「지금」이 줄어 「설계상」이 늘면 그건 빚을 갚은 게 아니라 **판정을 바꾼** 것이라
   *   눈으로 봐야 한다. 그래서 어느 방향이든 여기서 멈춘다. */
  const 셈 = 빨강.reduce((a, c) => ({ ...a, [c.차단자]: (a[c.차단자] || 0) + 1 }), {});
  /* 2026-08-17 `exam.result` 지금 → 학생 (`local_be03f4c7`) — 근거 전문은 `엔진도달` 그 칸의 주석.
   * 요지: 남은 ⓐⓑ 의 차단자가 둘 다 재료였다(회고 행 0 · 성적 0건 + 창 30일 불일치)라 repo 안에
   * 할 일이 없다. 래칫·빨강·`도달` 은 안 건드렸다 — 바뀐 것은 「누가 열 수 있나」 한 칸이다.
   * 2026-08-23 「지금」 1 → 0 — 빚을 갚았다(판정 변경 아님): '학습자 모델' 행동층 칸이 파일 지목이
   * 됐다(08-21 상태기반 착지 · 심문 G1 이 낡은 장부를 짚어 이행). */
  assert.deepEqual(셈, { 설계상: 2, 학생: 2 },
    `빨간 칸 갈래별 수가 바뀌었다 — 칸을 채웠으면 그 줄을 지우고, 판정을 바꿨으면 근거를 커밋에 적는다: ${JSON.stringify(셈)}`);
});

...
const { 검증, 공통필수, 널허용, 이벤트별필수, 문턱없음, 선택필드, 생산자, 엔진도달, 도달0상한, 생산자섰는데도달0상한, 행동층, 행동안바뀜상한, 차단자갈래, 서버칸, 서버사건 } = require('../lib/이벤트검증.js');
...
test('행동층 래칫 — 결과가 아무것도 안 바꾸는 부품이 늘어나면 빨개진다', () => {
  ...
  assert.equal(행동안바뀜상한, 2, '상한을 올렸다 — 래칫은 내리기만 한다(읽는 자리를 적으면 준다)');
});
...
test('차단자 — 빈 칸마다 «누가 열 수 있나»가 닫힌 어휘로 적힌다 (유호 지시 08-16 · F513)', () => {
  ...
  assert.deepEqual(셈, { 설계상: 2, 지금: 1, 학생: 2 },

...
test('C1 허용목록 — 표 밖 키는 벗겨지고, 모르는 챌린지는 전부 벗겨진다(새 키·새 모듈 기본값 = 안 나감)', () => {
  ...
  /* ⚠ [2026-08-12] 픽스처가 G2 → G3 로 옮겨 갔다. **이 시험이 예고한 그 날이 왔다**:
   *   G2 가 등록부에 서면서(`lib/게임스냅샷.등록challenge들`) 더는 「미등재」의 예가 아니다.
   *   축은 한 글자도 안 바뀐다 — 「등재 «전»에는 안 나간다」이고, 그 실물이 이제 G3 다.
   *   G3 를 등재하는 커밋이 이 줄을 다시 옮긴다(그게 이 픽스처의 일이다).
   *   🔑 G2 의 등재 «후» 판정(정답만 벗겨지고 다섯 키는 산다)은 `tests/게임스냅샷.test.js` 몫. */
  const 미래판 = { challenge_id: 'g3-알바변명', 지시문: 'x', 질문: 'x', 정답: '기준 교정문' };
...
const 두팩캐시 = new Map();
const { 게임배정: 두팩배정, G2앵커전부, 초급급수대 } = 세우기(배정경로, fetch금지, { 캐시: 두팩캐시, 바꾼소스: 두팩소스 });
const {
...
test('ⓐ 초급(1~2·미정)은 **G2** 를 받는다 — 「제외」가 아니라 「다른 모듈로 보낸다」', () => {
  for (const 급수 of ['Lv1', 'Lv2', null, '중급']) {
    ...
    assert.equal(r.retry_of_event_id, null, 'G2 재제출 조인은 아직 안 섰다 — 지어내지 않는다');
  }
});

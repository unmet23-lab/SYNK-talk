...
test('C1 허용목록 — 표 밖 키는 벗겨지고, 모르는 챌린지는 전부 벗겨진다(새 키·새 모듈 기본값 = 안 나감)', () => {
  ...
  /* ⚠ [2026-08-13] 픽스처가 G3 → G4 로 옮겨 갔다(G3 등재 커밋 6ab853ff — 이 픽스처의 일 그대로).
   *   축은 한 글자도 안 바뀐다 — 「등재 «전»에는 안 나간다」이고, 그 실물이 이제 G4 다.
   *   G4 를 등재하는 커밋이 이 줄을 다시 옮긴다(4모듈이 전부 서면 합성 미등재 id 로).
   *   🔑 등재 «후» 판정(공개 키만 산다)은 `tests/게임스냅샷.test.js` 몫. */
  const 미래판 = { challenge_id: 'g4-서류관문', 지시문: 'x', 질문: 'x', 정답: '기준 교정문' };

...
test('[탐지] 리뷰 P2-① 확장분 — 호환한자·반각 가나·astral 이 구판에서 «통과»로 샜다', () => {
  for (const [이름, cp] of [['호환한자(한국어 독음 구역)', 0xf914], ['반각 가타카나', 0xff71],
    ['astral 한자(확장B — 서러게이트 쌍)', 0x20000], ['astral 가나 보충', 0x1b000]]) {
    assert.deepEqual(찾기(`깨끗한 문장 ${String.fromCodePoint(cp)} 뒤`), [cp], `${이름} 를 놓쳤다`);
  }
});

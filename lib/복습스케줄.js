 ...
 *  ⚠ S·D(stability·difficulty)는 «일부러» 안 자른다(심문 S3): ts-fsrs 5.4.2 의 next 는
 *  last_review 기준이라 잘린 due 가 다음 계산에 안 들어간다 — 그래서 due 절단만으로 안전하다.
 *  이 전제는 «판올림에 취약»하다: 라이브러리를 올리면 next 의 기준이 바뀔 수 있고 그때 이 함수는
 *  조용히 어긋난다. 2026-09-11에 400갈래로 재확인했고 회귀 ④-b가 직접 대조한다.
 *  회귀 ⑧이 설치본 버전을 못박는다 — 올리는 날 그 검사가 이 재검토를 강제한다. */
function 가둠(카드) {

 ...
 *   행들  = { event_id, learner_id, event_type, occurred_at, retry_of_event_id?, payload?,
...
function 리뷰파생({ 행들, 교정들 } = {}) {
  ...
  for (const [sid, 태그들] of 확정판들) {
    ...
    for (const t of 카드들) 담기(행, t, 행.occurred_at, false, null, 'R1', '교정실패');
  }
  ...
  for (const s2 of 사건정렬(행목록.filter((x) => x.event_type === 'submission.created' && x.retry_of_event_id != null))) {
    ...
    for (const t of s1태그들.filter((x) => x !== 무오류표식 && 계약태그셋.has(x) && !s2셋.has(x))) {
      담기(s2, t, s2.occurred_at, true, null, 'R2', '재제출성공');
    }
  }
  ...
  for (const r of 사건정렬(행목록.filter((x) => x.event_type === 'quiz.answered'))) {
    ...
    // 카드는 언제나 `재는축` 이다 — 맞음의 `오류태그`('오류없음')는 라벨이지 카드가 아니다.
    담기(r, 빈칸.재는축, r.occurred_at, 판.판정 === '맞음', 확신도값(r.payload), 'R3', '문항축');
  }

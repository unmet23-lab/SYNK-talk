...
const { 답해석, 인자정책 } = require('./라디오채팅파서.js');
const { 몽골날짜 } = require('./몽골날짜.js');
const { 라디오태스크종 } = require('./라디오태스크.js');
const { 보기id들 } = require('./이벤트검증.js');
const { 그때유효평가 } = require('./동의게이트.js');

/* v2 (2026-08-12 · 반박 c757278 수리) — 판정이 넷 바뀌었다: ①선호·정서는 학생×몽골날짜×명령
 * **하루 첫 선언만** 승격(나머지 `하루접힘` — 방송 채팅 반복이 선호 «표본»으로 부풀던 자리)
 * ②겹치는 활성 링크는 고르지 않고 `링크겹침` 으로 센다(오귀속은 남의 학생 consent·급수로
 * 저장되는 소급 불가 사고라, 지어내는 쪽보다 제외가 옳다) ③시점 조회가 입력 순서가 아니라
 * **시각 정렬**로 고른다 ④스냅샷 보기 모양을 검증기 `보기id들` 로 검사(`스냅샷불량`). */
const 승격판 = 'radio-promote.v2';
...
const 명령과업 = Object.freeze({
  목표선언: Object.freeze({
    ...
    task_snapshot: Object.freeze({ 지시문: '오늘의 목표를 한 문장으로 적어요 — !목표 <문장>', 판: 'radio-cmd.v1' }),
    task_schema_ver: 'radio-cmd.v1',
  }),
  자습체크인: Object.freeze({
    task_ref: 'radio:cmd:자습체크인:v1',
    task_snapshot: Object.freeze({ 지시문: '자습을 시작하며 한 문장을 남겨요 — !출석 <한 문장>', 판: 'radio-cmd.v1' }),
    task_schema_ver: 'radio-cmd.v1',
  }),
});
 ...
 * 🔴 반박 ⑤: 「입력의 마지막 행」은 판정이 아니다 — Fn 질의에 order by 가 없어 행 순서가
 *   실행마다 갈리고, 그 순서로 고르면 사건이 **남의 학생**에게 그 학생의 consent·급수로
 *   저장된다(소급 불가). 겹침(둘 이상)은 정렬로 하나를 «고르는» 것도 넘겨짚기다 — 어느 쪽이든
 *   절반은 남이다. 아예 고르지 않고 호출부가 `링크겹침` 으로 센다(입력 순서 무관·결정적). */
function 활성링크(링크들, channel_id, t) {
  const 시각 = 때(t);
  const 맞음 = (링크들 || []).filter((l) => l.channel_id === channel_id
    && 때(l.confirmed_at) <= 시각
    && (l.unlinked_at == null || 시각 < 때(l.unlinked_at)));
  return { learner_id: 맞음.length === 1 ? 맞음[0].learner_id : null, 후보수: 맞음.length };
}

/* occurred_at 시점 유효 동의 — 술어의 정본은 `lib/동의게이트.그때유효평가` 다(반박 ⑧: 여기
 * 손으로 옮겨 적으면 다섯 번째 사본 — 「새 자리는 둘 중 하나를 고른다」의 그 자리). 이 함수가
 * 더하는 것은 「여럿이면 가장 최근 agreed_at」 하나뿐이다(정본 SQL 의 order by 와 같은 눈금). */
function 유효동의(동의목록, t, 지금) {
  const 시각 = 때(t);
  const 이제 = 때(지금);
  /* 동률(같은 agreed_at)은 2차 키로 가른다(재반박 ⑤ 잔여) — 입력 순서에 맡기면 불변 행에
   * 박히는 consent_id·consent_ver 가 실행마다 갈린다. localeCompare 금지(로케일 의존). */
  const 산것 = (동의목록 || [])
    .filter((c) => 그때유효평가(c, 시각, 이제))
    .sort((a, b) => (때(a.agreed_at) - 때(b.agreed_at))
      || (String(a.consent_id) < String(b.consent_id) ? -1 : 1));
...
function 라운드찾기(라운드들, t) {
  ...
  const 후보 = (라운드들 || []).filter((r) => 때(r.shown_at) <= 시각)
    .sort((a, b) => (때(a.shown_at) - 때(b.shown_at))
      || (String(a.round_id) < String(b.round_id) ? -1 : 1)); // 동률 결정성(재반박 ⑤ 잔여)
  return 후보.length ? 후보[후보.length - 1] : null;
}

/* 반박 ⑤ 동축 — 겹치는 표시 창이면 **가장 최근에 뜬 카드**가 그때 화면에 있던 것이다.
 * 입력 순서가 아니라 shown_from 으로 고른다(동률은 content_ref — 결정적이기만 하면 된다). */
function 카드찾기(카드들, t) {
  const 시각 = 때(t);
  const 후보 = (카드들 || []).filter((c) => 때(c.shown_from) <= 시각
    && (c.shown_to == null || 시각 < 때(c.shown_to)))
    .sort((a, b) => (때(b.shown_from) - 때(a.shown_from))
      /* localeCompare 금지(재반박 ⑨) — 로케일·정규화에 따라 0을 돌려주면 입력 순서로 되돌아간다. */
      || (String(b.content_ref) < String(a.content_ref) ? -1 : 1));
  return 후보.length ? 후보[0] : null;
}

/* 선호·정서의 「학생×몽골날짜×갈래」 접기 키(반박 ⑥). 갈래 = 선호는 `preference_dimension`,
 * 정서는 `affect_kind` — **명령이 아니라 뜻 축**이다(명령으로 키를 지으면 DB 기존 행에서 같은
 * 키를 못 짓는다: 행에는 명령이 없다). Fn 이 기존 행으로 같은 키를 지어 `이미하루키` 로 넘기고,
 * 키 조립이 두 곳에 살면 갈라지므로 조립은 **여기 하나**다. */
function 선호정서키(learner_id, 갈래, sent_at) {
  return `${learner_id}:${몽골날짜(new Date(sent_at))}:${갈래}`;
}

/* 원장 한 묶음 → 승격 계획. 순수 — 입력에 없는 것은 지어내지 않고 «제외»로 센다.
 * 재료 = { 메시지들, 라운드들, 링크들, 동의들(learner→[]), 학생들(learner→{level_current,goal_track}),
 *          카드들, 이미승격키(Set<message_id>), 이미승격라운드(Set<`${learner}:${round_id}`>),
 *          이미하루키(Set<선호정서키> — 앞선 실행·앞 배치가 이미 승격한 선호·정서의 날), 지금 } */
function 승격계획(재료) {
  const {
    메시지들 = [], 라운드들 = [], 링크들 = [], 동의들 = {}, 학생들 = {},
    카드들 = [], 이미승격키 = new Set(), 이미승격라운드 = new Set(), 이미하루키 = new Set(), 지금,
  } = 재료 || {};
  ...
  const 이번하루키 = new Set();     // 선호·정서 — 한 묶음 안의 「하루 첫 선언 1건」(반박 ⑥)
  ...
  for (const m of 정렬) {
    ...
    const { learner_id, 후보수 } = 활성링크(링크들, m.channel_id, m.sent_at);
    /* 겹침은 고르지 않는다(반박 ⑤) — 어느 쪽이든 절반은 남의 학생이고, 오귀속 행은 불변이라
     * 못 되돌린다. 원장은 남으므로 링크 이력이 정리되면 다음 실행이 정상 승격한다(소급 가능). */
    if (후보수 > 1) { 빼기(m, '링크겹침'); continue; }
    ...
    const 봉투 = {
      ...
      command_kind: m.command_kind, // 응답 분모 표시용(경미 ⑬ — Fn 이 사유에 실제 명령을 적는다)
    };

    if (m.command_kind === '답') {
      /* 「필수 인자 없으면 접수 안 함」의 정본은 명령표의 인자 정책 하나다(반박 ② — 여기 널
       * 검사를 따로 적으면 정책과 코드가 주석으로만 묶여 갈라진다). */
      if (인자정책(m.command_kind) === '필수' && m.command_arg == null) { 빼기(m, '인자없음'); continue; }
      ...
      /* 원장 jsonb 는 **못 믿는 입력**이다(반박 ③ — quiz_round.task_snapshot 생산자가 아직
       * 없어 모양 보증이 0). 모양 판정은 검증기 ⑦의 `보기id들` 그대로다: 문자열 원소·중복
       * id·label 누락이면 null — 그 행을 넣으면 ⑦이 거절할 모양이 불변 행으로 앉는다. */
      const ids = 보기id들(스냅.보기);
      if (!ids) { 빼기(m, '스냅샷불량'); continue; }
      const 자리 = Number(해석.선택);
      if (!Number.isInteger(자리) || 자리 < 1 || 자리 > ids.length) { 빼기(m, '자리없음'); continue; }
    ...
    if (사건이름 === 'preference.stated') {
      ...
      /* 하루 접기(반박 ⑥) — 방송 채팅은 하루 상한이 없어 `!빗소리` 반복이 선호 «표본»으로
       * 부풀고, 원신호 상한(창 안 최신 우선)에서 배정·제출을 밀어냈다. 같은 학생×몽골날짜×
       * 차원의 **첫 선언만** 승격한다: 첫째는 이미 승격된 뒤라 「마지막 선언」은 재실행마다
       * 판정이 뒤집혀 못 쓰고(불변 행), 날이 열려 있는 동안 「마지막」은 원리상 미확정이다.
       * 원장은 전부 남으므로 잃는 것은 없다(소급 재산출 가능). */
      const 하루키 = 선호정서키(learner_id, 규격.차원, m.sent_at);
      if (이미하루키.has(하루키) || 이번하루키.has(하루키)) { 빼기(m, '하루접힘'); continue; }
      이번하루키.add(하루키);
      const payload = {
      ...
      계획.push({ ...봉투, payload, 하루키 });
      continue;
    }

    if (사건이름 === 'affect.reported') {
      /* 자유 서술(인자)은 payload 로 안 올린다 — 원장 body 에 그대로 있고 재파싱 가능(계약 사유).
       * 하루 접기는 선호와 같은 자리다 — 스무 번 친 `!슬럼프` 는 스무 번의 슬럼프가 아니다. */
      const 하루키 = 선호정서키(learner_id, 'slump', m.sent_at);
      if (이미하루키.has(하루키) || 이번하루키.has(하루키)) { 빼기(m, '하루접힘'); continue; }
      이번하루키.add(하루키);
      계획.push({ ...봉투, payload: { affect_kind: 'slump', stated_via: 'radio_chat' }, 하루키 });
      continue;
    }
  ...
  /* 🔴 «접기»를 재현 가능하게 — c16 어휘 둘을 전 계획에 싣는다(심문 전건판정 ⑧ · 09-02).
   *
   * 왜 필요했나: 이 승격기는 「학생×몽골날짜×갈래」로 접는데(위 `선호정서키` · 하루접힘),
   * 그 «날짜»가 `correlation_id` 의 결정적 uuid **안에 녹아** 있어 역산이 안 됐다. 그리고
   * 접은 «판»(`승격판`)은 Fn 응답에만 실리고 사건에는 없었다. ⇒ 날짜 모듈이 바뀌거나 재실행되면
   * **같은 `sent_at` 이 다른 날짜로 접히는데 그 사실이 행 어디에도 안 남는다.**
   *
   * 🔑 둘은 한 벌이다 — 결과(`fold_date`)만 있으면 왜 그렇게 접혔는지 모르고,
   *   판(`promote_ver`)만 있으면 그 판이 무엇을 냈는지 다시 «계산»해야 한다(원장이 지워지면 그것도 못 한다).
   * 🔑 한 자리에서 주입한다 — 갈래마다 손으로 넣으면 새 갈래가 생기는 날 하나가 빠지고,
   *   빠진 갈래의 증상은 「그 사건만 재현 불가」라 조용하다.
   * ⚠ `occurred_at` 이 아니라 **접기에 실제로 쓴 값**(`sent_at`)으로 센다 — 둘이 갈리면
   *   장부의 날짜와 접힌 날짜가 달라져 이 칸이 거짓말을 시작한다. */
  for (const c of 계획) {
    c.payload = Object.assign({}, c.payload, {
      fold_date: 몽골날짜(new Date(c.occurred_at)),
      promote_ver: 승격판,
    });
  }
...
module.exports = { 승격판, 승격표, 명령과업, 선호규격, 결정적uuid, 활성링크, 유효동의, 라운드찾기, 카드찾기, 선호정서키, 승격계획 };

...
import { 모듈상수, 펴기, 검수확정 } from '../contents/교수멘탈문항.js';
import { G1스냅샷 } from './게임스냅샷.js';
import { 차원들, 보기세우기, 선택payload } from './선택로그.js';
import { 칸들 } from './작성과정.js';

/* §6 표의 고정값 — 값목록 정본은 계약 JSON 이고 여기는 지목만 한다. */
const TASK_TYPE = '숙제제출';
const TASK_FORMAT = '쓰기첨삭';

/**
 * 스냅샷 **모양**의 판 (`submission.task_schema_ver` · L0 §3-3).
 * 문항 자체의 판은 스냅샷 안의 `문항판` 이다 — 둘은 다른 축이다(발주 G1 §6).
 * 🔴 `게임스냅샷.스냅샷키들`(6키)이 바뀌는 날 이 판이 함께 오른다 — 회귀가 키 목록을
 *   못박아 두어, 키를 바꾸고 판을 안 올리면 그 테스트부터 빨개진다.
 */
export const 스냅샷모양판 = 'g1스냅샷.v1';
...
export const 사과전략 = Object.freeze({
  ...
  보기들: Object.freeze([
    ...
    Object.freeze({ option_id: 'g1-사과-대안', label: '대신 할 수 있는 것을 제안한다' }),
  ]),
});

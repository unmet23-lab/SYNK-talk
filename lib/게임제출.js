...
import { G1스냅샷, 스냅샷모양판, 과제유형, 게임스냅샷인가 } from './게임스냅샷.js';
 ...
 * 🔑 판정 정본은 `게임스냅샷.게임스냅샷인가` 하나다(배정 배선 ④) — 서버 거름망(C1)과 앱 라우팅이
 *   같은 판정을 두 곳에 적으면 갈라진 쪽이 조용히 통과한다(값은 팩 `모듈상수` 지목 그대로).
 * ⚠ `task_format` 은 배정 스냅샷의 키가 아니라(§6-8 — 그 축은 `submission` 안이다) 여기서
 *   안 묻는다. 형식은 제출 조립이 고정값으로 박는다.
 */
export function 게임과제인가(항목) {
  return 게임스냅샷인가(항목 && 항목.task_snapshot);
}

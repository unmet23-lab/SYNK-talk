...
const { 확인카드, 부정키, 축후보들 } = require('./성향확인.js');
const { 목표카드 } = require('./목표확인.js');
...
/** 지난날 응답은 오늘 응답 억제와 분리한다. 다른 축도 같은 날 답한 조건에서 긍정/부정만 다르게 비교한다. */
function 지난응답이력(카드들, 응답일, { 부정쌍 = null, 조회일 } = {}) {
  if (!조회일 || String(응답일) >= String(조회일)) throw new TypeError('지난응답이력: 응답일은 조회일보다 앞서야 한다');
  const 쌍들 = 카드들.map(카드쌍);
  if (!쌍들.length || 쌍들.some((p) => !p)) throw new TypeError('지난응답이력: 비교할 카드 쌍이 필요하다');
  return { 오늘답수: 0, 오늘답한키들: [],
    부정키들: 부정쌍 ? [부정키(부정쌍.trait_axis, 부정쌍.shown_key)] : [],
    축별마지막날: Object.fromEntries(쌍들.map((p) => [p.trait_axis, String(응답일)])) };
}

/** 후보 축마다 한 카드를 같은 지난날에 답한 것으로 준비한다. 축 공정순서가 부정 효과를 가리지 않게 한다. */
function 비교답카드들(원행들, { 기준, 시간대 } = {}) {
  const { 상태 } = 확인카드파생(원행들, { 기준, 시간대 });
  const 축들 = { 리듬: 상태.축.리듬, 작성과정: 상태.축.작성과정, 집중띠: 상태.축.집중띠 };
  const 후보들 = 축후보들(축들, {}), 본축 = new Set();
  return 후보들.filter((p) => { if (본축.has(p.축)) return false; 본축.add(p.축); return true; })
    .map((p) => 확인카드(축들, { ...첫이력(), 부정키들: 후보들.filter((q) => q !== p)
      .map((q) => 부정키(q.축, q.키)) }, 기준, 상태.estimator_version));
}
...
/** API 응답만 판정한다. 확인과 목표를 서로 대체하지 않고, 화면 렌더·노출은 이 함수의 증거가 아니다. */
function progress카드검사(몸, 오늘) {
  const 확인 = 몸 && 몸.오늘의확인, 목표 = 몸 && 몸.오늘의목표;
  const 목표기대 = 목표카드({ 오늘답함: false }, 오늘);
  const 확인빠짐 = 카드칸빠짐(확인, 확인카드칸들);
  const 목표빠짐 = 목표기대 ? 카드칸빠짐(목표, 목표카드칸들) : [];
  return { 확인통과: !!몸 && 몸.ok === true && 확인빠짐.length === 0,
    목표통과: !!몸 && 몸.ok === true && Object.prototype.hasOwnProperty.call(몸, '오늘의목표')
      && (목표기대 ? 목표빠짐.length === 0 && 목표.class_date === 오늘 : 목표 === null),
    확인빠짐, 목표빠짐, 목표기대: 목표기대 !== null, 화면검증: '미실행' };
}

/** 키 하나의 효과를 볼 때 쓰는 요약 조각. 단위·순서는 과제요약 산출 그대로이며 여기서 재계산하지 않는다. */
function 축줄(요약, 축) {
  return String(요약 ?? '').split('\n').find((줄) => 줄.startsWith(`${축}: `)) || '';
}

function 지표값(요약, 축, 지표) {
  const m = 축줄(요약, 축).match(new RegExp(`(?:^|[ (])${지표.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}=([^ )]+)`));
  return m ? m[1] : null;
}
...
module.exports = {
  첫이력, 확인카드파생, 카드쌍, 같은쌍, 부정뒤이력, 지난응답이력, 비교답카드들,
  progress카드검사, 축줄, 지표값,

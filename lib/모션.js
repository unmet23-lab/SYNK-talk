...
export function 세기계획({ 부터, 까지, 켬 = true, 줄임 = false }) {
  ...
  const 칸수 = 까지 - 부터;
  /* 총 420ms 를 칸수로 나누되 40~140ms 로 묶는다 — 1칸이면 툭 바뀌는 대신 한 박자(140ms),
   * 칸이 많으면 지루해지지 않게 40ms 바닥. 이 앱의 하루 수는 작아서(3~10) 보통 그 사이다. */
  return { 칸수, 간격: Math.min(140, Math.max(40, Math.round(420 / 칸수))) };
}

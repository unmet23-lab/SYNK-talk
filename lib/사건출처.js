...
const 출처종류 = {
  ...
  /* c13 — 성향 확인 답(Ⅲ⑥). 봇 «추정»에 대한 학생의 「맞아요/아니에요」는 학생이 말한 답이라
   *   explicit 이다 — 추정 자체(inferred)와 그 확인(explicit)이 갈려 있어야 「확인된 라벨」이
   *   추정보다 신뢰 높은 재료로 선다(§10 ① — 얻을 수 있는 최고 품질). */
  'estimate.responded': 'explicit',
  /* c14 — 목표 이행 자기 신고(교실 수집 ② · 유호 확정 08-31). 학생이 「해냈다/아직」을 말한
   *   답이라 explicit 이다 — 🔴 observed 로 두면 「지켰다고 말했다」가 「지켰다」로 쌓여
   *   거짓 축이 선다(정본 설계 §2 — 관측 짝은 mastery 파이프라인의 그 문형 실제 변화다). */
  'goal.responded': 'explicit',
  'data_use.granted': 'explicit',
  'data_use.revoked': 'explicit',

  // ── teacher: 앱 밖에서 사람이 넣는다(TOPIK 실성적 = 강사 입력 · L0 §3-2).
  'exam.result': 'teacher',
  /* c14 — 강사 교실 관찰(관찰태그 설계 v1.1 §2 — 확정된 관찰만 적재되고 그 행은 언제나
   *   teacher 다 · AI 초안은 사건이 되지 않는다). 확정의 질은 등급이 아니라 감사 칸
   *   (draft_modified 물리칸)이 진다. */
  'observation.noted': 'teacher',

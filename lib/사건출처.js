 ...
 *
 * ■ 이 파일이 쥔 표는 **둘**이다 (c14 · 08-31)
 *   ① `출처종류` — 어떻게 알게 됐나(`source_kind`) · ② `운영기록계열` — 그 앎이 동의 아래
 *   있나(`consent_ver`). 둘 다 **event_type 당 한 값**이고 둘 다 늘리는 것을 잊으면 소급이
 *   불가하다 — 같은 규율이라 같은 파일에 둔다. 까닭은 아래 ②의 머리말에 있다.
 */
'use strict';
...
/* ══ 둘째 축 — 「그 앎이 **무엇 아래** 모였나」 (c14 · 2026-08-31) ══════════════════════
 * 위 표와 **같은 결**이라 여기 둔다: event_type 당 한 값이고, 새 사건을 계약에 늘리면 여기도
 * 늘려야 하며, 안 늘리면 소급이 원리상 불가하다. 다른 것은 축 하나뿐이다 —
 * 위가 「어떻게 알게 됐나」(`source_kind`)라면 이건 「그 수집이 동의 아래 있나」다.
 *
 * ■ 왜 지금 생겼나 (08-31 실측 · c14 가 남긴 구멍)
 *   c14 가 「`observation.noted`·`exam.result` = 학원 운영 기록 계열 — 동의 게이트를 지나지
 *   않되 «조용히 버려짐»도 없다」를 **명문화했는데**, `learning_events.consent_ver` 가
 *   `not null`(c6:427)이라 **그 칸에 무엇을 넣을지는 안 정했다.** 게이트를 안 지나는 사건은
 *   동의 «행»이 없으므로 `v18.9` 같은 동의판을 넣으면 그 자리가 거짓이 된다.
 * 🔑 그래서 값이 **형식부터 다르다** — 동의판은 `v<숫자>.<숫자>` 이고 이것은 한글 한 낱말이라,
 *   이 값을 동의판으로 세는 집계가 생기면 파싱이 즉시 깨져 눈에 띈다. 조용히 섞이지 않는다.
 * 🚫 `lib/동의게이트.js` 에 두지 않았다 — 그 파일은 「동의가 **유효한가**」의 정본이고
 *   머리말이 「🚫 세 번째 뜻을 만들지 않는다」로 못박혀 있다. 이건 유효성 판정이 아니라
 *   **판정 밖**임을 적는 값이라 축이 다르다.
 * ⚠ `goal.responded` 는 학생 산출이라 게이트를 그대로 지난다 — 관찰과 같은 판에 섰다고
 *   같은 계열이 아니다(c14 ③). */

/** 동의 게이트를 지나지 않는 사건 — 학원 운영 기록 계열(계약 c14 ③). */
const 운영기록계열 = ['observation.noted', 'exam.result'];

/** 그 사건들이 `consent_ver` 에 남기는 값. 동의판 형식(`v18.9`)과 일부러 다르다. */
const 운영기록판 = '운영기록';

/**
 * 이 사건이 동의 게이트를 지나야 하나.
 * 🔴 모르는 사건은 **지나야 한다**(`true`) — 새는 방향이 「면제」인 자리는 좁게 연다.
 *   계약에 사건을 늘린 사람이 여기를 안 만지면 게이트가 걸리는 쪽으로 틀린다.
 */
const 동의게이트지나나 = (event_type) => !운영기록계열.includes(event_type);

module.exports = { 사건출처, 출처종류, 운영기록계열, 운영기록판, 동의게이트지나나 };

...
function 항목추가(로그, { date, step, status, duration_ms, hesitation_ms, spoke, threshold_db, text, audio, prompt_id, created_at, task_meta, capture_app, correlation_id, replayed_at, 선택, 선택때, compose_meta }) {
  ...
  const 항목 = {
    ...
    text: text || null,
    audio: audio || null,
    // 업로드 성공 후 로컬에 먼저 기록한다. 재전송 때 같은 서버 원본을 가리킨다.
    audio_ref: null,
...
/** 업로드된 서버 원본을 한 항목에 고정한다. 호출자는 반환 로그를 디스크에 쓴 뒤 사건을 보낸다. */
function 업로드기록(로그, id, audio_ref) {
  if (typeof audio_ref !== 'string' || !audio_ref) throw new Error('음성 참조가 없습니다');
  const 항목 = 로그.find((e) => e.id === id);
  if (!항목) throw new Error('업로드를 기록할 항목이 없습니다');
  if (항목.audio_ref && 항목.audio_ref !== audio_ref) throw new Error('기록한 음성 참조는 바꿀 수 없습니다');
  return 로그.map((e) => e.id === id ? { ...e, audio_ref } : e);
}

/** 저장된 동일 항목의 음성 참조만 보존한다. 새 로그에서 빠진 행은 복원하지 않는다. */
function 업로드참조보존(새로그, 저장로그) {
  const 저장항목 = new Map(저장로그.map((e) => [e.id, e]));
  return 새로그.map((e) => {
    const 이전 = 저장항목.get(e.id);
    if (!이전 || !이전.audio_ref) return e;
    if (e.audio_ref && e.audio_ref !== 이전.audio_ref) throw new Error('저장한 음성 참조는 바꿀 수 없습니다');
    return e.audio_ref ? e : { ...e, audio_ref: 이전.audio_ref };
  });
}
 ...
 * @param {object[]} 로그
 * @param {string} id
 * @param {{event_id?: string|null, 오류?: string|null, 끝?: boolean, audio_ref?: string}} 결과
 * @returns {object[]} 새 배열
 */
function 전송기록(로그, id, { event_id = null, 오류 = null, 끝 = false, audio_ref } = {}) {
  const 참조로그 = audio_ref ? 업로드기록(로그, id, audio_ref) : 로그;
  return 참조로그.map((e) =>
...
module.exports = { 흐름id, 다음시도번호, 항목추가, 직렬화, 역직렬화, 학습출석, 업로드기록, 업로드참조보존, 전송기록, 밀린것, 보낼것, 되듣기기록, 되듣기보낼것, 선택기록, 선택보낼것, 배달상태, 압축, 보존경계, 깨진기록안내 };

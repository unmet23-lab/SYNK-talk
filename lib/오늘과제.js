 ...
 *
 * ■ 멱등키는 **결정론적**이다
 *   `submission:{날짜}:{호흡}:{시도}`. 서버는 `(learner_id, idempotency_key)` 로 접으므로
 *   재전송이 중복 행을 만들지 않는다 — 오프라인 재시도가 안전한 것이 이 한 줄에 달렸다.
 *   🔴 uuid 를 쓰면 안 된다: 보내고 응답을 못 받은 재시도가 **매번 새 행**이 된다.
 *
...
function 제출사건(항목, audio_ref) {
  ...
  const 공통 = {
    idempotency_key: `submission:${항목.date}:${항목.step}:${항목.attempt}`,
     ...
     * 없으면 `null` 로 나간다 — 지어내지 않는다(옛 판 항목이 큐에 남아 있을 수 있다). */
    correlation_id: 항목.correlation_id || null,

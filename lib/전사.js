...
function 세그먼트값(본문) {
  ...
  for (const s of 원본) {
    ...
    /* 숫자 변환을 하지 않는다 — null·빈 문자열·boolean은 측정값 0이 아니다.
     * Number(null)을 거치면 미측정 로그확률이 0, 파생 신뢰도가 1로 꾸며진다. */
    const lp = s.avg_logprob;
    ...
    조각들.push({
      ...
      no_speech_prob: Number.isFinite(s.no_speech_prob) ? s.no_speech_prob : null,

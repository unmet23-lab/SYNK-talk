...
const 차원들 = Object.freeze({
  ...
  /* G1 「교수님 멘탈 구하기」 사과 전략 3장(발주_게임모듈.md G1 §4-4 · §6-6 ⑦ 「새 게임이
   * 축을 늘리는 비용은 이 상수의 한 줄이다」). 카드 문구·id 정본은 `lib/게임제출.사과전략`. */
  사과전략: 'g1_apology',
});
...
function 선택payload({ 차원, 보기, 고른것 = null, 전량거절 = false, 시작 = null, 끝 = null, 바꾼횟수 = 0, 사유 = null } = {}) {
  ...
  const payload = {
     ...
     * 🔴 **안 골랐으면 가리킬 자리가 없어 `null` 이고, 이제 계약이 그 `null` 을 받는다**
     *   (2026-08-10 · `이벤트검증.널허용` 에 `payload.position` 등재 — 그전까지 이 칸이 값으로
     *   필수라 `skipped`·`rejected_all` 갈래가 **원리상 계약을 통과하지 못했다**: 택1에 넣어 둔
     *   두 갈래를 같은 계약이 막고 있었다). 가드는 사라지지 않고 자리를 옮겼다 — **골랐다고
     *   적은 행**은 검증기 ⑦ 이 자리까지 대조한다(`options_shown` 의 그 자리와 일치해야 한다).

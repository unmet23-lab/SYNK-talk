...
/* ── 🔴 수집 실패 갈래의 값록 — 정본은 «여기 하나»다 (2026-09-02 · 심문 전건판정 ④) ──
 *
 * 왜 생겼나: 맥박은 `messages_seen` 만 들어서 **0 이 두 뜻**이었다 —
 *   ⓐ아무도 말 안 했다(조용한 방송)  ⓑ봇이 API 에 못 붙었다(수집 실패).
 * 그리고 더 나빴다: 실패한 회차는 맥박을 **아예 안 남겼다.** 그래서 실패는 표의 «구멍»으로만
 * 드러나는데, 구멍은 「폴링 한 번 실패」와 「봇이 죽어 있던 몇 시간」을 못 가른다.
 * 놓친 채팅은 원장에 복구 재료가 0 이라 **첫 송출 뒤에는 되물을 수 없다.**
 *
 * ⇒ 이제 **모든 회차가 맥박을 남긴다.** `ok=false` 인 행이 곧 「그 순간을 못 걷었다」의 증거다.
 * 🚫 DDL 에 CHECK 를 안 건다(계약 밖 어휘를 DDL 에 안 만든다 — `command_kind` 선례).
 *    값이 늘면 여기만 고친다. */
const 수집실패갈래 = Object.freeze([
  'api_error',      // liveChatMessages 가 2xx 가 아니다(재발견·토큰 갈래를 뺀 나머지)
  'chat_ended',     // 403·404 — 채팅방이 끝났다(재발견으로 돌아간다)
  'token_expired',  // 401 — OAuth 7일 만료
  'no_broadcast',   // 활성 방송을 못 찾았다(방송이 없는 것도 사실이다)
  'send_failed',    // 걷긴 했는데 인제스트로 못 보냈다
]);
const 실패갈래인가 = (v) => 수집실패갈래.includes(v);

/** HTTP 상태 → 실패 갈래 이름. 모르는 상태는 지어내지 않고 `api_error` 로 접는다. */
function 실패갈래(status) {
  if (토큰갱신필요(status)) return 'token_expired';
  if (재발견필요(status)) return 'chat_ended';
  return 'api_error';
}

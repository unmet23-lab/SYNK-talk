...
function 게임배정(재료) {
   ...
   * ⚠ 지금 이 갈래로 오는 것은 **G1 뿐**이다: `functions/deliver` 의 H3 조인이 원 제출을
   *   `challenge_id = 게임챌린지`(G1)로 한정한다. G2 재제출을 세우려면 그 SQL 이 먼저 넓어져야
   *   하고, 그건 Fn 재배포가 붙는 별도 트랙이다 — 여기서 G2 를 지레 받아 두면 「섰다」로 보이는데
   *   조인이 0건이라 영원히 안 도는 코드가 된다. */
  if (재제출 && 재제출.원사건) {

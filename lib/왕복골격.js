 ...
 * @param {string[]} [opt.연성함수목록] «선택 갈래»(플래그)에서만 부르는 함수들 — 낡아도 여기서 죽지 않고
 *        반환 손잡이 `배포판낡음`(slug → 상세)으로 돌아온다. 호출자는 그 갈래의 검증점을 «못쟀다»로
 *        접는다(초록을 낼 길이 없다). 비면 대조 자체를 안 해 행동이 옛 판과 같다(관통왕복 `--아니야`).
 */
async function 열기(도구, { 운영승인가능 = false, 키 = true, 사유 = '이 시험은 지울 수 없는 행을 남긴다', 함수목록 = null, 연성함수목록 = null } = {}) {
  ...
  // 연성 목록은 죽이지 않고 «낡음»을 돌려준다 — 그 갈래는 호출자가 «못쟀다»로 접는다(머리 @param).
  const 연성 = Array.isArray(연성함수목록) && 연성함수목록.length ? { 연성목록: 연성함수목록 } : {};
  const 게이트 = await require('../tools/배포대조.js').왕복전게이트(도구, e, { ...(함수목록 ? { 목록: 함수목록 } : {}), ...연성 });
  const 배포판낡음 = Object.freeze({ ...((게이트 && 게이트.연성낡음) || {}) });
  const 배포판미측정 = Object.freeze({ ...((게이트 && 게이트.연성미측정) || {}) });
  ...
  const sql = async (q) => {
    /* 태그드 템플릿(postgres.js 풍)으로 부르면 q 가 배열로 와서 API 400 이 「질의가 이상하다」로만
     * 보인다 — 라디오왕복시험이 첫 실측에서 정확히 이 모양으로 죽었다(2026-08-13). 여기서 가른다. */
    if (typeof q !== 'string') throw new Error('골격.sql 은 문자열 하나를 받는다 — 태그드 템플릿(sql`…`)이 아니라 sql(`…`) 로 부른다');
    const r = await 실행(q);
  ...
  return { e, ref, 이름, 리허설이다, M, sql, 실행, anon, service_role, 확인, 치명확인, 보고, 배포판낡음, 배포판미측정 };
}

...
const 생산자 = {
   ...
   * ⚠ 아직 한 자리다: ①듣기 재생 완료(=`intervention.delivered` 의 관측 짝 · ①-12 와 ①-2 의
   *   재생 축). 남은 두 자리는 **자기 목소리 되듣기**(parent = 자기 `submission.created` — 앱이
   *   `lib/제출로그.js` 에 이미 그 `event_id` 를 들고 있다)와 **교정 열람 밖의 개입 열람**이다.
   *   이 장부는 파일 지목까지만 지므로 그 둘의 부재는 여기서 안 빨개진다 — 절단문서 ①-2 가 진다. */

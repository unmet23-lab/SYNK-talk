...
const 생산자 = {
  ...
  'content.viewed': { 사유: 'c9 로 이름은 섰고 생산자는 아직 0 — 낼 화면 셋(재생 완료·자기 목소리 되듣기·개입 열람)이 `parent_event_id` 로 쓸 대상 사건 id 를 아직 손에 안 들고 있다(앱이 받는 과제 payload 에 `intervention.delivered` 의 event_id 가 실려야 한다). 🔴 여기 손실 시계는 **개원**이다 — 그 전에는 학생이 0이라 잃는 행이 0이고, 개원 첫 주부터는 「그날 열었는가」가 그날에만 존재한다. 배달 payload 에 id 를 싣는 커밋에서 파일로 바꾼다.' },

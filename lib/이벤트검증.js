...
const 선택필드 = {
  ...
  parent_event_id: '대화 턴이 응답한 선행 턴. 단발 사건에는 가리킬 앞이 없다. 값 규칙은 `retry_of_event_id` 와 같다(①-4).',
...
const 생산자 = {
  ...
  'correction.viewed': { 사유: '🔴 결과축 — S1-8(교정 열람 화면) 미구현. 손실은 학생이 교정을 처음 보는 날 시작되고 소급 불가(그때 봤는지는 그때만 안다). 화면 첫 커밋을 UI 가 아니라 이 사건 발화+실왕복으로 시작할 것(감사 도전안).' },
  'correction.responded': { 사유: '🔴 결과축 — correction.viewed 와 같은 자리(S1-8). learner_response(채택·무시·수정)를 받는 UI 가 곧 생산자다.' },

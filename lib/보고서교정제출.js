 ...
 *   `is_correct`(파생 · C0 §4-1 금지) ·
 *   `changed_selection`(어절 탭은 마지막 것만 남아 값이 «항상 거짓»이 된다 — 거짓 신호를
 *   만드느니 안 보낸다).
 *   ⚠ `confidence` 는 08-22 부터 **짚음 제출에만 · 학생이 표기했을 때만** 싣는다(Ⅲ⑦ · 유호
 *   확정 「하루 1회 예산 · 둘 다 착수」 — 옛 「안 묻는다 · 질문 총량 상한」 거절은 그 판정으로
 *   걷혔다). 값·규율은 라디오 기호식 그대로: 'low'(?)·'guess'(??) · 안 물린 표기는 키 자체를
 *   안 싣는다. 무산출(고칠 곳 없음·건너뜀)은 그대로 안 묻는다 — 회피·판정 자체가 신호다.
 *
 * ■ 🔴 이 파일은 ESM 이다 — 앱 번들 쪽이라 동봉 대상이 아니다(`lib/게임제출.js` 와 같은 자리).
 */

import { 검수확정, 앉음편성 } from '../contents/보고서교정문항.js';
 ...
 *
 * 🔑 `문항id` 는 배정 스냅샷의 `prompt_seed` 다 — G1(`lib/게임제출.js`)과 **같은 칸**이다.
 *   ⚠ 1단계는 이 열쇠를 「봉투 «밖» 칸(`항목.prompt_seed`)으로 받는다」로 닫았는데, 3단계에서
 *   그 칸을 실측하니 통로 어디에도 없었다: `functions/deliver` 는 `submissions` 에 `task_snapshot`
 *   만 싣고 `functions/tasks` 는 허용 목록만 되돌려 준다. 그대로 두면 이 함수가 **영원히 null**
 *   이고 증상은 오류가 아니라 조용한 말하기 폴백이다 — 근거·되돌린 절차는 `lib/게임스냅샷.js`
 *   머리말(`prompt_seed` 는 «공개»다)에 한 벌로 적혀 있다.
...
export function 게임재료(항목) {
  ...
  const 문항id = String((항목.task_snapshot && 항목.task_snapshot.prompt_seed) || '');
  if (문항id === '') return null;
...
/**
 * 한 앉음(3턴)의 재료 — 앵커 재료 하나에서 **화면에 뜰 순서 그대로** 편다.
 *
 * 🔴 왜 화면이 아니라 여기인가 — 편성(`팩.앉음편성`)과 스냅샷 조립(`G2스냅샷`)을 화면이 직접
 *   부르면 「그날 학생이 본 것」의 조립이 두 곳이 되고, 갈라지는 날 **행에 적힌 문항과 화면에
 *   뜬 문항이 다르다**(증상 없음). 화면은 이 배열을 그리기만 한다.
 * 🔑 한 벌이 아니면 `null` — 두 문항짜리 앉음을 내지 않는다(팩 `앉음편성` 머리말: 분모가 날마다
 *   달라지느니 그날 게임을 접고 말하기가 나간다).
 * 🔴 `retry_of_event_id` 는 **앵커 턴만** 진다 — 서버가 「이 사건의 재제출」로 배정한 것은 그
 *   문항 하나다. 셋 다 지면 나머지 둘이 남의 사건의 재제출로 적히고, 그 사슬은 결과 변수(설계
 *   전체에서 유일한)라 거짓이 그대로 학습된다.
 *
 * @param {object} 재료 `게임재료()` 의 결과(앵커)
 * @returns {readonly object[]|null} 3벌. 각 벌은 `게임재료()` 와 **같은 키 모양**이다.
 */
export function 앉음재료(재료) {
  if (검수확정 !== true) return null; // 게이트는 통로마다 다시 본다(fail-closed · `게임재료` 와 한 축)
  if (!재료 || !재료.문항id || !재료.task_ref) return null;
  const 순서 = 앉음편성(재료.문항id);
  if (!순서) return null;
  const 벌 = [];
  for (const 문항id of 순서) {
    const 스냅샷 = G2스냅샷(문항id);
    if (!스냅샷) return null; // 반쪽 앉음을 내지 않는다
    벌.push(Object.freeze({
      ...재료,
      문항id,
      스냅샷,
      보기: 스냅샷.보기,
      retry_of_event_id: 문항id === 재료.문항id ? 재료.retry_of_event_id : null,
    }));
  }
...
export function 짚음제출사건(재료, 입력 = {}) {
  ...
  /* Ⅲ⑦ — 머리말 「안 싣는 것」의 confidence 갱신 참조. 값 검증은 조립이 진다(모르는 값을
   * 실으면 그 축이 거짓이 된다 — 라디오 파서가 ?·?? 만 내는 것과 같은 자리). */
  if (확신도 === 'low' || 확신도 === 'guess') payload.confidence = 확신도;

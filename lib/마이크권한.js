...
/* 문장의 정본은 오류 문구 표다(번역이 착지할 자리 · D1-2) — 여기 리터럴을 다시 적으면
 * 몽골어가 붙는 날 이 자리만 한국어로 남는다. 둘 다 순수 CJS 라 node:test 가 그대로 돈다. */
const { 말 } = require('../contents/문구_오류.js');

/** 거절 문구는 「무엇이 막혔는지 + 어디서 푸는지」를 함께 말한다. */
const 거절_메시지 = 말('err.mic_denied');
...
async function 마이크준비({ 권한요청, 오디오모드 }) {
  ...
  try {
    ...
    return { ok: false, 메시지: 말('err.mic_check_failed') + ': ' + 사유(e) };
  }
  ...
  try {
    ...
    return { ok: false, 메시지: 말('err.audio_setup_failed') + ': ' + 사유(e) };
  }
  return { ok: true };
}

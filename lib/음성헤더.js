...
/**
 * `capture_meta.server` → L0 `submissions.audio_duration_sec` (numeric(6,2)).
 *
 * 🔴 **잰 값이 `capture_meta` 안에만 있으면 「몇 초 말했나」는 집계가 안 된다.** 검수 큐도
 *   게임의 「발화 길이」도 그 열을 읽는데(`발주_게임모듈` 「서버가 헤더에서 파생」), 열이 비면
 *   평균·분포가 **전량 0** 으로 나오고 그건 「말을 안 했다」와 같은 모양이다. 수집은 잰 자리에
 *   두는 것으로 끝나지 않는다 — 읽는 자리에 닿아야 한 벌이다.
 * 🔑 못 잰 것은 **null** 이다(이 파일 머리말 「모르는 것을 안다고 적지 않는다」) — 0 으로 적으면
 *   측정 실패가 0.0초짜리 발화라는 **거짓 관측**이 되어 평균을 영구히 끌어내린다.
 * ⚠ `numeric(6,2)` 밖의 값은 insert 를 통째로 던지고, 그러면 그 사건은 `CONTRACT_VIOLATION`
 *   `retryable:false` 로 접혀 **발화가 영구 소멸한다**. 원본 밀리초는 `capture_meta` 에 그대로
 *   남으니 여기서는 비운다 — 열 하나 때문에 발화를 잃지 않는다.
 */
function 길이초(server) {
  const ms = server && typeof server === 'object' ? server.duration_ms : null;
  if (typeof ms !== 'number' || !Number.isFinite(ms) || ms < 0) return null;
  const 초 = Math.round(ms / 10) / 100;
  return 초 > 9999.99 ? null : 초;
}

/**
 * Storage 응답이 「그 객체가 없다」인가.
 *
 * 🔴 **실측(2026-08-07 리허설): 없는 객체는 404 가 아니라 `400` 으로 온다** — 404 는 본문 안에만
 *   있다: `{"statusCode":"404","error":"not_found","message":"Object not found","code":"NoSuchKey"}`.
 *   상태 코드만 보면 「파일 없음」이 「일시 실패」에 섞이고, 그러면 **파일 없는 행을 ingest 시점에
 *   못 박는** 능력이 통째로 사라진다(그 행은 나중에 「전사 실패」와 구분되지 않는다).
 * 🔑 판정이 여기 있는 이유: `functions/events` 안에 두면 **리허설을 쏴야만** 드러나고, 그건
 *   회귀가 못 지키는 자리다. 순수 함수로 내려 CI 가 잡게 한다.
 * ⚠ 400 밖은 「없다」로 접지 않는다 — 401·403·5xx 를 「없음」으로 적으면 **살아 있는 파일을
 *   사라졌다고 기록**하게 된다(반대 방향의 거짓말이고, 이쪽이 더 비싸다).
 */
function 파일없음(status, 본문) {
  if (status === 404) return true;
  if (status !== 400) return false;
  return /"statusCode"\s*:\s*"404"|"code"\s*:\s*"NoSuchKey"|"error"\s*:\s*"not_found"/.test(String(본문 || ''));
}

module.exports = { 정본, 헤더읽기, 길이초, 파일없음 };

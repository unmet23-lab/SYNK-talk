 ...
 *   정본. `policy_ver` 재료 1은 이 파일과 로컬 전이 의존성의 묶음 해시다(§5-1 · 09-11) —
 *   성향 지표 매핑·나침반 맥락만 바뀌어도 판이 오른다.
 ...
 *   · 부정 제외(철학 적용기준 「관측과 성향」 · 2026-09-11) — 부정한 (축·키)에 대응하는
 *     지표만 생성 입력에서 뺀다. 같은 축의 다른 관측·확인은 유지한다. 원관측과 확인축은
 *     바꾸지 않는다. 카드 의미·지표 경로는 lib/성향확인.js `부정지표들`이 정한다.
 *     남은 지표가 있는 축은 axes_used 에 계속 센다. 제외키·제외지표·미처리부정키를 반환하며,
 *     제외축은 지표 제외로 축 줄 전체가 비었을 때만 든다. 부정이 없으면 요약 바이트는 같다.
 *
...
const { 부정지표들 } = require('./성향확인.js');          // 카드 의미와 지표 연결의 한 원천
 ...
 *          왜배우나?: string|null, 토픽쓸곳?: string|null, 부정키들?: string[]|null}} [조각]
 *   goal_snapshot·level_snapshot·나침반 `season_goal`(㉢ — 원값을 받는다 · 렌더는 시즌줄이 진다) ·
 *   나침반 입학 회차 `why_learning`·`topik_use`(09-05 — 원값 · 렌더는 목적줄들이 진다) ·
 *   `부정키들`(「아니야」 받은 (축·키) — 활동 창과 별도로 읽은 `성향확인.부정키들(부정이력)` 산출 · 없으면 제외 0)
 * @returns {{요약: string, 쓸축수: number, axes_used: string[], 제외축: string[],
 *            제외키: string[], 제외지표: string[], 미처리부정키: string[], evidence_refs: object}}
 */
function 과제요약(상태, 조각) {
  if (!상태 || typeof 상태 !== 'object' || !상태.축 || typeof 상태.축 !== 'object') {
    throw new TypeError('과제요약: 학습자상태() 산출(축 객체 포함)이 필요하다');
  }
  const { 목표 = null, 급수 = null, 시즌목표 = null, 왜배우나 = null, 토픽쓸곳 = null, 부정키들 = null } = 조각 || {};
  const 부정 = 부정지표들(상태.축, 부정키들);
  const 제외 = new Set(부정.제외지표);
  /* 렌더용 복사만 만든다. 중첩 시간대 하나를 빼더라도 원관측·다른 띠·분모는 그대로다. */
  function 남긴값(경로, 값) {
    if (제외.has(경로)) return undefined;
    if (!값 || typeof 값 !== 'object') return 값;
    return Object.fromEntries(Object.entries(값)
      .map(([k, v]) => [k, 남긴값(`${경로}.${k}`, v)])
  ...
  for (const [축이름, 값] of Object.entries(상태.축)) {   // 순서 = 상태 객체 그대로(재정렬 금지)
    ...
    const 앞 = Object.entries(값).map(([k, v]) => 지표렌더(k, v)).filter(Boolean);
    if (!앞.length) continue;
    const 안 = Object.entries(남긴값(축이름, 값)).map(([k, v]) => 지표렌더(k, v)).filter(Boolean);
    if (!안.length) { 제외축.push(축이름); continue; }
    axes_used.push(축이름);

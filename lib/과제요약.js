 ...
 *   · 부정 제외(철학 적용기준 「관측과 성향」 · 09-05 「아니야」= 빼기 · 2026-09-11) — 학생이 맞아? 카드에
 *     「아니야」로 부정한 추정의 «축»은 싣지 않고 `axes_used` 에도 안 센다(짐작의 재료를 벤더에게 다시
 *     주지 않는다 — 한 키만 부정돼도 그 축 줄 전체가 빠진다). 원관측(행)·정정 이력(estimate.responded ·
 *     학습자상태 확인축)은 그대로다 — 여기는 «생성 입력»만 가른다. 부정 키의 형식·추출·축 풀기는
 *     lib/성향확인.js 한 원천(`부정키들`·`부정축들`)이고 이 파일은 축 이름만 안다. 조각 `부정키들` 이
 *     비면 산출은 한 글자도 안 바뀐다. 뺀 축은 `제외축` 으로 되돌려 준다(로그·회귀 재료 · 실릴 축만 센다).
 *
...
const { 부정축들 } = require('./성향확인.js');            // 「아니야」 키의 형식·축 풀기는 그 파일 하나(progress 와 공유)
 ...
 *   `부정키들`(「아니야」 받은 (축·키) — `성향확인.부정키들(원신호)` 산출 그대로 · 없으면 제외 0)
 * @returns {{요약: string, 쓸축수: number, axes_used: string[], 제외축: string[], evidence_refs: object}}
 */
function 과제요약(상태, 조각) {
  ...
  /* 부정 제외(머리말) — 축 이름 집합만 안다. 형식(`축:키`)을 여기서 다시 풀지 않는다(한 원천). */
  const 제외 = new Set(부정축들(Array.isArray(부정키들) ? 부정키들 : []));
  ...
  for (const [축이름, 값] of Object.entries(상태.축)) {   // 순서 = 상태 객체 그대로(재정렬 금지)
    ...
    if (제외.has(축이름)) { 제외축.push(축이름); continue; } // 「아니야」 축 — 실릴 축만 「뺐다」로 센다(빈 축은 뺀 게 아니다)
    axes_used.push(축이름);
  ...
  return {
    ...
    axes_used,
    제외축,                                   // 「아니야」로 뺀 축(머리말 부정 제외) — 부정이 없으면 []
    evidence_refs: {

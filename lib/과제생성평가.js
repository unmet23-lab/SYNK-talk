...
const 통과선 = Object.freeze({              // §8-B 「통과선」 — 축별 각각(합산 금지)
  connect: 0.9, answerable: 0.9, level_fit: 0.9, fresh: 0.8, fun: 0.7, accuracy: 0.95, goal_use: 0.7, state_use: 0.7,
});
...
function 결과검증(결과, 시험지, 전문) {
  ...
  행들.forEach((행, i) => {
    ...
    if (!s || typeof s !== 'object' || Array.isArray(s)) 사유.push(`행 ${i}: axis_scores 객체 아님`);
      ...
      if (축키들.some((k) => s[k] === 0) && !(typeof 행.grader_note === 'string' && 행.grader_note.trim())) sayNote(i);
    }
...
function 집계(결과, 시험지) {
  ...
  for (const c of 시험지.사례) {
    ...
    for (const k of 축키들) {
      ...
      const 점 = (v1 === 1 && v2 === 1) ? 1 : 0;                                  // 둘 다 통과해야 1(평균 아님)
      축[k].합 += 점; 축[k].분모 += 1;
  ...
  for (const k of 축키들) {
    ...
    축[k].통과 = 축[k].비율 !== null && 축[k].비율 >= 통과선[k];
  }
...
module.exports = {
  총사례, 칸당최소, 회차수, 급수들, 상태갈래들, 축키들, 통과선, null허용축, 셀판정축, 사람축,
  행키들, 비교축, 존재축, 기록축, 채점표판, 시험지판, 앵커, 칸키,
  배분, 시험지조립, 사례본문, input_hash, 응답해시, case_id, 결과검증, 집계, 비교축차이,
};

...
const 채점표판 = '채점표.v3';   // v3 = ⑤재미 앵커 증보(Z4·Z5·A4 — 유호 3회전 채점 실물 08-25) · v2 = ⑦ 재정의(사실 판정 + 절제 구간 · 유호 확정 08-25)
const 시험지판 = '시험지.v1';
 ...
 *
 * 🔴 v3 증보(2026-08-25 유호 3회전 채점 · 축은 안 늘리고 앵커만 또렷하게) — 왜 셋을 더하나:
 *   3회전 흠 24건 중 **질문 쪽이 13건**(질문만 7 + 문장·질문 둘 다 6)인데, Z1~Z3·A1~A3 는
 *   그 13건의 무늬를 하나도 안 담고 있었다. Z1 은 「예/아니오·사실 확인」만 막아서
 *   「하루 중에 몸을 움직이는 시간이 있어요?」(누구나 그렇다)를 못 걸렀고, 학생을 몰아세우는
 *   질문(「왜 그게 아까운 줄 모르고 사요?」)은 어느 칸에도 없었다. 앵커가 못 보는 것을
 *   채점자도 못 본다 — 축을 늘리는 대신 «자»를 3회전 실물로 갈아 끼운다.
 *   ⚠ 채점 손은 안 늘어난다(축 여덟 그대로) — 8축을 유지한 채 ⑥ 정조준을 흐리지 않으려는 판정.
 *      「성향 파악」·「자기성찰」 축 신설은 ⑥이 선을 넘은 «다음»(5회전 예약 · 트랙 §8-B). */
const 앵커 = Object.freeze({
  ...
  '0점조건': Object.freeze([
    ...
    /* Z4·Z5 = 유호 3회전 실물. Z1 의 「예/아니오」와 다르다 — 형식은 열려 있는데 «답이 하나»다. */
    { anchor_id: 'Z4', 조건: 'ⓓ 누구나 같은 답을 하는 질문 — 형식은 열려 있어도 물으나 마나다', sentence: '저녁을 먹고 나서 동네를 한 바퀴 걸었어요.', question: '하루 중에 몸을 움직이는 시간이 있어요?' },
    { anchor_id: 'Z5', 조건: 'ⓔ 학생의 선택에 이유를 요구하는 질문 — 대화가 아니라 취조가 된다', sentence: '지갑에 천 원밖에 없어서 우유만 하나 샀어요.', question: '왜 그게 아까운 줄 모르고 사요?' },
  ]),
  '1점앵커': Object.freeze([
    ...
    /* A4 = 유호님이 「꼭 기억해줘」로 못박은 자리(08-25 P084#1 채점 메모). 원문 표기 「라던지」는
     * 이 칸이 채점자에게 보이는 «본보기»라 「라든지」로 바로잡아 실었다 — 뜻은 그대로다. */
    { anchor_id: 'A4', 기준: 'ⓓ 10대 후반에게 «비밀친구»처럼 — 가벼운 예시를 던져 웃으며 답하게 하는 질문', sentence: '저는 아침에 일어나면 제일 먼저 휴대폰을 봐요.', question: '요즘 아침에 일어나자마자 하는 일이 있어요? 여자친구한테 연락하기라든지, 비밀인가요?' },
  ]),
...
function 사례본문(전문, 사례) {
  ...
  if (본문 == null) throw new Error('사례본문: 프롬프트 템플릿 펜스 또는 학생 표식(---학생---)을 못 찾았다');

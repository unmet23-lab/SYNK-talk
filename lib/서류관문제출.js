...
export function 변환제출사건(턴재료, 입력 = {}) {
  if (!턴재료 || !턴재료.task_ref || !턴재료.스냅샷) return null;
  if (!키있나(입력)) return null;
  const 문항 = 펴기(턴재료.스냅샷.prompt_seed);
  if (!문항 || 문항.종류 !== '변환') return null; // 빈칸 턴 교차 호출 차단(판정 정본 = 팩)
 ...
 * 🔑 `task_type` 은 **`변환`(숙제제출)** 을 쓴다 — G2 이탈 선례(그 모듈의 열린 산출 통로
 *   이름): 이탈은 판정 없이 나간 것이라 퀴즈 갈래의 뜻(답을 냈다/모른다고 했다)과 다르다.
...
export function 이탈사건(재료, 입력 = {}) {
  ...
  return {
    ...
    submission: { task_ref: 재료.task_ref, task_format: 이탈형식 },
    payload: { ver: 1 },
  };
}

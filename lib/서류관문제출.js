 ...
 *
 * ■ 🚫 안 싣는 것
 *   `confidence` 는 **빈칸 제출에만 · 학생이 표기했을 때만** 싣는다(Ⅲ⑦ · 유호 확정 08-22 「하루
 *   1회 예산 · 둘 다 착수」 — 라디오 기호식 `?`(low)·`??`(guess)과 같은 값 · 안 물린 표기는 안
 *   싣는다 · 모름 제출은 회피 자체가 신호라 그대로 안 묻는다) ·
...
export function 빈칸제출사건(턴재료, 입력 = {}) {
  ...
  const { 본문, attempt_no, latency_ms = null, 확신도 = null } = 입력;
  if (typeof 본문 !== 'string' || 본문.trim() === '') return null; // 🔴 ⑥ 구멍 — 이 층이 막는다
  if (!Number.isInteger(attempt_no) || attempt_no < 1) return null;
  /* Ⅲ⑦(유호 확정 08-22) — 확신도는 라디오 승격기와 **같은 값·같은 규율**이다: 'low'(?)·
   * 'guess'(??) 둘뿐이고 안 물린 표기는 키 자체를 안 싣는다(null 을 실으면 「안 물렸다」와
   * 「물었는데 무표기」가 행에서 같은 모양이 된다 — lib/라디오승격.js :237 그대로).
   * 소비자는 이미 서 있다 — 자기인식축이 quiz.answered 전부의 payload.confidence 를 센다. */
  const 표기 = 확신도 === 'low' || 확신도 === 'guess' ? { confidence: 확신도 } : {};
  return {
    ...봉투(턴재료, 입력, G4과제유형.빈칸),
    ...
    payload: { ver: 1, attempt_no, ...지연칸(latency_ms), ...표기 },
  };
}

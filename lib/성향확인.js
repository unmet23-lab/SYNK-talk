 ...
 *
 * ■ 네 소비자가 이 한 파일을 쓴다 — 판정이 두 벌이 되면 서버가 보여준 것과 행에 남는 것이 갈린다.
 ...
 *   · 생성 요약(lib/과제요약.js ← deliver 생성모드) — `부정키들`·`부정지표들`: 「아니야」 받은
 *     그 짐작의 지표만 다음 생성 입력에서 뺀다. 같은 축의 다른 관측·확인까지 지우지 않는다.
 *
 ...
 *   되고, 그 순간 확인은 상호작용이 아니라 설문이 된다(재미없는 설문 금지 · §10). 그리고 **다음
 *   생성에서도 뺀다** — 원관측(행)과 정정 이력(estimate.responded · 확인축)은 그대로 두고, 벤더에게
 *   가는 재료에서 해당 지표만 거른다(빼는 자리는 lib/과제요약.js · 카드 의미와 지표 연결은 여기).

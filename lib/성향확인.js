'use strict';
/* 성향 확인 — 추정 후보 판정과 estimate.responded 사건 조립을 맡는 순수 함수.
 * 기준은 appsscript docs/코어엔진_설계.md §10, 세층합류 설계와 Talk 이벤트 계약 c13이다.
 * progress의 확인카드, 앱의 확인사건, 학습자상태의 확인축, 과제요약의 부정 필터가 함께 사용한다.
 *
 * 후보 문구는 실제 관측이 뒷받침하는 방향·빈도·뜻에 맞춘다. 추정은 학생의 명시적 답과 구분하며,
 * 규칙을 발전시킬 때 판을 올려 어떤 추정에 답했는지 사건에서 식별할 수 있게 한다.
 * 부정된 (축·키)는 다시 묻거나 생성 재료에 쓰지 않는다. 원관측과 정정 이력은 보존하고,
 * 같은 축의 다른 근거까지 지우지 않는다. 하루 상한은 아래 하루상한 상수를 따른다.
 */

const { 확인안내 } = require('../contents/문구_성향확인.js');

/** 이 «후보 판정»의 판 — 규칙이 바뀌면 올린다. 사건에 실리는 `estimator_version` 은 추정을 낸
 *  학습자상태 판과 이 판을 `+` 로 합친다(둘 중 하나만 적으면 나머지 반쪽의 판올림이 행에서 안 갈린다). */
const 판정판 = '성향확인.v2';
/** 하루 «두 장»(철학 Ⅱ-8 v1.22 · 엔진 v3 D7 · 09-06) — 서버(progress)와 회귀가 이 수 하나를 읽는다. 옛 판은 1 이었다. */
const 하루상한 = 2;

/** 판정 우선순위 — 앞이 먼저다(더 구체적인 관찰 먼저: 리듬(제출 습관) → 작성과정(쓰는 손) →
 *  집중띠(시간대)). (축·키) 쌍의 «축» 순서 정본이고, 키의 소비자는 `contents/문구_성향확인.js`. */
const 후보순서 = Object.freeze(['여유제출', '반복제출', '고쳐쓰기', '한번에쓰기',
  '주로새벽', '주로오전', '주로오후', '주로저녁']);

/** 부정 재노출 금지 키의 «형식» 한 원천 (심문 G13 — SQL 조립과 lib 리터럴 두 곳이던 것).
 *  서버(progress)는 SQL 이 (trait_axis, shown_key) 쌍을 내고 TS 층이 이 함수로 조립한다 —
 *  SQL 문자열 안에 구분자를 다시 적지 않는다(두 곳에 적힌 형식은 새 축이 서는 날 갈린다). */
const 부정키 = (축, 키) => `${축}:${키}`;

/** `부정키` 의 역 — `'리듬:여유제출'` → `{ 축: '리듬', 키: '여유제출' }`. 형식의 원천이 위 한 줄이라
 *  푸는 쪽도 **이 파일**에 산다(다른 파일이 `split(':')` 을 적으면 구분자가 두 곳이 된다 — G13 그대로).
 *  규격 밖(구분자 없음 · 빈 마디)은 null — 조용히 빈 축으로 접지 않는다. */
function 부정키풀기(키) {
  const s = String(키 ?? '');
  const i = s.indexOf(':');
  if (i <= 0 || i === s.length - 1) return null;
  return { 축: s.slice(0, i), 키: s.slice(i + 1) };
}

/**
 * `estimate.responded` 행들 → 부정된 (축·키) 키 목록(«아니다» 받은 것 전부 · 첫 등장 순 · 중복 없음).
 * 🔴 서버(progress)의 SQL 필터 `response = '아니다'` 와 **같은 규칙**이다 — 한 번이라도 「아니에요」
 *   받은 (축·키)는 다시 안 묻고(머리말) 다음 생성에서도 뺀다(철학 적용기준 「관측과 성향」). 학습자상태
 *   확인축의 «키별 마지막 답» 접기는 라벨 셈의 규칙이지 재노출·제외 규칙이 아니다. 이 함수가
 *   재확인으로 부정을 해제하는 새 정책을 만들지는 않는다.
 * 🔑 생성 호출자는 활동 창과 별도로 스냅 기준까지의 부정 이력을 걷는다. 여기서 기간·건수로
 *   다시 자르지 않는다. 다른 event_type 행이 섞여 있어도 된다(골라 읽는다).
 * @param {Array<object>} 행들 learning_events 행(event_type · payload 가 있는 모양)
 * @returns {string[]} `부정키(축, 키)` 문자열 목록
 */
function 부정키들(행들) {
  const 나옴 = [];
  const 본 = new Set();
  for (const e of Array.isArray(행들) ? 행들 : []) {
    if (!e || e.event_type !== 'estimate.responded') continue;
    const p = e.payload && typeof e.payload === 'object' ? e.payload : null;
    if (!p || p.response !== '아니다') continue;                       // 「맞다」·모르는 값은 부정이 아니다
    if (typeof p.trait_axis !== 'string' || !p.trait_axis) continue;   // 반쪽 행은 안 센다 — 지어내지 않는다
    if (typeof p.shown_key !== 'string' || !p.shown_key) continue;
    const k = 부정키(p.trait_axis, p.shown_key);
    if (본.has(k)) continue;
    본.add(k);
    나옴.push(k);
  }
  return 나옴;
}

/** 카드가 확인한 주장에 대응하는 지표. 표본 수와 다른 관측은 그대로 둔다.
 *  작성과정의 두 키는 같은 지표의 반대 방향이므로 현재 값이 반대 방향이면 보존한다.
 *  집중띠도 해당 시간대가 과반이 아니라고 확인되면 보존한다. 남은 시간대의 분모·비율을 재계산하면
 *  부분 노출이 다른 생활 습관인 것처럼 보이므로 원래 n·띠수·완주율은 바꾸지 않는다.
 *  키를 모르면 임의로 축 전체를 지우지 않고 미처리로 반환한다. */
const 확인지표 = Object.freeze({
  '리듬:여유제출': '리듬.마감여유분_중앙',
  '리듬:반복제출': '리듬.제출률',
  '작성과정:고쳐쓰기': '작성과정.되돌림_중앙',
  '작성과정:한번에쓰기': '작성과정.되돌림_중앙',
  '집중띠:주로새벽': '집중띠.띠.새벽',
  '집중띠:주로오전': '집중띠.띠.오전',
  '집중띠:주로오후': '집중띠.띠.오후',
  '집중띠:주로저녁': '집중띠.띠.저녁',
});

function 부정지표들(축들, 키들) {
  const 제외키 = [], 제외지표 = [], 미처리부정키 = [];
  for (const k of new Set(Array.isArray(키들) ? 키들.map(String) : [])) {
    if (!Object.prototype.hasOwnProperty.call(확인지표, k)) { 미처리부정키.push(k); continue; }
    const { 축, 키 } = 부정키풀기(k);
    const 값 = 축들 && 축들[축];
    if (!값 || typeof 값 !== 'object') continue;
    const 지표 = 확인지표[k].split('.').slice(1).reduce((v, p) => v && v[p], 값);
    if (지표 === null || 지표 === undefined) continue;
    /* 복수 표본은 새 카드를 묻는 조건이다. 표본이 한 개로 줄었다는 이유로 과거 부정을
     * 풀지 않는다. 여기서 보존하는 것은 지표 값이 부정한 주장과 다른 방향인 경우뿐이다. */
    if (키 === '여유제출' && Number.isFinite(지표) && 지표 <= 0) continue;
    if (키 === '반복제출' && Number.isFinite(지표) && 지표 !== 1) continue;
    if (키 === '고쳐쓰기' && Number.isFinite(지표) && 지표 < 1) continue;
    if (키 === '한번에쓰기' && Number.isFinite(지표) && 지표 !== 0) continue;
    if (축 === '집중띠' && Number.isFinite(지표.n) && Number.isFinite(값.n)
      && 값.n > 0 && 지표.n * 2 <= 값.n) continue;
    제외키.push(k);
    if (!제외지표.includes(확인지표[k])) 제외지표.push(확인지표[k]);
  }
  return { 제외키, 제외지표, 미처리부정키 };
}

/**
 * 리듬축 산출 → 확인 후보 키들(없으면 [] — 지어내지 않는다).
 * 🔴 재료는 **`학습자상태()` 의 리듬축 산출 그대로**다 — 배정↔제출 구간 잇기·마감 여유 계산의
 *   정본이 그 함수 하나이고, 여기서 다시 재면 서버가 보여준 추정과 상태가 배운 추정이 갈린다.
 * @param {{제출률: number, 마감여유분_중앙: number|null, n: number, 여유n: number}|null} 리듬
 */
function 리듬후보들(리듬) {
  if (!리듬 || typeof 리듬 !== 'object') return [];
  const 후보 = [];
  // 방향 — 마감 «전»(중앙값 양수). 복수 어의: 「요즘 ~들」이 성립하려면 여유 표본이 둘은 있어야 한다.
  if (Number.isFinite(리듬.마감여유분_중앙) && 리듬.마감여유분_중앙 > 0
    && Number.isFinite(리듬.여유n) && 리듬.여유n >= 2) 후보.push('여유제출');
  // 등식 — 빠짐없이(제출률 1). 같은 복수 어의로 배정이 둘은 있어야 「빠짐없이」가 말이 된다.
  if (리듬.제출률 === 1 && Number.isFinite(리듬.n) && 리듬.n >= 2) 후보.push('반복제출');
  return 후보;   // 성립하는 것 전부 · 우선순위 순 — 부정 건너뛰기가 다음 후보를 쓴다
}

/** 첫 후보 하나(없으면 null) — 후보 «목록»의 정본은 위 함수다. */
const 리듬후보 = (리듬) => 리듬후보들(리듬)[0] ?? null;

/**
 * 작성과정축 산출 → 확인 후보 키들 (v2 · 세층합류 §5).
 * 🔴 재료는 `학습자상태()` 작성과정축 그대로 — compose_meta 온전 행의 중앙값 넷.
 *   되돌림_중앙의 **방향**(≥1 = 절반 이상의 제출에서 한 번은 고쳤다)과 **등식**(0 = 절반 이상이
 *   한 번에)만 본다 — 「몇 번이면 많은가」 같은 품질 문턱은 없다(문턱 금지 규율 그대로).
 * @param {{되돌림_중앙: number|null, n: number}|null} 작성과정
 */
function 작성과정후보들(작성과정) {
  if (!작성과정 || typeof 작성과정 !== 'object') return [];
  if (!Number.isFinite(작성과정.n) || 작성과정.n < 2) return [];   // 복수 어의
  const 중앙 = 작성과정.되돌림_중앙;
  if (!Number.isFinite(중앙)) return [];
  if (중앙 >= 1) return ['고쳐쓰기'];       // 방향 — 고치면서 완성하는 편
  if (중앙 === 0) return ['한번에쓰기'];    // 등식 — 한 번에 쭉 쓰는 편
  return [];
}

/** 띠 이름 → 확인 키. 키 공간의 정본은 이 표다(문구 파일이 같은 키를 그린다). */
const 띠키 = Object.freeze({ 새벽: '주로새벽', 오전: '주로오전', 오후: '주로오후', 저녁: '주로저녁' });

/**
 * 집중띠축 산출 → 확인 후보 키들 (v2 · 세층합류 §5).
 * 🔴 재료는 `학습자상태()` 집중띠축 그대로 — 띠별 {완주율, n}.
 *   「주로」의 어의 = **과반**(그 띠의 n 이 전체 n 의 절반을 넘는다 — 절반이면 「주로」가 아니라
 *   「반반」이다). 복수 어의 = 그 띠 n≥2. 완주율은 안 본다 — 「그때 잘되더라」 판정은 품질
 *   문턱이라 금지고, 묻는 것은 「그 시간이 집중이 잘돼요?」라는 **학생의 답**이다.
 * @param {{띠: Record<string,{완주율:number|null,n:number}>, n: number}|null} 집중띠
 */
function 집중띠후보들(집중띠) {
  if (!집중띠 || typeof 집중띠 !== 'object' || !집중띠.띠 || typeof 집중띠.띠 !== 'object') return [];
  if (!Number.isFinite(집중띠.n) || 집중띠.n < 2) return [];
  for (const [이름, t] of Object.entries(집중띠.띠)) {
    if (!띠키[이름] || !t || !Number.isFinite(t.n)) continue;
    if (t.n >= 2 && t.n * 2 > 집중띠.n) return [띠키[이름]];   // 과반 — 있으면 하나뿐이다
  }
  return [];
}

/** 고정 우선순위 — 더 구체적인 관찰 먼저. 🔴 이제 «동률 깨기»로만 쓴다(아래 기아 방지). */
const 축순서 = Object.freeze(['리듬', '작성과정', '집중띠']);
const 축뽑기 = Object.freeze({ 리듬: 리듬후보들, 작성과정: 작성과정후보들, 집중띠: 집중띠후보들 });

/**
 * 축들 산출 → (축, 키) 후보 쌍 전부 · **가장 오래 안 물은 축 먼저**.
 * 축 이름은 학습자상태 축 이름 그대로다(trait_axis 열린 집합 — 계약 개정 0).
 *
 * 🔴 **왜 정렬이 바뀌었나 — 기아**(심문 전건판정 2026-09-02 ㉮ · 이 판정 최대 수확).
 *   고정 우선순위 + `.find()` 첫 후보 + 「부정된 키만 제외」의 셋이 겹쳐, 성실한 학생은
 *   **매일 리듬 카드만** 받았다. '맞다'로 답해도 그 키는 후보에 남으므로(긍정 키 재노출 허용 —
 *   `lib/동의율.js` 가 그 사실을 적어 뒀다) `작성과정`·`집중띠` 는 **한 번도 안 물어진다**.
 *   그러면 §6 동의율 자가 두 채널에서 영원히 n=0 을 내는데, 그 0 은 「학생이 안 답했다」로
 *   읽힌다 — 실제로는 **「한 번도 안 물었다」**다. 안 물은 날의 라벨은 영원히 없다(소급 불가).
 *   ⇒ 축을 «마지막으로 답한 날»로 정렬하고, 고정 우선순위는 동률 깨기로 내린다.
 *   16주 시효(성향추출 v1.0)도 이 정렬이 자연히 지킨다 — 오래된 축이 스스로 앞으로 온다.
 *
 * @param {{리듬?: unknown, 작성과정?: unknown, 집중띠?: unknown}|null} 축들
 * @param {Record<string,string>} [축별마지막날] 축 → 마지막으로 답한 날('YYYY-MM-DD').
 *   **모르면 안 준다** — 빈 값은 「한 번도 안 물음」과 같이 맨 앞이고, 전부 비면 고정 우선순위다
 *   (서버가 이 조회에 실패해도 옛 행동으로 내려앉을 뿐 카드를 잃지 않는다).
 */
function 축후보들(축들, 축별마지막날) {
  const a = 축들 && typeof 축들 === 'object' ? 축들 : {};
  const 날 = 축별마지막날 && typeof 축별마지막날 === 'object' ? 축별마지막날 : {};
  const 언제 = (축) => (typeof 날[축] === 'string' ? 날[축] : '');   // '' < 어떤 ISO 날짜 — 안 물은 축이 맨 앞
  return 축순서
    .map((축, i) => ({ 축, i, 키들: 축뽑기[축](a[축]) }))
    .filter((x) => x.키들.length)
    .sort((x, y) => (언제(x.축) !== 언제(y.축)
      ? (언제(x.축) < 언제(y.축) ? -1 : 1)
      : x.i - y.i))
    .flatMap(({ 축, 키들 }) => 키들.map((키) => ({ 축, 키 })));
}

/**
 * 오늘 보여줄 확인 카드 — 하루 1회 · 부정 재노출 금지 · 문구 없는 키는 안 낸다.
 * v2: 첫 인자가 리듬 산출 «하나»에서 **축들 객체**가 됐다(세층합류 §5 — 확인카드 다축 일반화).
 *   소비자 전량(progress·회귀)이 같은 커밋에서 함께 옮긴다 — 두 시그니처를 겸용으로 받으면
 *   어느 판으로 불렸는지 행에서 안 갈린다.
 * @param {{리듬?: unknown, 작성과정?: unknown, 집중띠?: unknown}|null} 축들
 *   `학습자상태()` 산출의 `축` 그대로(서버가 행을 걷어 그 함수에 먹인다)
 * @param {{오늘답함: boolean, 부정키들: string[], 축별마지막날?: Record<string,string>}} 이력
 *   `estimate.responded` 행에서 파생(서버). `축별마지막날` 은 기아 방지용 — 없으면 고정 우선순위.
 * @param {string} 기준시각  그 상태 계산의 as_of — 사건 `estimate_as_of` 로 그대로 실린다
 * @param {string} 추정판  그 상태의 `estimator_version`(예: 학습자상태.v15)
 * @returns {{trait_axis: string, shown_key: string, shown_text: string,
 *           estimator_version: string, estimate_as_of: string}|null}
 */
function 확인카드(축들, 이력, 기준시각, 추정판) {
  if (!이력) return null;
  /* 하루 «두 장»(철학 Ⅱ-8 v1.22 · 엔진 v3 D7 · 09-06) — 옛 판은 하루 1회였다. 서버는 `오늘답수`(수)를 주고,
   * 옛 호출(`오늘답함` 불리언)은 «상한 도달»로 읽는다 — 옛 호출이 조용히 두 장을 내지 않게. */
  const 오늘수 = typeof 이력.오늘답수 === 'number' ? 이력.오늘답수 : (이력.오늘답함 ? 하루상한 : 0);
  if (오늘수 >= 하루상한) return null;
  const 부정 = new Set(Array.isArray(이력.부정키들) ? 이력.부정키들.map(String) : []);
  /* 오늘 이미 답한 (축·키)는 오늘 다시 안 낸다 — 둘째 장은 «다른» 카드라야 두 장이다. */
  const 오늘답한 = new Set(Array.isArray(이력.오늘답한키들) ? 이력.오늘답한키들.map(String) : []);
  /* 부정 재노출 금지 — 부정된 (축·키)는 «건너뛰고 다음 후보»다(전부 부정이면 카드 없음). */
  const 후보 = 축후보들(축들, 이력.축별마지막날)
    .find(({ 축, 키 }) => !부정.has(부정키(축, 키)) && !오늘답한.has(부정키(축, 키))) ?? null;
  if (!후보) return null;
  const 안 = 확인안내(후보.키);
  if (!안 || !안.문장.length) return null;                    // 문구 없는 키 — 빈 카드를 내지 않는다
  return {
    trait_axis: 후보.축,
    shown_key: 후보.키,
    shown_text: 안.문장.join('\n'),                           // 학생이 실제로 본 문장 그대로 행에 남는다
    estimator_version: `${String(추정판 || '')}+${판정판}`,
    estimate_as_of: String(기준시각),
  };
}

/**
 * 학생 답 → `estimate.responded` 사건 (계약 c13 · 검증기 필수 6 전부).
 * 🔴 카드의 다섯 값을 **그대로 되싣는다** — 앱이 지어내면 「무엇의 긍정/부정인가」가 갈린다
 *   (task_ref 되돌리기와 같은 무늬). 모르는 답 값은 null(보내지 않는다 — 조립기가 값록을 진다).
 * @param {ReturnType<typeof 확인카드>} 카드  서버가 내려준 그대로
 * @param {'맞다'|'아니다'} 응답
 * @param {{correlation_id: string, idempotency_key: string, occurred_at?: string}} 키들
 */
function 확인사건(카드, 응답, 키들) {
  if (!카드 || !카드.trait_axis || !카드.shown_key || !카드.shown_text
    || !카드.estimator_version || !카드.estimate_as_of) return null;
  if (응답 !== '맞다' && 응답 !== '아니다') return null;      // 값록(계약 「확인응답」) — 모르는 값 거부
  if (!키들 || !키들.correlation_id || !키들.idempotency_key) return null;
  return {
    event_type: 'estimate.responded',
    correlation_id: String(키들.correlation_id),
    idempotency_key: String(키들.idempotency_key),
    /* 공통 필수 — occurred_at 은 답한 «그 순간»(호출자가 들면 그것 · 아니면 지금). level_snapshot
     * 은 ⓑ 완화 규약(키 존재 + null) 그대로다: 이 카드에 급수가 안 흐르니 null 이 유일하게
     * 정확한 값이다(「그때 화면이 알던 값」— 화면이 몰랐으면 null · 이벤트검증 널허용). */
    occurred_at: String(키들.occurred_at || new Date().toISOString()),
    level_snapshot: null,
    payload: {
      /* ver — 서버(events)가 «모든» payload 에 정수 ver 를 요구한다. 이 칸이 빠져 있던 것을
       * 08-24 첫 실왕복(⑯)이 잡았다 — 게임로그 접두 구멍에 가려 이 조립기의 산출이 서버에
       * 닿아본 적이 없었고, 그래서 lib 검증만으로는 안 보였다(다른 조립기는 전부 ver:1). */
      ver: 1,
      trait_axis: String(카드.trait_axis),
      shown_key: String(카드.shown_key),
      shown_text: String(카드.shown_text),
      response: 응답,
      estimator_version: String(카드.estimator_version),
      estimate_as_of: String(카드.estimate_as_of),
    },
  };
}

module.exports = {
  판정판, 하루상한, 후보순서, 부정키, 부정키풀기, 부정키들, 부정지표들,
  리듬후보들, 리듬후보, 작성과정후보들, 집중띠후보들, 축후보들,
  확인카드, 확인사건,
};

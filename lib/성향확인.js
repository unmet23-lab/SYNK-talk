 ...
 *   · 생성 요약(lib/과제요약.js ← deliver 생성모드) — `부정키들`·`부정축들`: 「아니야」 받은 추정의
 *     축을 **다음 생성 입력에서 뺀다**(철학 적용기준 「관측과 성향」 · 09-05 「아니야」= 빼기 · 2026-09-11).
 *
 ...
 *   가는 «재료»에서만 그 축을 거른다(빼는 자리는 lib/과제요약.js · 형식·추출은 여기 한 원천).
 ...
 *   확인축의 «키별 마지막 답» 접기는 라벨 «셈»의 규칙이지 재노출·제외의 규칙이 아니다 — 부정된 키는
 *   재노출이 안 되므로 뒤집힐 기회 자체가 없다(두 규칙이 갈릴 행이 원리상 안 생긴다).
 * 🔑 재료는 호출자가 걷어 온 행 그대로다 — 창·상한은 걷는 쪽 규격이고 여기서 다시 자르지 않는다
 *   (전 이력을 주면 전 이력에서 뺀다). 다른 event_type 행이 섞여 있어도 된다(골라 읽는다).
...
/** 부정키 목록 → 부정된 «축» 이름들(첫 등장 순 · 중복 없음 · 규격 밖 키는 무시). 생성 요약이 축 단위로
 *  뺀다 — 한 키만 부정돼도 그 축의 관측 줄 전체가 빠진다(짐작의 재료를 벤더에게 다시 주지 않는다). */
function 부정축들(키들) {
  const 축들 = [];
  for (const k of Array.isArray(키들) ? 키들 : []) {
    const 풀린 = 부정키풀기(k);
    if (풀린 && !축들.includes(풀린.축)) 축들.push(풀린.축);
  }
  return 축들;
}
...
module.exports = {
  판정판, 하루상한, 후보순서, 부정키, 부정키풀기, 부정키들, 부정축들,

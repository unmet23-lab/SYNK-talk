 ...
 *   규칙(0점→이유 필수«단 ⑦ 제외» · ⑦ null · 쌍둥이 이음)을 두 곳에 적으면 어느 한쪽이 새고,
...
function 매김적용({ 결과, 시험지, case_id, 점수들, note, 채점자, 지금 }) {
  ...
  for (let i = 0; i < 평가.축키들.length; i += 1) {
    ...
    const v = 점수들[i];
    /* 🔴 ⑦(구간축)만 «안 쟀다»를 받는다 — 유호 확정 08-25 「이번 회전엔 ⑦을 안 묻는다」.
     *   왜 null 인가: ⑦은 «세는 칸»이라 0 이 「흠」이 아니라 「안 썼다」다. 안 물은 것을 0 으로
     *   적으면 「안 썼다」와 구별이 안 되고, 오늘 아침에 잡은 그 병(초안 O 가 판정으로 읽힘)의
     *   거울상이 된다. null = 분모에서 빠짐 = 이 판은 ⑦을 «안 쟀다»가 파일에 그대로 남는다.
     *   ⚠ 그 대가는 집계가 진다 — ⑦ 분모 0 이면 §8-B 는 통과할 수 없다(안 잰 축은 통과가 아니다). */
    if (v === null && k === 평가.구간축) { 매김[k] = null; continue; }
    if (v !== 0 && v !== 1) return { ok: false, 오류: `${k} 값 ${JSON.stringify(v)} — 0·1 뿐(null 은 ⑦에만)` };
    매김[k] = v;
  }
  const 영점 = 평가.축키들.filter((k) => k !== 평가.구간축 && 매김[k] === 0);   // ⑦=0 은 절제(v2) — 이유 안 묻는다

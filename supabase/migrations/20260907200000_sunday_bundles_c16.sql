...
/* 🔴 engine 을 API 에 노출하는 날 통째로 읽히지 않게 — 정책 0 = 서비스 역할(함수)만 읽고 쓴다.
   시험 `tests/L0스키마.test.js` 가 「RLS 없는 표」를 잡는다(잊은 표 = 노출하는 날의 구멍). */
alter table engine.sunday_bundles enable row level security;

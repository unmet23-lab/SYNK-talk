/* c16 — 접기 어휘 둘(fold_date·promote_ver) · CHECK 이름 c15→c16 (유호 지시 2026-09-02 「c16도 지금 해줘」)
 *
 * ■ 무엇을 여나 — **payload 어휘 둘뿐이다. 새 사건 0 · 새 열 0 · CHECK 값 변경 0.**
 *   ①`fold_date` — 그 사건이 «어느 현지 날짜에 접혔나»(YYYY-MM-DD).
 *   ②`promote_ver` — 그렇게 접은 «판»(예: radio-promote.v2).
 *   근거 = appsscript `docs/_ops/심문결과/라디오24_설계-전건판정.md` ⑧.
 *   라디오 승격은 「학생×몽골날짜×명령」의 하루 첫 선언만 올리는데, 그 날짜가 `correlation_id` 의
 *   결정적 uuid **안에 녹아** 있어 역산이 안 됐고, 접은 판은 Fn 응답에만 실렸다. ⇒ 날짜 모듈이
 *   바뀌거나 재실행되면 **같은 sent_at 이 다른 날짜로 접히는데 그 사실이 행 어디에도 안 남는다.**
 *
 * ■ 이 조각이 «DDL 로» 하는 일은 CHECK 72개의 이름을 c15 → c16 으로 옮기는 것뿐이다.
 *   본문(check 식)은 한 글자도 안 건드린다 — 값이 는 CHECK 가 0 이라서다.
 *   🔑 이름 옮김은 **기계로** 지었다(c15 조각에서 읽어 이름만 치환) — 손으로 옮기면 하나가 빠지고,
 *     빠진 것의 증상은 「그 제약만 옛 이름으로 살아남음」이라 조용하다.
 *
 * ■ 허용필드는 DDL 이 아니라 **계약 파일**이 진다(`계약/수집_교정_계약.json` · 양쪽 저장소 동일 바이트).
 *   검증기(`lib/이벤트검증.js`)는 그 파일을 «읽어» 쓰므로 사본이 없다.
 *
 * 되돌림: 아래 add constraint 들의 _c16 을 _c15 로 되돌려 다시 붓는다;
 *         계약 파일 둘을 c15 로; delete from engine.schema_migrations where version='20260902100000'; */

begin;

do $migration$
declare
  migration_version constant text := '20260902100000';
  migration_name constant text := '20260902100000_fold_vocab_c16.sql';
  expected_checksum constant text := '0d42203d1b48b73b197f485fad40096b450912fd516a409a29bc34944937e3e5'; -- migration-checksum
  base_version constant text := '20260902000000';
...
-- 살아 있는 CHECK 72개 — c15 본문 그대로 이름만 c16(이번 판은 값이 는 CHECK 0 — 어휘는 payload·계약 층).
alter table engine.generation_attempts
  drop constraint if exists attempts_gate_values_c15,
  add constraint attempts_gate_values_c16 check ( gate_failed_reasons is null or gate_failed_reasons <@ array[ '길이','한국어비율','빈출력','금칙서식','질문형태','식별자역유입','중복']::text[] );

alter table engine.generation_attempts
  drop constraint if exists attempts_response_present_c15,
  add constraint attempts_response_present_c16 check ( result is null or result not in ('성공','검문탈락','응답파손','응답초과') or (raw_response is not null and responded_at is not null) );

alter table engine.generation_attempts
  drop constraint if exists attempts_result_gate_c15,
  add constraint attempts_result_gate_c16 check ( case when result = '검문탈락' then gate_failed_reasons is not null and cardinality(gate_failed_reasons) >= 1 else gate_failed_reasons is null end );

alter table engine.generation_attempts
  drop constraint if exists attempts_ver_nonempty_c15,
  add constraint attempts_ver_nonempty_c16 check ( btrim(model) <> '' and btrim(prompt_ver) <> '' and btrim(policy_ver) <> '' and btrim(estimator_version) <> '' and btrim(schema_ver) <> '' and btrim(skill_taxonomy_ver) <> '' );

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_counts_order_c15,
  add constraint batch_runs_counts_order_c16 check ( target_count is null or (target_count >= 0 and loaded_count <= enrolled_count) );

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_counts_pair_c15,
  add constraint batch_runs_counts_pair_c16 check ((target_count is null) = (loaded_count is null));

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_enrolled_nonneg_c15,
  add constraint batch_runs_enrolled_nonneg_c16 check (enrolled_count >= 0);

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_finished_cols_c15,
  add constraint batch_runs_finished_cols_c16 check ( finished_at is null or (target_count is not null and loaded_count is not null and partial_count is not null and skipped_game_count is not null and skipped_existing_count is not null) );

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_level_dist_ok_c15,
  add constraint batch_runs_level_dist_ok_c16 check (engine.level_dist_ok(level_distribution, enrolled_count));

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_partial_pair_c15,
  add constraint batch_runs_partial_pair_c16 check ((partial_count is null) = (loaded_count is null));

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_partial_range_c15,
  add constraint batch_runs_partial_range_c16 check ( partial_count is null or (partial_count >= 0 and partial_count <= enrolled_count) );

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_roster_equation_c15,
  add constraint batch_runs_roster_equation_c16 check ( finished_at is null or run_kind <> '배치' or (loaded_count + skipped_game_count + skipped_existing_count = enrolled_count) );

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_skipped_range_c15,
  add constraint batch_runs_skipped_range_c16 check ( (skipped_game_count is null or (skipped_game_count >= 0 and skipped_game_count <= enrolled_count)) and (skipped_existing_count is null or (skipped_existing_count >= 0 and skipped_existing_count <= enrolled_count)) );

alter table engine.generation_batch_runs
  drop constraint if exists batch_runs_ver_nonempty_c15,
  add constraint batch_runs_ver_nonempty_c16 check ( btrim(model) <> '' and btrim(prompt_ver) <> '' and btrim(policy_ver) <> '' and btrim(estimator_version) <> '' and btrim(schema_ver) <> '' and btrim(skill_taxonomy_ver) <> '' );

alter table radio.broadcast_segment
  drop constraint if exists broadcast_segment_kind_c15,
  add constraint broadcast_segment_kind_c16 check (kind in ('radio_loop', 'live_lecture', 'asmr_mode', 'other'));

alter table engine.classes
  drop constraint if exists classes_key_nonblank_c15,
  add constraint classes_key_nonblank_c16 check (btrim(class_key) <> '');

alter table engine.companion_qa
  drop constraint if exists companion_qa_answer_paired_c15,
  add constraint companion_qa_answer_paired_c16 check (handoff or btrim(answer) <> '');

alter table engine.companion_qa
  drop constraint if exists companion_qa_question_nonblank_c15,
  add constraint companion_qa_question_nonblank_c16 check (btrim(question) <> '');

alter table engine.corrections
  drop constraint if exists corrections_promotion_intent_c15,
  add constraint corrections_promotion_intent_c16 check (promotion_intent = false or actor_kind = 'teacher');

alter table engine.corrections
  drop constraint if exists corrections_supersedes_not_self_c15,
  add constraint corrections_supersedes_not_self_c16 check (supersedes is null or supersedes <> correction_id);

alter table engine.corrections
  drop constraint if exists corrections_verdict_c15,
  add constraint corrections_verdict_c16 check (verdict is null or verdict in ( 'AI 교정이 맞다', '고칠 곳이 있다', '원문이 이미 맞다' ));

alter table ops.cron_runs
  drop constraint if exists cron_runs_outcome_c15,
  add constraint cron_runs_outcome_c16 check (outcome in ('대기', '성공', '실패', '타임아웃', '전송오류', '상태없음', '유실', '발사실패'));

alter table engine.generation_jobs
  drop constraint if exists jobs_anchor_present_c15,
  add constraint jobs_anchor_present_c16 check ( status not in ('착지','마감폴백','대상아님') or assigned_event_id is not null );

alter table engine.generation_jobs
  drop constraint if exists jobs_claim_cols_c15,
  add constraint jobs_claim_cols_c16 check (status <> 'claimed' or (owner is not null and lease_until is not null));

alter table engine.generation_jobs
  drop constraint if exists jobs_deciding_pair_c15,
  add constraint jobs_deciding_pair_c16 check ((deciding_attempt_id is null) = (deciding_result is null));

alter table engine.generation_jobs
  drop constraint if exists jobs_deciding_result_matches_c15,
  add constraint jobs_deciding_result_matches_c16 check (deciding_result is null or deciding_result = outcome);

alter table engine.generation_jobs
  drop constraint if exists jobs_deciding_scope_c15,
  add constraint jobs_deciding_scope_c16 check ( case when outcome is null then deciding_attempt_id is null when outcome in ('검문탈락','타임아웃','벤더오류','응답파손','입력초과','응답초과') then deciding_attempt_id is not null else deciding_attempt_id is null end );

alter table engine.generation_jobs
  drop constraint if exists jobs_draft_present_c15,
  add constraint jobs_draft_present_c16 check (status = '적재실패' or event_draft is not null);

alter table engine.generation_jobs
  drop constraint if exists jobs_idle_cols_c15,
  add constraint jobs_idle_cols_c16 check (status <> '대기' or (owner is null and lease_until is null));

alter table engine.generation_jobs
  drop constraint if exists jobs_load_failed_cols_c15,
  add constraint jobs_load_failed_cols_c16 check ( status <> '적재실패' or (outcome = '내부오류' and assigned_event_id is null and event_draft is null and load_error is not null) );

alter table engine.generation_jobs
  drop constraint if exists jobs_nontarget_cols_c15,
  add constraint jobs_nontarget_cols_c16 check (status <> '대상아님' or not_target_reason is not null);

alter table engine.generation_jobs
  drop constraint if exists jobs_nonterminal_cols_c15,
  add constraint jobs_nonterminal_cols_c16 check ( status in ('착지','마감폴백','대상아님','적재실패') or (outcome is null and closed_at is null and assigned_event_id is null and winning_attempt_id is null) );

alter table engine.generation_jobs
  drop constraint if exists jobs_skill_ids_present_c15,
  add constraint jobs_skill_ids_present_c16 check ( status = '적재실패' or not_target_reason is not null or cardinality(skill_ids) between 1 and 2 );

alter table engine.generation_jobs
  drop constraint if exists jobs_status_outcome_pairs_c15,
  add constraint jobs_status_outcome_pairs_c16 check ( case status when '대상아님' then outcome = '대상아님' when '마감폴백' then outcome = '예산소진' when '적재실패' then outcome = '내부오류' when '착지' then outcome is null or outcome not in ('대상아님','예산소진') else true end );

alter table engine.generation_jobs
  drop constraint if exists jobs_terminal_cols_c15,
  add constraint jobs_terminal_cols_c16 check ( status not in ('착지','마감폴백','대상아님','적재실패') or (outcome is not null and closed_at is not null) );

alter table engine.generation_jobs
  drop constraint if exists jobs_ver_nonempty_c15,
  add constraint jobs_ver_nonempty_c16 check ( btrim(model) <> '' and btrim(prompt_ver) <> '' and btrim(policy_ver) <> '' and btrim(estimator_version) <> '' and btrim(schema_ver) <> '' and btrim(skill_taxonomy_ver) <> '' );

alter table engine.generation_jobs
  drop constraint if exists jobs_winner_fence_current_c15,
  add constraint jobs_winner_fence_current_c16 check (winning_fence is null or winning_fence = fence);

alter table engine.generation_jobs
  drop constraint if exists jobs_winner_fence_pair_c15,
  add constraint jobs_winner_fence_pair_c16 check ((winning_attempt_id is null) = (winning_fence is null));

alter table engine.generation_jobs
  drop constraint if exists jobs_winner_only_success_c15,
  add constraint jobs_winner_only_success_c16 check (winning_attempt_id is null or outcome = '성공');

alter table engine.generation_jobs
  drop constraint if exists jobs_winner_present_c15,
  add constraint jobs_winner_present_c16 check (outcome <> '성공' or winning_attempt_id is not null);

alter table engine.generation_jobs
  drop constraint if exists jobs_winner_result_only_success_c15,
  add constraint jobs_winner_result_only_success_c16 check (winning_result is null or winning_result = '성공');

alter table engine.generation_jobs
  drop constraint if exists jobs_winner_result_pair_c15,
  add constraint jobs_winner_result_pair_c16 check ((winning_attempt_id is null) = (winning_result is null));

alter table engine.l10n_reviews
  drop constraint if exists l10n_reviews_final_paired_c15,
  add constraint l10n_reviews_final_paired_c16 check (
      (verdict = '원문을 고쳐야 한다' and final_mn is null)
      or (verdict <> '원문을 고쳐야 한다' and btrim(coalesce(final_mn, '')) <> '')
    );

alter table engine.l10n_reviews
  drop constraint if exists l10n_reviews_supersedes_not_self_c15,
  add constraint l10n_reviews_supersedes_not_self_c16 check (supersedes is distinct from review_id);

alter table engine.l10n_reviews
  drop constraint if exists l10n_reviews_verdict_c15,
  add constraint l10n_reviews_verdict_c16 check (verdict in ('초벌이 맞다', '고쳤다', '원문을 고쳐야 한다'));

alter table engine.l10n_strings
  drop constraint if exists l10n_strings_id_ascii_c15,
  add constraint l10n_strings_id_ascii_c16 check (string_id ~ '^[a-z0-9]+([._-][a-z0-9]+)+$');

alter table engine.l10n_strings
  drop constraint if exists l10n_strings_ko_nonblank_c15,
  add constraint l10n_strings_ko_nonblank_c16 check (btrim(source_ko) <> '');

alter table engine.l10n_strings
  drop constraint if exists l10n_strings_max_len_c15,
  add constraint l10n_strings_max_len_c16 check (max_len is null or max_len between 1 and 4000);

alter table engine.l10n_strings
  drop constraint if exists l10n_strings_status_c15,
  add constraint l10n_strings_status_c16 check (status in ('pending', 'verified', 'discarded'));

alter table engine.learners
  drop constraint if exists learners_gender_c15,
  add constraint learners_gender_c16 check (gender is null or gender in ('female', 'male', 'undisclosed'));

alter table engine.learners
  drop constraint if exists learners_goal_track_c15,
  add constraint learners_goal_track_c16 check (goal_track is null or goal_track in ('study', 'work', 'culture'));

alter table engine.learners
  drop constraint if exists learners_group_no_c15,
  add constraint learners_group_no_c16 check (group_no between 1 and 20);

alter table engine.learners
  drop constraint if exists learners_home_aimag_c15,
  add constraint learners_home_aimag_c16 check (home_aimag is null or home_aimag in ( 'ulaanbaatar', 'arkhangai', 'bayan-olgii', 'bayankhongor', 'bulgan', 'darkhan-uul', 'dornod', 'dornogovi', 'dundgovi', 'govi-altai', 'govisumber', 'khentii', 'khovd', 'khovsgol', 'omnogovi', 'orkhon', 'ovorkhangai', 'selenge', 'sukhbaatar', 'tov', 'uvs', 'zavkhan'));

alter table engine.learners
  drop constraint if exists learners_seat_no_c15,
  add constraint learners_seat_no_c16 check (seat_no between 1 and 20);

alter table engine.learners
  drop constraint if exists learners_signup_attempts_nonneg_c15,
  add constraint learners_signup_attempts_nonneg_c16 check (signup_attempts >= 0);

alter table engine.learners
  drop constraint if exists learners_temp_password_paired_c15,
  add constraint learners_temp_password_paired_c16 check (temp_password_hash is null or temp_password_expires_at is not null);

alter table engine.learning_events
  drop constraint if exists learning_events_correction_target_c15,
  add constraint learning_events_correction_target_c16 check ( case when event_type in ('correction.viewed', 'correction.responded') then correction_id is not null else correction_id is null end );

alter table engine.learning_events
  drop constraint if exists learning_events_event_type_c15,
  add constraint learning_events_event_type_c16 check (event_type in (
...
alter table engine.learning_events
  drop constraint if exists learning_events_task_type_c15,
  add constraint learning_events_task_type_c16 check (task_type is null or task_type in ( '숙제제출', '다시쓰기', '퀴즈응답', '대화턴', '발화녹음', '출석발화', '라디오퀴즈', '목표선언', '자습체크인' ));

alter table engine.pipeline_jobs
  drop constraint if exists pipeline_jobs_discard_reason_c15,
  add constraint pipeline_jobs_discard_reason_c16 check (discard_reason is null or (status = 'discarded' and discard_reason in ('무음', '손상', '중복', '과제 불일치', '타인 음성', '판정 불가')));

alter table engine.season_compass
  drop constraint if exists season_compass_answers_c15,
  add constraint season_compass_answers_c16 check ( ( self_in_5y_changed is null and answers ?& array['why_learning', 'self_in_5y', 'topik_use', 'season_goal'] and answers - array['why_learning', 'self_in_5y', 'topik_use', 'season_goal'] = '{}'::jsonb ) or ( self_in_5y_changed is not null and answers ?& array['self_in_5y', 'season_goal'] and answers - array['self_in_5y', 'season_goal'] = '{}'::jsonb ) );

alter table engine.season
  drop constraint if exists season_dates_c15,
  add constraint season_dates_c16 check (ends_on is null or ends_on >= starts_on);

alter table engine.season_review
  drop constraint if exists season_review_decided_c15,
  add constraint season_review_decided_c16 check ( (verdict is null and note is null and decided_by is null and decided_at is null) or (verdict is not null and decided_by is not null and decided_at is not null and note is not null and btrim(note) <> '') );

alter table engine.season_review
  drop constraint if exists season_review_self_c15,
  add constraint season_review_self_c16 check (verdict_by_self is null or verdict_by_self in ('closer', 'same', 'redirected'));

alter table engine.season_review
  drop constraint if exists season_review_verdict_c15,
  add constraint season_review_verdict_c16 check (verdict is null or verdict in ('closer', 'same', 'redirected'));

alter table engine.staff
  drop constraint if exists staff_role_c15,
  add constraint staff_role_c16 check (role in ('teacher', 'inspector', 'director', 'l10n_reviewer'));

alter table engine.submissions
  drop constraint if exists submissions_due_paired_c15,
  add constraint submissions_due_paired_c16 check ( (due_at is null) = (due_ver is null) );

alter table engine.submissions
  drop constraint if exists submissions_task_format_c15,
  add constraint submissions_task_format_c16 check (task_format is null or task_format in ( '낭독', '응답', '자유발화', '모의면접', '높임전환', '쓰기첨삭', '번역' ));

alter table engine.submissions
  drop constraint if exists submissions_translation_source_c15,
  add constraint submissions_translation_source_c16 check ( task_format is distinct from '번역' or nullif(btrim(task_snapshot->>'mn'), '') is not null );

alter table engine.teacher_notes
  drop constraint if exists teacher_notes_body_nonblank_c15,
  add constraint teacher_notes_body_nonblank_c16 check (btrim(body) <> '');

alter table engine.teacher_notes
  drop constraint if exists teacher_notes_disposition_c15,
  add constraint teacher_notes_disposition_c16 check (disposition in ('confirmed', 'retry'));

alter table engine.teacher_notes
  drop constraint if exists teacher_notes_origin_c15,
  add constraint teacher_notes_origin_c16 check (origin in ('as_is', 'edited', 'written'));
...
begin
  ...
  values ('20260902100000', '20260902100000_fold_vocab_c16.sql', expected_checksum);
end
$migration2$;

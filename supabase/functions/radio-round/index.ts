/* 라디오 라운드 문 — 문항 팩·노출 이력 → «제안», 전광판 ack → `radio.quiz_round` 행.
 ...
 *              돌려주는 `화면` 을 봇이 그대로 오버레이에 흘린다(정답·해설은 안 실린다).
 *   ㉡ `노출` — 전광판이 실제로 그린 뒤 내보내는 `{종류:'노출', round_id, at}` 을 받아
 *              그때서야 `radio.quiz_round` 에 앉힌다. `shown_at = ack.at` 이다.
 *   ㉢ `마감` — `{종류:'마감확인'}` 을 받아 `closed_at` 을 적는다. 이게 없으면 승격기의
 *              「마감후」 제외(`라디오승격` 216행)가 **영원히 안 걸려** 늦은 답이 승격된다.
 *              🔑 시간 만료는 화면의 «추정»이라 전광판이 ack 를 안 보낸다 ⇒ `closed_at` 은
 *              그대로 null 이고, 승격기는 null 을 「안 닫혔다」로 읽는다(정직한 빈칸).
 *

...
async function 배달하기(오늘: string, 한사람: string | null = null, 맥락: string | null = null) {
  ...
  const results: Record<string, unknown>[] = new Array(대상.length);
  ...
  /* 🔑 **힙 봉우리를 재서 봉투에 싣는다** — 546 이 났을 때 「메모리였나 CPU 였나」를 밖에서
   *   가를 유일한 재료다(리허설 함수 런타임 로그는 관리 API 로 안 읽힌다 — 실측 2026-08-22).
   *   못 재는 런타임이면 null 이고 배달은 그대로 간다 — 계량이 배달을 막지 않는다. */
  const 힙 = () => { try { return (Deno as unknown as { memoryUsage?: () => { rss: number; heapUsed: number } }).memoryUsage?.() ?? null; } catch { return null; } };
  let 최대rss = 0, 최대힙사용 = 0;
  const 힙재기 = () => { const m = 힙(); if (m) { 최대rss = Math.max(최대rss, m.rss); 최대힙사용 = Math.max(최대힙사용, m.heapUsed); } };
  힙재기();
  ...
  const 일꾼 = async () => {
    for (;;) {
      ...
      const 몇째 = 다음++;
      if (몇째 >= 대상.length) return;
      const 학생 = 대상[몇째] as Record<string, unknown>;
      results[몇째] = await 한명(학생, 오늘, ver, 스냅기준);
      ...
      /* 🔴 **다 쓴 원신호를 놓는다** — 이 학생의 raw 사건 배열은 위 두 줄(`한명`·`회수요약`)이
       *   마지막 소비자이고, 그 뒤로는 아무도 안 본다. 안 놓으면 `대상` 이 학급 «전원»의 사건
       *   객체를 배치가 끝날 때까지 붙잡아, 실행 내내 살아 있는 힙이 학급 크기에 비례한다.
       * 🔑 바이트로는 작아 보이는데(리허설 823명 원신호 전량 1.86 MB) 실제로 무거운 것은
       *   **JS 객체 수**다 — 창 안 사건 5,909개가 중첩 payload 를 든 객체로 부풀고, 그 위에
       *   동시 8 갈래의 중간 계산이 겹친다. 실측 2026-08-22: 재적 648 은 41초·200 인데
       *   823 에서 같은 함수가 `546 WORKER_RESOURCE_LIMIT` 으로 죽었다(벽시계가 아니라 자원).
       * ⚠ `대상` 의 원소를 **고쳐 쓴다** — 이 배열은 이 함수가 방금 만든 지역 값이고 소비자가
       *   여기뿐이다. 밖에서 온 값이었다면 남의 것을 비우는 셈이라 하면 안 되는 짓이다. */
      학생.원신호 = null;
      학생.마지막스냅샷 = null;
      if ((몇째 & 63) === 0) 힙재기();   // 64명마다 — 재는 값이 재는 비용을 넘지 않게
    }
  };
  await Promise.all(Array.from({ length: Math.min(동시, 대상.length) }, () => 일꾼()));
  ...
  const 몸 = {
    ...
    /* 🔑 배치가 자원 상한(546)에 얼마나 붙어 있었나. 죽은 실행은 응답이 없으니 이 값은
     *   «살아 돌아온 실행»의 것이고, 그래서 「어디까지가 안전한가」의 유일한 눈금이다. */
    메모리: 최대rss ? { 최대rss, 최대힙사용 } : null,
  };

...
async function 배달하기(오늘: string, 한사람: string | null = null, 맥락: string | null = null) {
  ...
  const 동시 = 8;
  /* 🔴 **학급을 한 번에 들어 올리지 않는다** — 커서로 묶음씩 흘린다(2026-08-22 · 546 수리).
   ...
   * 🔑 묶음 100 · 동시 8 = 커넥션 9(커서 1 + 트랜잭션 8)로 풀(postgres 기본 max 10) 안이다.
   *   묶음을 키우면 힙이, 동시를 키우면 커넥션이 먼저 벽에 닿는다 — 두 손잡이의 벽이 다르다.
   * ⚠ 묶음 사이에는 벽(barrier)이 선다 — 앞 묶음이 다 끝나야 다음 묶음이 온다. 완전 파이프라인
  ...
  const 묶음처리 = async (학생들: Record<string, unknown>[]) => {
    ...
    const 일꾼 = async () => {
      for (;;) {
         ...
         *   **같은 방향의 두 걸음**이다(묶음 밖은 커서가, 묶음 안은 이 두 줄이 놓는다).
  ...
  if (한사람) {
    ...
    await 대상질의(스냅기준, null).cursor(묶음크기,
      (행들) => 묶음처리(행들 as unknown as Record<string, unknown>[]));
  }

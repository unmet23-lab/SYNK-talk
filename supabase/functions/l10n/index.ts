 ...
 *
 * ■ 이 목록이 «안전»을 지는 게 아니다 — 그건 스키마가 이미 졌다
 *   이 함수가 닿는 표에는 학생 식별자가 한 칸도 없다. 그래서 여기 누구를 더해도 **학생 데이터가
 *   새지 않는다.** 이 목록이 지는 것은 다른 것이다: **「누가 이 일을 하나」를 한 곳에 적어 두는 것.**
 *   그래야 어떤 역할을 주는 일이 «다른 문까지 조용히 여는» 일이 되지 않는다.
 *
 * ■ 왜 셋인가 (유호 확정 2026-08-27 「1,2로 진행하자」)
 *   · `director` — 원장은 세 문 전부에 든다(`review`·`teach` 와 같은 규약).
 *   · `teacher`  — **몽골 강사가 몽골어 원어민**이다. 바깥 계약자보다 제품을 알고, 이미 신뢰
 *     관계가 있고, 9~10월 채용을 안 기다려도 된다. 유호님이 그 사람들에게 맡기기로 정했다.
 *     ⚠ 안전이 넓어지는 게 아니다 — 강사는 이미 학생 발화·녹음을 본다(`teach` 문). 여기 더한다고
 *       새로 열리는 자원이 없다. **반대 방향**(감수자가 학생 자원으로 가는 길)은 그대로 막혀 있다.
 *   · `l10n_reviewer` — 바깥 사람에게 맡기는 날의 자리. 그 사람은 **이 문 하나만** 받는다.
 *
 * ■ 🚫 `inspector` 는 여전히 **안 넣는다** — 안 안전해서가 아니라 **그렇게 정한 적이 없어서**다.
 *   발화 검수자가 문구 감수도 한다는 결정은 없다. 필요해지는 날 «결정»으로 더하지, 「어차피
 *   직원이니까」로 더하지 않는다 — 그 논리를 한 번 허용하면 이 목록은 곧 `staff` 전체가 된다.
 */
const 감수역할 = ['l10n_reviewer', 'teacher', 'director'];

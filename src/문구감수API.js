...
/* 어휘·판정은 `lib/문구감수.js` 하나에서 온다 — 서버(Edge Fn)가 먹는 그 파일 그대로다.
 * 화면이 자기 목록을 따로 들면 DB CHECK 와 갈라지고, 그때 증상은 400 이 아니라 **500** 이다
 * (DB 가 CHECK 에 없는 값을 받고 그 자리에서 죽는다 · `tests/문구감수.test.js` 머리말). */
export { VERDICT, 원문결함, 확정요청 } from '../lib/문구감수.js';

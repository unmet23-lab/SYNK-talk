 ...
 * 쓰고 있었다(`쓸수있나` 의 `>= 6` · 도움말 「6자 이상」) — 상수로 세워 오류 문장의 `{n}` 까지
 * 한 자리에서 나오게 한다. ⚠ 서버가 진짜 게이트다(여기 값은 화면을 미리 잠그는 편의일 뿐). */
const 최소비번 = 6;
...
export default function 인증화면({ 로그인성공, 시작단계 = 단계.로그인, 닫기 }) {
  ...
  const 제출 = {
    ...
    [단계.첫등록]: {
      ...
      쓸수있나: 번호형식 && 뒷자리.length === 4 && 새비번.length >= 최소비번 && !답검사(가입답),
      버튼: '시작하기',
    ...
    [단계.임시]: {
      제목: '임시번호로 들어가기',
      쓸수있나: 번호형식 && 임시번호.length === 6 && 새비번.length >= 최소비번,
      버튼: '새 비밀번호로 시작하기',

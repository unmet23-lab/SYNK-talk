  ...
  return (
    <View style={s.wrap}>
      <ScrollView contentContainerStyle={s.inner}>
        ...
        <View style={s.칸}>
          ...
          {상황줄.map(({ 이름, 쓰임 }) => (
            ...
            >
              ...
              {/* ⚠ 이 칸은 **몽글 줄만** 들려준다 — 고르는 자리는 첫등록 가이드 고르기(D4-1)로
                  섰고 까몽 소리는 앱에서 난다(마린은 「곧 와요」 비활성). 이 미리듣기 칸을
                  캐릭터 셋으로 넓히는 건 까몽·마린 소리의 귀검수 필요가 생기는 날이다. */}
        ...
        {/* ── ⑦ 녹음 단추 — ✅ **㉮ 확정**(유호 08-28 「녹음띠는 ㉮로 가자」) ──
            🔑 판정이 났으므로 이 칸은 이제 «묻는 자리»가 아니라 **박자 눈검수 자리**다.
            옛 세 안 비교(㉯ 코랄 원 + 밴드 · ㉰ 그대로)는 **지웠다** — 낡은 판을 남기면
            다음 사람이 「아직 고르는 중인가」로 읽는다. 경위는 git 이 쥔다.
            🔴 **알고 고른 대가를 여기 남긴다**: 대기 판에는 코랄이 없어 **녹음 «전» 화면의
            신호가 0점**이다(테마 R1 신호자리 = 녹음버튼). 이것은 결함이 아니라 확정이다 —
            「신호가 없다」를 고치겠다고 코랄 원을 되살리지 않는다. 되돌리려면 유호님 판정이 먼저다. */}
        <View style={s.칸}>
          <View style={s.칸머리}>
            <Text style={s.칸이름}>⑦ 녹음 단추 — 펠트 띠 3상태 (㉮ 확정 · 말하기 화면에 배선됨)</Text>
          </View>
          <Text style={s.눈금}>
            띠가 말하는 문장 = 대기(실이 아직 안 꿰어짐 · 코랄 없음) → 말하는중(꿰는 중 · 여기서 신호가 선다)
            → 맺음(매듭 · 확인 단계) · 셋은 «한 자»로 잘려 같은 틀이라 상태가 바뀌어도 띠가 안 움직인다 ·
            녹음 중에는 띠가 통째로 얕게 숨 쉰다(1 → 1.03)
          </Text>
          <View style={[s.무대, { alignItems: 'center', gap: 10, paddingVertical: 14 }]}>
            {녹음상태들.map((상태) => (
              <View key={`녹음-${상태}`} style={{ alignItems: 'center', gap: 4 }}>
                <녹음띠 상태={상태} 폭={248} />
        ...
        {/* ── ⑧ 막힘 카드 — 서버가 «막았을 때»만 서는 화면 (08-28) ──
            🔴 이 칸이 없으면 **아무도 이 카드를 못 본다.** 카드는 `GET /v1/tasks` 가 `blocked` 를
            실어 올 때만 서는데, 그 상태를 에뮬레이터에서 만들 길이 없다 — 검수문의 존재 이유가
            정확히 그 자리다(머리말).
            🔑 여기서 볼 것 = 펠트 «내려놓은 바늘»이 글 위에 앉는 박자. 막힌 날은 학생이 잘못한
            것이 아니라 일이 잠깐 멈춘 것이고, 그 뜻을 그림이 말 없이 진다.
            🔑 그리고 이 조각엔 **유채가 없다** — 이 화면의 신호 1점(녹음 버튼)을 안 깬다. */}
        <View style={s.칸}>
          <View style={s.칸머리}>
            <Text style={s.칸이름}>⑧ 막힘 카드 — 펠트 «내려놓은 바늘»</Text>
          </View>
          <Text style={s.눈금}>
            동의가 아직 없는 학생이 보는 판입니다. 학생번호는 가짜예요 — 그림과 배치만 보시면 됩니다.
          </Text>

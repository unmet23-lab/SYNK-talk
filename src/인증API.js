 ...
 *
 * 🔴 **PUT 은 「방금 확인한」 세션으로 친다 — 화면이 쥔 옛 토큰이 아니다.**
 *   GoTrue 는 비밀번호가 바뀌면 **그 요청을 친 세션 하나만 남기고 나머지를 전부 죽인다**
 *   (리허설 실측 2026-08-10 · 죽은 쪽은 `refresh_token_not_found` · access_token 403).
 *   옛 토큰으로 치면 살아남는 것은 옛 세션이고, 확인하려고 방금 만든 세션은 **모듈 기억
 *   (`최근세션`)에 시체로 남는다** — 그 뒤 첫 401 에서 `토큰되살리기` 가 「기억이 더 새것」이라며
 *   그 시체를 꺼내 주고(위 함수 첫 갈래), 갱신도 그 시체의 refresh_token 으로 시도해 실패한다.
 *   증상은 **「비밀번호를 바꾸고 조금 있다가 로그인이 풀리는데, 앱을 껐다 켜면 된다」** 하나뿐이라
 *   (키체인엔 아직 산 옛 토큰이 있다) 어느 층도 빨개지지 않는다.
 *   순서를 뒤집으면 **살아남는 세션과 기억이 같아진다** — 덤으로 남는 것이 「방금 본인을 확인한」
 *   세션이라, 훔쳐간 토큰은 이 변경으로 정확히 끊긴다(그게 비밀번호 변경의 목적이다).
 *
 * @returns {Promise<object>} 살아남은 새 세션. 🔴 호출부는 이것을 **반드시** `로그인성공` 에
 *   넘긴다 — 안 넘기면 앱 state·키체인이 방금 죽은 옛 세션을 든 채로 남는다.
 * ⚠ 남은 창 하나: PUT 이 서버에 닿은 뒤 응답만 유실되면 옛 세션은 이미 죽었는데 이 함수는
 *   던진다. 그때도 **모듈 기억은 산 세션**이라 도는 앱은 첫 401 에서 스스로 복구하고, 앱을
 *   다시 켜는 경우에만 재로그인이 필요하다(키체인이 옛 것). 비밀번호를 바꾼 직후라 재로그인은
 *   학생이 이유를 아는 유일한 자리다 — 그래서 이 창은 기계로 막지 않고 여기 적어 둔다.
 */
export async function 비밀번호변경({ 학생번호, 현재비밀번호, 새비밀번호 }) {
  ...
  try {
    r = await 시한fetch(`${URL_}/auth/v1/user`, {
      ...
      headers: {
        apikey: ANON,
        Authorization: `Bearer ${세션.access_token}`,
        'Content-Type': 'application/json',
      },

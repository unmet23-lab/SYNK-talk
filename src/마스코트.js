/* 마스코트 — 살아있는 반응 층 (행동 정본 = 캐릭터_생명감_설계.md v1.1 · 그림 = 정본_4K · 09-05).
 *
 ...
 *   ③ 기억감 — **09-02 에 열렸다**(유호 확정 축 A). 「며칠 만이네」(재회)와 「오래 같이 지낸
 *      아이는 기본 거리가 가까워진다」(길들여짐) 둘이 선다. 재료는 날짜뿐이다 — 학생 데이터를
 *      한 톨도 안 본다(판정 = `lib/마스코트생명` 만남갱신·재회눈금·거리배율 · 기억 = `_만남`).
 *      🔴 재회는 «기쁨»이지 «추궁»이 아니다 — 없던 날을 세지 않는다(말할순간 학생판 D).
 *      ⏳ 아직 0인 것: **궤적 반응**(「어제 그 문장 또 나왔네」) — 서버 꼬리 «종류» 칸이 응답에
 *      없어 재료가 없다(생명감 §5 「미리 안 연다」). 재료가 서는 날 사건 이름 하나로 열린다.
 *
 * ■ 원칙 넷의 기계 자리 (생명감 §1)
 *   · 무언이 기본 — 첫 언어는 표정·몸짓이고 말풍선은 «가끔»(사건당 최대 1회 · 확률 절반).
 *   · 평가하지 않는다 — 문구는 전부 `lib/마스코트생명.혼잣말()` 풀에서만 온다(여기서 짓지 않는다 ·
 *     그 풀의 정본은 SYNK-appsscript `docs/캐릭터/혼잣말_정본.json` 450벌이다).
...
import {
  거리배율, 만남갱신, 오늘줄, 잘시간인가, 재회반응, 탭반응고르기, 평상혼잣말, 표시배, 표정컷, 혼잣말,
} from '../lib/마스코트생명.js';
/* 기억감의 그릇 — 「마지막으로 만난 날」과 「함께한 날수」가 여기 산다(강사말 상한과 같은 지도).
   🔑 서버로 나가는 것이 0 이다 — 마스코트의 기억은 학생 데이터가 아니라 «우리 둘의 날짜»뿐이다. */
...
const 컷그림 = {
  ...
  몽글_윙크: require('../assets/마스코트/몽글_윙크.webp'),
  몽글_으쓱: require('../assets/마스코트/몽글_으쓱.webp'),
  몽글_졸림: require('../assets/마스코트/몽글_졸림.webp'),
  몽글_민망: require('../assets/마스코트/몽글_민망.webp'),
  마린_본체: require('../assets/마스코트/마린_본체.webp'),
  마린_놀람: require('../assets/마스코트/마린_놀람.webp'),
  마린_눈웃음: require('../assets/마스코트/마린_눈웃음.webp'),
  마린_눈감음: require('../assets/마스코트/마린_눈감음.webp'),
  마린_졸림: require('../assets/마스코트/마린_졸림.webp'),
  마린_민망: require('../assets/마스코트/마린_민망.webp'),
  마린_몰래기쁨: require('../assets/마스코트/마린_몰래기쁨.webp'),
  마린_집중: require('../assets/마스코트/마린_집중.webp'),
...
/* 🔑 `밤인가` 는 여기 없다 — `lib/마스코트생명.js` 하나가 안다(08-28). 졸림 판정과 «밤 혼잣말»이
   같은 자를 써야 해서다: 두 곳에 적으면 「밤인데 밤말을 안 하는」 어긋남이 조용히 산다. */
 ...
 * @param {'몽글'|'까몽'|'마린'} [props.캐릭터] 학생이 고른 가이드(08-28 반입). 안 넘기면 «몽글»
...
export default function 마스코트({
   ...
   * 🔑 캐릭터에 그 상황의 소리가 «없으면» 판정이 낼까:false 로 답한다(까몽엔 안심·잠결 소리가
   *    없다) — 그때는 조용하다. 비슷한 소리로 메우지 않는 것이 정본의 정지선이다. */
  const 마지막목소리 = useRef(0);
  const 직전소리 = useRef(null);   // 같은 소리를 연달아 내지 않기 위한 기억 하나
  const 목소리 = (상황 = '기본') => {
    const 답 = 목소리판정({
      캐릭터, 상황, 지금: Date.now(), 마지막: 마지막목소리.current,
          ...
          if (걸음.말) {
            ...
            if (글) {
              ...
              /* 대본 걸음의 표정 — 걸음이 스스로 말한다: 질문 슬롯(@질문)은 «물음», 기쁨 컷을
                 ...
                 대본 정본 필드는 안 늘린다 — 연출 prop 이 상황을 진다(@질문의 물음은 그대로). */
              const 말표정 = 걸음.말 === '@질문' ? '물음' : (연출.상황 || (걸음.표정 === '기쁨' ? '기쁨' : '기본'));
              목소리(말표정);
  ...
  useEffect(() => {
    ...
    if (사건.이름 === '무오류첫열람') {
      set표정('놀람');
      예약(() => { set표정('기쁨'); 점프(14); 목소리('기쁨'); 가끔말하기(혼잣말(캐릭터, '무오류첫열람'), '기쁨'); }, 400);
      기본복귀(때.기본복귀 + 400);
      ...
      기울이기(-6, 때.몸짓지속);
      목소리('기쁨');
      가끔말하기(혼잣말(캐릭터, '재도전'), '기쁨');
    }
  }, [사건 && 사건.때]); // eslint-disable-line react-hooks/exhaustive-deps

  /* ── 기억감 — 「며칠 만이네」와 「길들여짐」 (09-02 신설 · 유호 확정 축 A) ─────────
   * 🔴 이 파일 머리말이 「③ 기억감 — 이 층은 아직 0이다」라고 적어 두던 자리다. 그동안
   *   학생이 사흘 만에 앱을 열어도 마스코트는 **처음 만난 것처럼** 똑같이 웃었다.
   * 🔑 두 걸음으로 나눈 까닭: 기록 읽기는 async 이고 `줄임`(reduce-motion)은 그것과 **따로**
   *   정해진다. 한 effect 에 두면 둘의 도착 순서에 따라 정지 화면에서도 재회 표정이 한 번
   *   나는 날이 생긴다 — 그건 「멈추면 정지 화면」(생명감 §1-3)을 조용히 어기는 것이다.
   *   ⇒ ①은 «기억», ②는 «반응». ②가 줄임을 보고 돈다. */
  const [거리배, set거리배] = useState(1);
  const [재회대기, set재회대기] = useState(null);

  /* ① 기억 — 오늘의 만남을 적고, 얼마나 오래 같이 지냈는지 읽는다(마운트 1회). */
  useEffect(() => {
    let 살아있음 = true;
    (async () => {
      try {
        const 기록 = await 마스코트기록읽기();
        const { 기록: 새기록, 눈금, 날수 } = 만남갱신(기록, 오늘줄(new Date()));
        if (!살아있음) return;
        set거리배(거리배율(날수));
        set재회대기(눈금);
        await 마스코트기록쓰기(새기록);
      } catch {
        /* 마스코트의 실패 모드는 «조용함»이지 오류 화면이 아니다(보조 원칙 ③).
           기억을 못 읽으면 오늘은 그냥 처음 만난 것처럼 군다 — 화면은 죽지 않는다. */
      }
    })();
    return () => { 살아있음 = false; };
  }, []);

  /* ② 반응 — 며칠 만이냐에 따라 «다르게» 반가워한다.
     🔴 재회는 «기쁨»이지 «추궁»이 아니다 — 없던 날을 세지 않는다(말할순간 학생판 D).
     🔑 연출 중이면 조용히 넘긴다 — 한시 연기와 겹치면 둘 다 안 읽힌다(둘 중 연출이 화면의 명령이다). */
  useEffect(() => {
    if (!재회대기) return;
    if (연출 || 연출중 || 멈춤()) { set재회대기(null); return; }
    깨우기();
    const 반응 = 재회반응(캐릭터);
    set표정(반응.첫);
    예약(() => {
      ...
      점프(10);
      목소리('기쁨');
      가끔말하기(혼잣말(캐릭터, 재회대기), '기쁨');
    }, 400);
    기본복귀(때.기본복귀 + 400);
    set재회대기(null);
  }, [재회대기, 줄임]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    const 중얼 = setInterval(() => {
      ...
      /* 평상 혼잣말의 «자리»는 시계와 달력이 정한다(아침·겨울·여름 — lib/마스코트생명.자리고르기).
         그 자리 풀이 비면 idle 풀로 떨어진다. 판정은 전부 lib 이 지고 여기는 시각만 넘긴다. */
      말하기(평상혼잣말(캐릭터, new Date()));
    }, 때.idle혼잣말주기);
    return () => { clearInterval(두리번); clearInterval(중얼); };
    /* 캐릭터가 deps 에 있는 이유: 두 인터벌이 클로저로 그 값을 문다 — 빠지면 가이드를 바꾼
       학생에게 «옛 가이드의 문구»가 78초마다 계속 나온다(교체가 조용히 반만 된다). */
  }, [줄임, 졸림, 말, 표정, 연출중, 캐릭터]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  /* 길들여짐의 실물 — «거리»다(★1 · 가이드 정본 §7-1). 오래 같이 지낸 아이에게 마스코트가
     조금 더 가까이 선다: 84px → 최대 92px, 석 달에 걸쳐.
     🔑 크기를 여기서 «레이아웃»으로 준다 — 연출 어휘의 `크기`(다가옴 1.14 · 사라짐 0)와 곱하지
       않으려는 것이다. 그 값은 «지금 이 장면»의 연기고, 이것은 «그동안»의 누적이라 축이 다르다.
       한 transform 에 둘을 곱하면 다음 사람이 「지금 배율이 왜 이 값인가」를 못 센다.
     🔑 reduce-motion 이어도 이 값은 산다 — 크기는 «움직임»이 아니라 «자리»다. */
  /* 🔴 캐릭터 배율(09-05) — 그림마다 캔버스를 채운 정도가 달라서 같은 84pt 자리에 그리면
     덩치가 갈린다. 까몽이 몽글의 85% 로 작게 보이던 자리다(유호 「몽글이보다 까몽이 더
     커야해」). 값과 실측은 `lib/마스코트생명.표시배율` 이 쥔다 — 여기서 숫자를 안 적는다.
     🔑 `몸너비` 하나가 Skia 격자(너비/높이)와 일반 Image 스타일을 다 먹으므로, 여기서
       곱하면 깊이 격자도 «같은 비율»로 따라간다(격자는 정규화 좌표라 어긋나지 않는다). */
  const 캐릭터배 = 표시배(캐릭터);
  const 몸너비 = Math.round(s.몸.width * 거리배 * 캐릭터배);
  const 몸높이 = Math.round(s.몸.height * 거리배 * 캐릭터배);
      ...
      <Pressable onPress={탭} hitSlop={10} accessibilityLabel="마스코트" {...쓰다듬.panHandlers}>
        {몸된다 ? (
          ...
          <마스코트몸
            ...
            탭시각={탭시각}
            기지개시각={기지개시각}
            /* 4D 깊이 격자는 캐릭터마다 다르다(실루엣·눈 자리에서 나온다) — 컷과 «같은 축»으로
               고른다. 없는 캐릭터(마린)는 저쪽이 몽글로 폴백하고, 그림도 위에서 그렇게 폴백한다. */
            캐릭터={캐릭터}
          />
        ) : (
          <Animated.Image
            ...
            fadeDuration={0}
            style={[s.몸, { width: 몸너비, height: 몸높이 }, {

...
export default function 마스코트({ 사건 = null, 자리 = null, 말건네기 = null, 잡담 = true, 연출 = null }) {
  ...
  /* 목소리 — 몽글이 내는 유일한 소리(「무?」 하나). **표정은 재생 속도가 만든다**
   * (유호 확정 08-25 · 판정 정본 = `lib/몽글목소리.js`). 여기서는 «어느 자리에 어느 표정»만
   * 고르고, 겹침 판정·속도 계산·미세 변주는 전부 그 lib 이 진다.
   * 🚫 표정을 여기서 숫자로 적지 않는다 — 적는 순간 정본이 두 벌이 된다. */
  const 마지막목소리 = useRef(0);
  const 목소리 = (표정 = '기본') => {
    const 답 = 목소리판정({ 표정, 지금: Date.now(), 마지막: 마지막목소리.current, 흔들: Math.random() });
    if (!답.낼까) return;

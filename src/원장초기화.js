...
export default function 원장초기화({ 토큰, 닫기 }) {
  ...
  async function 만들기() {
    if (!쓸수있나) return;
    set오류(''); set결과(null); set도는중(true);
    try {
      set결과(await API.초기화요청({ 학생번호, 토큰 }));
    } catch (e) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.inner} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.칸}>
        ...
        <TextInput
          value={학생번호}
          onChangeText={(t) => { set학생번호(t); set결과(null); }}
      ...
      {결과 && (결과.잠금해제 ? (
        ...
        <View style={s.결과}>
          <Text style={s.결과머리}>{결과.학생번호} 학생의 잠금을 풀었어요</Text>
          <Text style={s.결과꼬리}>
        ...
        <View style={s.결과}>
          <Text style={s.결과머리}>{결과.학생번호} 학생에게 알려 주세요</Text>
          <Text style={s.번호} selectable>{결과.임시번호}</Text>
          <Text style={s.결과꼬리}>
            {결과.유효분}분 동안 쓸 수 있어요. 학생이 이 번호로 들어와 자기 비밀번호를 직접 정해요.

 ...
 *
 * ■ 🔴 학생 앱에서 이걸 부르는 자리가 «0곳»인 것은 미완이 아니라 **확정이다**
 *   유호 확정 2026-08-31 「일단 준비만 해두고 나중에 필요할때 사용하자」.
 *   지금 유일한 소비자 `src/검수문.js` ⑤ 칸은 `__DEV__` 안에 있어 **프로덕션 번들에서 통째로
 *   제거된다** — 즉 학생은 이 별을 아직 한 번도 못 본다. 그게 맞는 상태다.
 *   🚫 **죽은 코드로 알고 지우지 말 것.** 자산(`assets/모션/반짝별_*.webp`)·변환 통로
 *   (`tools/반짝별변환.py`)·박자 상수까지 한 벌로 서 있고, 걸 자리가 정해지는 날
 *   `<반짝임 보임 />` 한 줄이면 끝난다. 지우면 그 한 줄이 다시 며칠이 된다.
 *   ⚠ 걸 자리가 안 정해진 까닭 = 앱의 축하는 전부 몽글이가 지고, 「어제의 나」는
 *   「신호(코랄)가 없다·위계는 밀도로 준다」가 머리말에 박혀 있고, 포인트·스토어·리그는
 *   유호님이 빼셨다. **억지로 걸면 그 설계를 깬다.**
 *   ▶ 되돌아오는 자리 = 앱 화면을 다음에 손볼 때. 갈래 셋은 그때 다시 낸다
 *   (ㄱ 말하기 제출 완료 · ㄴ 교정 답장 도착 · ㄷ 「어제의 나」에서 어제와 오늘이 이어질 때).
 */
...
/* 마지막 별이 사그라진 뒤 숨 한 번 — 이것까지가 «한 박자»다. */
const 잔여 = 100;

/**
 * 이 층의 **박자 사실**. 부르는 화면이 이 수를 «글자로 다시 적지 않는다».
 *
 * 🔴 08-27 에 검수문 ⑤ 칸이 「1.30초 · 0 → 167 → 333ms」를 손으로 박아 두고 있었다.
 *   여기 상수를 고치면 그 지면만 옛 수를 들고 서는데, **유호님이 판정하시는 자리가 바로 그 지면**이다
 *   — 옛 수를 보시며 새 박자를 판정하시게 된다(영상 레인이 같은 날 같은 병을 잡았다:
 *   「지면에 통계를 문자열로 박아 뒀다 · 지면이 거짓말한다」).
 * 🔑 가드를 세우는 대신 **갈릴 수 있는 자리를 없앤다** — 수는 여기 한 곳에만 산다.
 */
export const 박자 = {
  늦음들: 별들.map((b) => b.늦음),
  한별ms: 끝,
  총길이ms: 별들[별들.length - 1].늦음 + 끝 + 잔여,
};
...
export default function 반짝임({ 보임, 크기 = 120, 줄임 = false, 끝나면 = null }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => { if (마침.current) 마침.current(); }, 박자.총길이ms);

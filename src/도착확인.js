...
import { 로그읽기, 기기비우기 } from './저장';
import { 밀린것 } from '../lib/제출로그.js';
...
export default function 도착확인({ 돌아가기, 가기 }) {
  ...
  /**
   * 🔴 **나가기 전에 밀린 것을 센다.** 이 화면이 지우는 것은 기기에만 있는 기록이라,
   *   아직 안 올라간 발화가 있는 채로 나가면 그 발화는 **어디에도 남지 않는다**(서버엔
   *   없고 기기에선 지워진다). 몽골 회선에서 전송 실패는 예외가 아니라 상시다.
   * 🔑 재전송은 여기서 안 한다 — 밀린 것을 올리는 자리는 말하기 화면 한 곳이고(`밀린것`
   *   루프), 여기에 둘째 통로를 내면 같은 항목이 두 곳에서 올라간다.
   */
  async function 물어보기() {
    set알림('');
    try {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.inner}>
      ...
      {확인 !== null && (확인 > 0 ? (
        <View style={s.card}>
          <Text style={s.확인머리}>아직 보내지 못한 발화가 {확인}건 있어요</Text>
          <Text style={s.note}>
            인터넷에 연결하고 말하기 화면을 한 번 열어 주세요 — 밀린 것이 그때 올라가요.
            {'\n'}지금 나가면 그 발화는 어디에도 남지 않아요.
          </Text>

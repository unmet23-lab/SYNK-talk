      ...
      beforeSend(event) {
        try {
          ...
          /* 합성/실사용을 가르는 태그. 실유저가 온 뒤 통계를 가를 유일한 열쇠다.
             🔴 **키가 ASCII 여야 한다 — 한글 키는 Sentry 가 조용히 버린다**(2026-08-26 실측).
                같은 이벤트에 `출처_한글시험` 과 `origin_ascii_test` 를 나란히 실어 보냈더니
                도착지에 **ASCII 쪽만** 남았다. 이벤트 자체는 200 으로 통과하고 태그만 사라지므로
                증상이 없다 — 그래서 08-15 부터 08-26 까지 이 태그는 **한 번도 존재하지 않았다**
                (그동안 아무도 도착지를 안 봤다). 형제 사고 = memory `workflow-schema-ascii-keys`.
             🔑 **값은 한글이어도 산다** — 같은 실측에서 값 `ascii키가_살았다` 가 그대로 도착했다.
                그래서 키만 ASCII 로 바꾸고 값은 사람이 읽는 낱말로 둔다. */
          event.tags = { ...(event.tags || {}), origin: 합성 ? '합성밟기' : '실사용' };
        } catch {

 ...
 *
 * ■ 🔑 프리로드는 **게이트를 통과한 뒤에만** 한다 (§3 프리로드 · §2 게이트 ②의 충돌 자리)
 *   발주 §147 은 「다음 항목 오디오 백그라운드 프리로드(대기 0초)」를 채택했는데, 서명 발급은
 *   그 자체로 감사 1행이고 **그 행이 승인 게이트 ②를 연다.** 켜자마자 다음 것을 당겨오면
 *   열지도 않은 항목이 전부 게이트 ②를 미리 통과한다 — 계약이 「서버가 밀면 안 된다」고 적은
 *   바로 그 구멍이 클라이언트 쪽에서 그대로 열린다.
 *   👉 그래서 **지금 항목의 청취 게이트를 통과한 순간**에만 다음 하나를 당긴다. 검수자는
 *      곧 넘어갈 참이라 대기는 여전히 0이고, 새는 항목은 **마지막 하나**로 갇힌다.
 *      🔴 0이 아니다 — 확정하고 앱을 닫으면 그 다음 한 건에 열지 않은 서명 기록이 남는다.
 *      완전히 닫으려면 감사 행이 「미리 받음」과 「열어 봄」을 갈라야 하는데 그건 §2 개정이라
 *      이 화면 밖이다(계약 개정 자리에 적어 둔다).
 *
 * ■ 🚫 재제안 금지
...
import { 큐받기, 오디오서명받기, 승인하기, 폐기하기, 폐기사유, 오류태그, 기본쪽크기 } from './검수API.js';
...
export default function 검수화면({ 토큰, 돌아가기 }) {
  ...
  const 확정가능 = !!항목 && !!항목.ai_correction_id && !!서명 && 게이트통과 && !보내는중
    && 검증전사.trim() !== '' && 교정문.trim() !== '';
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.inner} keyboardShouldPersistTaps="handled">
      ...
      {불러오는중 && <Text style={s.메모}>큐를 읽는 중이에요…</Text>}

      {!불러오는중 && !항목 && (
        <View style={s.카드}>
          <Text style={s.빈머리}>검수할 것이 없어요</Text>
          <Text style={s.메모}>
            큐는 「제출 + AI 교정」이 둘 다 있는 발화만 담아요.
      ...
      {항목 && (
        <View style={s.카드}>
          ...
          {세그먼트.length > 0 ? (
            ...
            <Text style={s.메모}>
              구간 정보가 없어 전체 재생으로만 들어요 — 아직 구간을 만드는 통로가 없어요.
              {문턱.재료 ? '' : ' 그래서 필요 시간은 하한 3초예요.'}
            </Text>
          )}
...
const s = StyleSheet.create({
  ...
  back: { paddingTop: 8 },
  backText: { fontFamily: 폰트.모노, fontSize: 11, letterSpacing: 모노트래킹.라벨, color: 색.잉크_메타 },
});

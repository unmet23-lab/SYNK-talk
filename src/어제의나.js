...
export async function 진행받기(토큰) {
  ...
  /* c14 교실 수집 ② — 목표 카드도 같은 봉투에서 갈라 담는다(한 조회 규율 그대로). */
  return { 견줌: 견줌(몸), 오늘의확인: (몸 && 몸.오늘의확인) || null, 오늘의목표: (몸 && 몸.오늘의목표) || null };
}
 ...
 * ⚠ 지연(700·780ms)은 «수가 다 오른 뒤»를 겨눈 값이다 — 흔한 두 칸(190ms×2)이 280+380=660ms
 *   에 끝나는 것을 기준으로 잡았다. 칸수가 아주 많은 날(어제 1 → 오늘 20 같은)에는 말이 먼저
 *   붙는데, 그 경우까지 맞추려면 세기 종료를 기다려야 하고 그러면 이 줄이 수에 «묶여» 수가
 *   없는 날에도 지연을 받는다 — 흔한 쪽을 맞추고 드문 쪽을 버렸다. */
    ...
    <View style={s.wrap}>
      ...
      {값 ? (
        <Animated.View style={[s.카드, 카드등장]}>
          ...
          {값.교정재발화.오늘 ? (
            <뒤따르는줄 style={s.고리} 지연={700}>어제 받은 교정을 오늘 다시 말했어요.</뒤따르는줄>
          ) : null}

          {늘었다 ? <뒤따르는줄 style={s.늘어남} 지연={780}>{늘었다}</뒤따르는줄> : null}
        </Animated.View>
      ) : null}

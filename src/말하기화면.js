...
function 녹음카드({ step, 제시문, 안내, 선택지, 선택차원, 텍스트병기, date, 로그, 기록추가, 완료, prompt_id }) {
  ...
  const 남기기 = async (status) => {
     ...
     * 🔴 **안 고른 채 말한 학생의 행은 지금 통로가 없다.** 계약이 `payload.position` 을 무조건
     *   필수로 걸어 `skipped`·`rejected_all` 갈래가 원리상 검증을 못 지난다(`lib/선택로그.js` 의
     *   같은 자리 · 대기열 P1 의 계약 판정 1건). 여기서 0 이나 -1 로 지어내지 않는다 —
     *   그러면 「안 골랐다」가 「첫째를 골랐다」로 적히고, 그 오독은 어디서도 안 빨개진다. */

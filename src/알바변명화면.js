...
/* NPC 사장님 — **이 게임의 압박 게이지가 곧 사장님 연출이다**(발주서 §4-3 「남은 시간은 숫자
 * 시계가 아니라 사장님의 표정·자세로 보여준다」 · 게임층 설계 §4 규격 3). 실시간 3단 전환이
 * 허용된 자리는 4모듈 중 여기 하나뿐이고, 문턱은 `lib/NPC연출` 상수 하나가 진다. */
import NPC from './NPC.js';
import { 압박상태, 전이상태 } from '../lib/NPC연출.js';
...
/** 녹음 상한(초) — 이 게임의 규칙값이다. **한 곳에 둔다**: 녹음카드와 사장님 압박 문턱이
 *  같은 수를 봐야 「남은 시간을 표정으로」가 성립한다(둘이 갈리면 사장님이 엉뚱한 때 돌아선다).
 *  🚫 숫자는 화면에 안 뜬다(`숫자숨김`) — 읽히는 시계는 회피 지연 축을 오염시킨다(§4-3). */
const 녹음상한초 = 30;
...
export default function 알바변명화면({
  ...
  /* 압박 3단 — 녹음 단계에 들어선 순간부터 흐른다(§4-3 ③ 「신호음 → 300ms → 녹음」이라
   * 단계 전환이 곧 녹음 시작이다). 다시 시도로 이 단계에 재진입하면 처음부터 다시 흐른다.
   * 🔑 rAF 를 쓰지 않는다 — 탭 스로틀·저전력에서 통째로 죽는다(마스코트 생명감 v3 실측이
   *   그 자리에서 상태 기계 셋을 잃었다 · 「생명감 상태 기계에 rAF 금지」). */
  useEffect(() => {
    if (단계 !== '녹음') { set압박(전이상태('시작')); return undefined; }
    const 시작 = Date.now();
    set압박(전이상태('시작'));
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.inner} keyboardShouldPersistTaps="handled">
      ...
      {/* 사장님 — 전화 저편의 상대역. 상황·녹음·끝을 통틀어 «한 자리»에 산다:
          자리를 옮기면 학생 눈이 그 이동을 좇아 압박이 아니라 움직임을 읽는다.
          🔴 상태만 바뀌고 숫자·게이지는 어디에도 안 붙는다(규격 3 · 모호함이 설계). */}
      <View style={s.사장님자리}>
...
const s = StyleSheet.create({
  ...
  /* 사장님 자리 — 카드 위 한 자리에 고정(옮기면 압박이 아니라 «움직임»으로 읽힌다). */
  사장님자리: { alignItems: 'center', marginBottom: 4 },

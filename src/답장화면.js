...
/* 마스코트 — 이 화면이 «보는 화면»이라 캐릭터가 산다(생명감 설계 §5 배치 = 홈·답장 ·
   ...
   배선 회귀 tests/마스코트배선.test.js 가 그 새 판을 지킨다.
      ...
      {교정 ? (
        <교정카드틀>
          ...
          {교정.growth_note && 교정.growth_note.text ? (
            <>
              {/* 🔑 꼬리를 위와 가르는 금 — 08-25 Loom 굽기 · 09-01 반입. 여태 1px 실금이었고
                  요소사전 1-b 가 러닝 스티치를 「실금의 실물판」으로 적어 둔 그 자리다.
                  🔑 이 조각에 **유채가 0.000%** 다 — 이 화면의 신호 1점은 오류 태그라는 R1 규율을
                     그대로 지킨다(코랄은 이 앱에서 「지금 녹음 중」이라는 낱말이다). 그리고
                     이 화면의 「🚫 면·배지를 안 만든다」에도 안 걸린다 — 실 한 줄이지 면이 아니다.
                  🔑 상자를 안 두르고 **Fragment** 로 형제만 늘린다 — `카드` 의 gap 14 를 그대로 타야
                     위아래 사이가 다른 칸들과 같아진다(View 로 감싸면 그 칸만 따로 논다). */}
...
const s = StyleSheet.create({
  ...
  /* borderTop·paddingTop 을 걷었다 — 금은 이제 `부품 구분선`(구운 펠트 실땀)이 내고,
     그림이 제 여백을 쥐고 있어 padding 을 함께 두면 사이가 두 배로 벌어진다(검수화면과 같은 손질). */
  꼬리: {

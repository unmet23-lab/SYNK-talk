...
export default function 나침반화면({ 토큰, 돌아가기 }) {
  ...
  return (
    <ScrollView style={s.wrap} contentContainerStyle={s.inner} keyboardShouldPersistTaps="handled">
      ...
      {!세션 && (
        <View style={s.카드}>
          <Text style={s.요약}>학생번호를 넣고 열면 이번 시즌 질문이 뜹니다.</Text>
          <TextInput
            style={s.입력}
            value={학생번호입력}
      ...
      {세션 && (
        <View style={s.카드}>
          ...
          <Text style={s.메모}>
            {시즌회차
              ? '이번 시즌 나침반이에요 — 5년 뒤의 나를 다시 보고, 이번 시즌 목표를 한 줄 적어요.'
              : '입학 나침반이에요 — 세 가지를 처음 적고, 이번 시즌 목표를 한 줄 더해요.'}
          </Text>
          {세션.이미적힘 && (
            <Text style={s.메모}>
              이 시즌은 이미 적혀 있어요({세션.이미적힘}) — 다시 저장하면 그 자리를 고쳐요.
            </Text>
          )}
        </View>
      )}

...
async function main() {
   ...
   * `--지금 <ISO>` = 창 앵커 고정 — 왕복시험이 한 실행에서 여러 번 재는 사이 창이 미끄러지면
   * 창 밖으로 빠진 옛 판정 1건이 «옳은데 적색»(off-by-one 얼굴)으로 나온다. 질의는 하한만
   * 걸므로(`>= 시작`) 앵커 뒤에 만든 판정도 창 안이다. 틀린 값은 조용히 now() 로 떨어지지
   * 않고 죽는다 — 조용한 미적용은 통과의 얼굴이다. */
  const 지금인자 = 값('--지금', null);
  if (지금인자 && Number.isNaN(new Date(지금인자).getTime())) die(`--지금 이 시각이 아니다: ${지금인자}`);
  const 지금 = 지금인자 ? new Date(지금인자) : new Date();

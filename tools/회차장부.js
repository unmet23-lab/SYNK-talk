...
const 잡이상수 = (r) => 숫자(r.실패) + 숫자(r.타임아웃) + 숫자(r.전송오류) + 숫자(r.상태없음)
                     + 숫자(r.발사실패) + 숫자(r.유실);

/* 스스로 아물었나 — 이상 «이력»과 이상 «상태»를 가른다 (2026-08-24 · F482 무늬의 처방).
 *
 * ■ 왜 — 실측: transcribe-batch 가 08-21 새벽 타임아웃 1회를 낸 뒤 505회 연속 성공했는데,
 *   7일 창 합산 판정이라 사흘째 🔴 였다. 상시 경보는 진짜 경보를 죽인다(F482 — 「진짜 밀린
 *   날에 무시당한다」). 이 저장소의 잡들은 멱등 + 짧은 주기라 한 회차가 끊겨도 다음 회차가
 *   잇는 구조고, 그 자기 치유를 판정이 알아야 경보가 어휘를 지킨다.
 * ■ 조건 둘 다 — ①마지막 발사가 마지막 이상 «뒤»다(그 뒤로도 돌았다 — 멈춘 잡을 아묾으로
 *   읽으면 죽음이 회복으로 보인다) ②마지막 이상이 24시간보다 옛날이다(매일 잡도 최소 한
 *   주기를 재발 없이 보였다). 재발하면 마지막이상이 갱신돼 그 즉시 생이상으로 돌아온다.
 * ■ 반환 — null(이상 이력 자체가 없다) · true(아묾) · false(생이상). 못 읽는 시각은 생이상
 *   쪽으로 접는다 — 새는 방향이 「통과」면 안 되는 자리다. */
const 아묾문턱ms = 24 * 3600 * 1000;
function 아물었나(r, 지금 = Date.now()) {
  if (!잡이상수(r)) return null;
  const 이상t = Date.parse(r.마지막이상 || '');
  const 발사t = Date.parse(r.마지막발사 || '');
  if (!Number.isFinite(이상t) || !Number.isFinite(발사t)) return false;
  return 발사t > 이상t && (지금 - 이상t) > 아묾문턱ms;
}

function 요약찍기(행들, 지금 = Date.now()) {
  if (!행들.length) {
  ...
  for (const r of 행들) {
    ...
    const 이상 = 잡이상수(r);
    const 아묾판 = 아물었나(r, 지금);
    if (아묾판 === true) 아묾.push({ jobname: r.jobname, 이상, 마지막이상: r.마지막이상 });
    else 생이상 += 이상;
    말(`  ${이상 ? (아묾판 ? '🟡' : '🔴') : '✅'} ${r.jobname}${아묾판 ? ' — 아묾(이상 뒤 24시간+ 재발 0 · 발사는 계속됐다)' : ''}`);
...
async function main() {
  ...
  if (이상행.length) {
    ...
    /* 타임아웃의 어휘 — pg_net 의 「Timeout of 5000 ms」는 **응답을 5초 안에 못 받았다**지
     * 함수가 죽었다가 아니다(사유의 시간 분해가 어디까지 갔는지 말해준다 — DNS·TCP 를 지나
     * HTTP 왕복에서 끊겼으면 발사는 나갔고 함수는 마저 돌았을 수 있다). 이 저장소의 잡은
     * 전부 멱등이라 설령 중간에 죽어도 다음 회차가 잇는다 — 그래서 cron 쪽 timeout 을 늘리지
     * 않는다(응답은 어차피 아무도 안 읽는다 · radio-promote 머리말). 진짜 신호는 «효과»다:
     * transcribe 면 대기(전사 안 된 행)가 안 줄어드는 것 — 그건 위 분모 줄이 든다. */
    if (이상행.some((r) => r.outcome === '타임아웃')) {
      말('  ⓘ 타임아웃 = 응답을 못 기다렸다는 뜻(발사가 나갔다면 함수는 마저 돌았을 수 있다) — 멱등 잡은 다음 회차가 잇는다. 죽음의 신호는 타임아웃 «연속»과 대기 잔존이다.');
    }
  }
  ...
  if (json) {
    json내기({
      ...
      /* 판정 어휘는 0·1·2 로 닫혀 있다(형제 rot-check 의 `아는판정` — 값을 늘리면 그쪽이
       * 「못 잼」을 낸다). 아묾은 판정 0 이되 별도 칸으로 싣는다 — 정보는 남고 경보는 접힌다. */
      판정: (안적힘 > 0 || 이상 > 0) ? 1 : 0,
      안적힘, 이상, 아묾,
      요약: 값.요약 || [], 대조: 값.대조 || [],
  ...
  if (아묾.length) {
    console.error(`[회차장부] ✅ 생이상 0 · 아묾 ${아묾.length}잡(${아묾.map((a) => `${a.jobname} ${a.이상}건`).join(' · ')})`
      + ' — 이상 «이력»은 위 요약 줄에 남아 있다(재발하면 그 즉시 생이상으로 돌아온다).');
    return 0;
  }
...
module.exports = { main, 아물었나, 아묾문턱ms };

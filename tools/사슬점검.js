 ...
 *   잰다   = 순수 계산층 — `lib/학습자상태.js`(8축) · `lib/성과회수.js`(관측·창별·연결률).
...
const { 학습자상태, 창일수, 쓰는사건: 상태가쓰는사건 } = require('../lib/학습자상태.js');
...
function 합성이력() {
  ...
  for (let d = 0; d < 날수; d += 1) {
    ...
    행들.push({ event_id: iv, event_type: 'intervention.delivered', occurred_at: t(d + 0.05),
      intervention_id: `IV-${d}`, policy_ver: '오늘과제.v3', estimator_version: '학습자상태.v6' });
    개입들.push({ iv, d });
...
function 판정(행들, 기준시각, 자세히 = false) {
  ...
  /* ② 축 계산 — 8축 중 몇 개가 값을 내나. 여기가 이 도구의 급소다. */
  const 상태 = 학습자상태(행들, { as_of: 끝시각 });

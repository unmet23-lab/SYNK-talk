...
/* 🎯 조준 축은 **없다** — 파일을 읽어 채점할 뿐이다(자격증명·네트워크 0). `--운영` 을 안 받는다.
 * 🔴 셋 다 자기 낱말이다 — 이 도구는 자식 프로세스를 안 부른다. `--fixture`·`--대조` 는
 *   `main(argv)` 안의 클로저(`인자값`)가 읽어서 #Q112 추출기엔 「자식 인자」로 보였다(#Q113 에서 닫음).
 * ⚠ 위치 인자(출력 JSON 경로)는 `--` 로 안 시작하니 이 판정에 안 걸린다. */
const 아는플래그 = ['--json', '--fixture', '--대조'];
...
const 옛글자 = new RegExp('[\\u3040-\\u30FF\\u3400-\\u4DBF\\u4E00-\\u9FFF\\uF900-\\uFAFF\\uFF66-\\uFF9F\\u{1AFF0}-\\u{1B16F}\\u{20000}-\\u{3FFFF}]', 'u'); // [vNEXT] 리뷰 P2-① — lib/옛글자.js 와 같은 확장(사본 대조=tests/옛글자.test.js)
...
function scoreOne(fx, out) {
  ...
  // 기대 태그를 채워도 오류없음이 섞이면 상충된 판정이다. 미검출과 통과가 동시에 서지 않는다.
  r.판정.태그 = !r.미검출 && (기대충족 || Boolean(맞은대안));
  r.대안통과 = Boolean(맞은대안);
...
function main(argv) {
  /* 🔴 값 자리 판정보다 **앞**이다 — 뒤에 두면 모르는 낱말이 값 자리 셈에 먼저 섞여서,
   *   나가는 사유가 「출력 파일을 안 줬다」로 층을 갈아탄다(사람은 경로를 고치러 간다). */
  const 플래그오류 = 인자게이트('eval채점', argv, 아는플래그);

...
async function main() {
  ...
  /* 🔴 **`맥락` 을 붙여서 부른다 — 안 붙이면 활성되는 날 ②가 400 으로 죽는다** (2026-08-22).
   *   `deliver` 의 §3-1 게이트는 활성 «전»에는 누락을 봐 주지만(`맥락 !== null` 일 때만 검사),
   *   활성 함수(`engine.gen_active_from()`)가 서는 순간 **누락도 400** 이 된다(fail-closed).
   *   그러면 이 시험은 그날 처음으로, 그것도 배달이 아니라 **인자 하나** 때문에 빨개진다 —
   *   활성 조각을 미는 사람에게는 「내 조각이 배달을 깼다」로 보이는 빨강이다.
   * 🔑 활성 뒤 cron 이 부를 모양 그대로 부른다(`&맥락=배치`) — 시험이 라이브와 다른 문장을
   *   쓰면 그 차이만큼 못 재는 것이 생긴다.
   * ⚠ `점검` 은 붙이지 않는다: 그 갈래는 `배달하기` 를 아예 안 지나 맥락이 뜻이 없고,
   *   cron 의 점검 잡도 안 붙인다(L0 §10745). */
  const 호출 = async (질의 = '', 키 = service) => {
    const 맥락질의 = 질의.includes('맥락=') || 질의.includes('점검')
      ? 질의
      : `${질의}${질의.includes('?') ? '&' : '?'}${new URLSearchParams({ 맥락: '배치' })}`;
    for (let i = 0; ; i++) {
      const r = await fetch(`https://${ref}.supabase.co/functions/v1/deliver${맥락질의}`, {

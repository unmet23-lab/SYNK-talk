 ...
 *   메시지 16건이 이 창들 사이에 앉아 승격 6 + 제외 10사유를 만든다(각 사유 정확히 1건 —
 *   단 「비링크」는 앞선 인제스트 왕복시험의 잔존 원장 행도 세므로 응답 층에선 ≥1 로 읽는다).
 */
...
const 분전 = (n) => new Date(Date.now() - n * 60_000).toISOString();
...
async function main() {
  ...
  const ch = (n) => `UC-${회차}-${n}`;
  /* E 채널의 겹침은 이력이 만든다(반박 ⑤ 모양 그대로): E1 링크가 -60분~-20분에 살았고
  ...
  await sql(`
    ...
    values ('${ch('A')}','${id.A}'::uuid,'승격왕복', now() - interval '3 days', null, '${판}'),
           ('${ch('B')}','${id.B}'::uuid,'승격왕복', now() - interval '3 days', null, '${판}'),
           ('${ch('D')}','${id.D}'::uuid,'승격왕복', now() - interval '3 days', null, '${판}'),
           ('${ch('E')}','${id.E1}'::uuid,'승격왕복', now() - interval '60 minutes', now() - interval '20 minutes', '${판}'),
           ('${ch('E')}','${id.E2}'::uuid,'승격왕복', now() - interval '40 minutes', null, '${판}')`);
  ...
  확인('승격 6건 — 퀴즈 2(재도전 쌍)·선호 1·정서 1·체크인 1·목표 1', 첫.몸.승격 === 6, 첫.몸);
  const 제외 = 첫.몸.제외 || {};
  for (const 사유 of ['중복응답', '무동의', '해석불가', '마감후', '하루접힘', '자리없음', '스냅샷불량', '링크겹침', '인자없음']) {
    확인(`제외 「${사유}」 = 1 — 그 픽스처 하나만 걸렸다`, 제외[사유] === 1, { [사유]: 제외[사유] });
  }
  확인('제외 「비링크」 ≥ 1 — 앞선 인제스트 왕복 잔존 행도 세므로 하한으로 읽는다', (제외['비링크'] || 0) >= 1, { 비링크: 제외['비링크'] });

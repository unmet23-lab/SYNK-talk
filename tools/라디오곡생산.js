...
/* 1시간(3600초)을 여덟 벌로 — 한 벌이 ≈450초라 8벌이면 3,600초에 딱 앉는다.
 * 결 배분은 «치우치지 않게» 돌린다(같은 결이 연달아 나오면 반복이 더 들린다). */
const 결차례 = ['시티팝', '차분', '전자', '시티팝', '차분', '전자', '시티팝', '차분'];
...
async function 한벌(n, 결, 낼곳, k, 규격) {
  ...
  const id = `synk-radio-${String(n).padStart(2, '0')}`;

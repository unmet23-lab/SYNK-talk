 ...
 * 🔴 **`--코드` 갈래는 옛 계정 전용이다**(F269 · 2026-08-09) — 코드를 새로 발급하는 도구는
 *   더 없다. 학생 계정은 §4-1 대로 첫 등록에서만 서므로, 새 회차는 `⓪ 새학생` 갈래를 쓴다
 *   (명부는 `node tools/명부등록.js SYNK-T0N --전화 "…" --적용`, 계정은 앱의 first-login).
 *
 * ■ 왜 이게 필요한가
...
/* 🎯 조준 축이 **있다**(골격 한 겹 건너) → `공용플래그` 를 편다.
 * 🔑 `--운영승인` 도 **자기 낱말**이다 — 골격이 argv 에서 직접 읽고(`왕복골격.js` 열기),
 *   `운영승인가능:true` 인 도구는 저장소에서 이것 하나뿐이다. 도구 자신의 소스엔 그 낱말이
 *   안 나오므로 기계가 못 세운다 — 여기 사람이 적는 자리다(빼면 승인 통로가 통째로 죽는다).
 * ⚠ `--적용 --판 --확정자` 는 **자식 인자**다(`execFileSync` 로 형제 도구에 넘긴다) — 넣지 않는다.
 *   넣으면 그 낱말들이 이 도구의 CLI 에서 통과해 버린다(#Q112 ㉠ 함정 · F594). */
const 아는플래그 = [...공용플래그, '--새학생', '--운영승인'];

async function main() {
  const args = process.argv.slice(2);
  /* 게이트가 `골격.열기()` 앞이다 — 이 시험은 리허설 DB 에 지울 수 없는 행을 남긴다. */
  const 플래그오류 = 인자게이트('왕복시험', args, 아는플래그);
  if (플래그오류) die(플래그오류);

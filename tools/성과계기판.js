...
/* ㉡ «본인 동의율» — 엔진점수 둘째 축(세층합류 §6 · 09-01). 재료는 이미 이 질의에 실려 온다:
 * `쓰는사건` 이 학습자상태 재료를 펴 넣고, 확인축이 `estimate.responded` 를 읽으므로. 별도
 * 왕복 0 — 같은 행 묶음에서 자만 하나 더 센다. 셈의 정본은 lib 하나(판별·마지막 답 접기). */
const { 동의율표 } = require('../lib/동의율.js');
/* 읽기 커버리지(세층합류 §3 · c15) — 같은 질의의 task.assigned 행에서 «이 과제가 세 층에서
 * 무엇을 읽었나»를 센다. 재료·왕복 0 추가(쓰는사건에 task.assigned 가 이미 있다 — 리듬축 재료). */
const { 읽기커버리지 } = require('../lib/읽기기록.js');
...
function 계수내기(행들, 옵션) {
  ...
  return {
    ...
    기준시각,
    기준선창,
    /* ㉡ 동의율(세층합류 §6) — 같은 행 묶음에서 센다. 창 = 이 계기판의 질의 창(누적 아님):
     * 라벨엔 시효(두 시즌)가 있어 창 값이 자연스럽고, 창이라는 사실은 화면 각주가 든다. */
    동의율: 동의율표(행들),
    읽기: 읽기커버리지(행들),
...
function 요약찍기(계수, { ref, 잘림 }) {
  ...
  /* ㉡ 동의율 — 판별로만 낸다(판 가로질러 합산 금지). n = 맞다 + 아니다 꼴(분모와 함께). */
  const 동 = 계수.동의율;
  if (동 && 동.총행수 > 0) {
    for (const [판, 칸] of Object.entries(동.판들)) {
      console.log(`  ㉡동의율 ▸ ${판}  ${값표시(비율(칸.맞다, 칸.n))} (n ${칸.n} = 맞다 ${칸.맞다} + 아니다 ${칸.아니다}`
        + ` · 학생 ${칸.학생수} · 접힘 ${칸.접힘수} · 뒤집힘 ${칸.뒤집힘수})`);
    }
  } else {
    console.log('  ㉡동의율  라벨 0 — 아직 아무도 안 물었거나 안 답했다(0 이 아니라 «없음»이다)');
  }
  /* 읽기 커버리지 — 기록 있는 행만 깊이를 안다(기록 없음 = 옛 행·폴백 — 0 으로 안 접는다). */
  const 읽 = 계수.읽기;
  if (읽 && 읽.총행수 > 0) {
    const 분포 = Object.entries(읽.깊이분포).map(([d, n]) => `${d}층 ${n}`).join(' · ');
    console.log(`  읽기기록  배정 ${읽.총행수} = 기록 ${읽.기록있음} + 없음 ${읽.기록없음} · 깊이(기록분) ${분포}`);
  } else {
    console.log('  읽기기록  배정 행 0 — 창 안에 생성 착지가 없다');
  }
...
<h2>㉡ 본인 동의율 <span class="muted">(엔진점수 둘째 축 · 이 계기판의 질의 창 안)</span></h2>
${계수.동의율 && 계수.동의율.총행수 > 0 ? Object.entries(계수.동의율.판들).map(([판, 칸]) => `<h3 style="font-size:13px;font-weight:500;margin:16px 0 6px">${esc(판)} <span class="muted">· n ${칸.n} = 맞다 ${칸.맞다} + 아니다 ${칸.아니다} · 학생 ${칸.학생수} · 접힘 ${칸.접힘수} · 뒤집힘 ${칸.뒤집힘수}</span></h3>
<table><thead><tr><th>축</th><th>키</th><th class="n">맞다</th><th class="n">아니다</th><th class="n">동의율 / 분모</th></tr></thead>
<tbody>${Object.entries(칸.축별).map(([축, a]) => Object.entries(a.키별).map(([키, c]) => `<tr><td>${esc(축)}</td><td>${esc(키)}</td><td class="n">${c.맞다}</td><td class="n">${c.아니다}</td><td class="n"><b>${값표시(비율(c.맞다, c.맞다 + c.아니다))}</b><span class="muted"> / ${c.맞다 + c.아니다}</span></td></tr>`).join('')).join('')}</tbody></table>`).join('')
    : '<p class="note">라벨이 아직 없습니다 — 0이 아니라 «안 물었다»입니다(무응답은 행이 없어 분모에 들어오지 않습니다).</p>'}
<p class="note">🔴 <b>동의율은 정확도가 아니라 본인 확인의 비율입니다.</b> 학생의 「맞아요」는 추정기의 정답 라벨이되 참값 보장이 아니라서(자기보고 한계), 셋째 눈은 시즌 회고의 강사입니다. 같은 (학생·축·키)의 여러 답은 <b>마지막 답 하나</b>로 접습니다(접힘·뒤집힘이 그 수) · 판(estimator_version)을 가로질러 합산하지 않습니다 — 다른 판의 답은 다른 질문에 대한 답입니다.</p>

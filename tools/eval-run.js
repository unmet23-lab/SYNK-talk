 ...
 *   판과 대조한다(이중 자물쇠).
 *   ⚠ 단 그 게이트는 **이 시험이 부르는 함수로 한정**한다(`게이트함수들`). 2026-08-12 실측:
 *   `correct` 를 갓 재배포하고도 남의 트랙이 미뤄 둔 `deliver·progress·tasks` 의 낡음 때문에
 *   평가가 통째로 막혔다. 처방이 「남의 함수 3개를 대신 재배포하라」가 되는데, 그 저장소엔
 *   그때 깨진 미커밋(`lib/보고서교정제출.js`)이 있어 **따를 수 없는 처방**이었다(F103).
 *   좁히는 방향은 늘 「통과」로 새므로 `tests/왕복골격.test.js` 가 「소스가 부르는 함수 ⊆
 *   게이트함수들」을 같이 잰다 — 이 시험이 다른 함수를 부르게 되면 거기서 빨개진다.
...
/* 배포판 게이트를 이 시험이 «실제로 부르는» 함수로 한정한다 — 이 시험은 `correct` 하나만
 * 부른다. 넓히려면 부르는 자리를 먼저 늘려야 한다(회귀가 그 순서를 강제한다). */
const 게이트함수들 = ['correct'];

const { 공용플래그, 인자게이트 } = require('../lib/플래그.js');   // 모르는 낱말 거절(공용 판정 · F435)

/* 🎯 조준 축이 **있다** — `--원격` 갈래가 `lib/왕복골격.js 열기()` 를 지나고 그 안에서
 *   `자격증명.읽기()` 가 `--운영` 으로 과녁을 갈아탄다(한 겹 건너). 그래서 `공용플래그` 를 편다.
 * 🔴 여섯 낱말이 **전부 자기 것**이다 — 이 도구는 자식 프로세스를 한 번도 안 부른다.
 *   #Q112 의 추출기는 `인자('--출력')` 꼴(도우미가 argv 를 «닫아 쥔» 모양)을 못 따라가
 *   `--출력 --프롬프트 --표본 --맥락` 넷을 「자식 인자」로 봤다. 그 처방대로 선언에서 뺐다면
 *   머리말이 인쇄하는 그 명령이 죽는다(F103) — #Q113 이 추출기 쪽에서 닫았다. */
const 아는플래그 = [...공용플래그, '--로컬', '--원격', '--출력', '--프롬프트', '--표본', '--맥락'];
...
function 항목만들기(id, 글) {
  ...
  if (검증.사유) {
    항목.검증사유 = 검증.사유;
    /* 🔴 못 읽은 응답의 **원문을 남긴다** — 안 남기면 `형식밖` 이 진단 불가다. 잘린 JSON
     *   (토큰 고갈)과 울타리·군말(형식 이탈)은 처방이 정반대인데(예산 재산정 vs 프롬프트 문구)
     *   파일에는 사유 한 낱말만 남아 둘이 **같은 모양**이 된다. 08-12 첫 API 기준선이 실제로
     *   그 상태였다 — 형식밖 6건의 원인을 파일만 봐서는 아무도 못 말한다.
     * 🔑 조건이 `고친문장` 부재인 것은 채점기의 「무응답」 술어와 **같아야** 해서다: 채점에서
     *   빠지는 항목이 곧 원문이 남는 항목이다. 파싱은 됐는데 제품 규칙이 버린 것
     *   (`계약밖태그`·`교정문없음`)은 원형이 이미 실려 있으므로 원문을 겹쳐 적지 않는다. */
    if (항목.고친문장 === undefined) 항목.원문 = String(글 == null ? '' : 글);
  }
  return 항목;
}
...
async function main() {
  /* 🔴 게이트가 «맨 앞»이다 — 이 도구는 뒤에서 벤더에 **유료 제출**을 낸다. 오타 하나에
   *   픽스처를 읽고 리허설을 열고 나서 죽으면, 그때는 이미 돈이 나간 뒤일 수 있다. */
  const 플래그오류 = 인자게이트('eval실행', process.argv.slice(2), 아는플래그);
  if (플래그오류) die(플래그오류);
  const 출력경로 = 인자('--출력');
  ...
  const 저장 = (완주) => {
    ...
    const 몸 = {
      ...
      /* `_` 로 시작하는 키는 그 파일의 설명문이지 맥락 항목이 아니다 — 같이 세면 산출물 머리가
       * 「13건」이라 적는데 실제로 맥락이 붙은 것은 12건이다. 나중에 그 파일을 근거로 인용할 때
       * 분모가 하나 부풀어 있고, 그 거짓은 산출물만 봐서는 안 보인다. */
      맥락시험: 맥락경로
        ? `${맥락경로} (${Object.keys(맥락표).filter((k) => !k.startsWith('_')).length}건)`
        : null,
      항목: 항목배열,
    };

...
async function main() {
  const { e, ref, 리허설이다, sql, anon, 확인, 보고 } = await 골격.열기('라디오왕복시험', {
    사유: '이 시험은 원장(radio.chat_message)에 지우지 않는 행을 남긴다',
  });

  /* 🔑 시크릿은 **`.env` 도 본다**(2026-08-17 · F501 ③과 같은 병을 이 층에서 또 봤다).
   *   착지 도구(`tools/라디오시크릿_착지.js`)는 이 값을 `.env` 에 적는다 — Management API 가
   *   시크릿 값을 안 돌려주므로 그 파일이 **유일한 회수 통로**다(그 도구 머리말). 그런데 여기서는
   *   `process.env` 만 봤다. 그래서 이 시험을 돌리려면 사람이 매번 `.env` 를 열어 시크릿을 손으로
   *   복사해 셸에 붙여야 했고(머리말 7행이 그렇게 시킨다), 그건 철학 ㉡ 「사람 손을 늘리는 해법」이다.
   * 🔑 칸은 **과녁을 따라간다** — 리허설 `<이름>` · 운영 `<이름>_PROD`(착지 도구가 그렇게 가른다).
   *   한 칸으로 뭉치면 과녁을 바꾼 날 반대쪽 값으로 전건 401 이고 그 증상은 「조용함」이다.
   * 🔑 `lib/자격증명.js` 를 직접 안 든다 — 골격이 이미 병합해 둔 `e`(파일 ← process.env 순)를 쓴다.
   *   처음에 그 모듈을 직접 require 했다가 `tests/왕복골격.test.js` 가 막았다: 왕복 도구가 골격을
   *   우회하면 다음 수리가 몇 도구에만 붙는다(그 회귀의 존재 이유 그대로). 가드가 옳았다.
   *   📌 2026-08-18(F533): 그 가드는 이제 **코드만** 판정한다 — 그래서 이 줄이 모듈 이름을
   *     낱말 그대로 적을 수 있다. 앞 판에서는 「자격 모듈」로 에둘러 적어야 했고, 그러면
   *     정작 이 사실을 찾으려는 다음 사람이 그 낱말로 grep 해도 못 찾았다. */
  const 칸 = 리허설이다 ? 'RADIO_INGEST_SECRET' : 'RADIO_INGEST_SECRET_PROD';
  const 비밀 = e[칸] || e.RADIO_INGEST_SECRET || '';
  if (!비밀) {
    console.error(`[라디오왕복시험] ${칸} 가 없다 — 배포에 넣은 값과 같은 것을 주어야 한다.`);
    console.error(`       ① \`.env\` 의 ${칸} (착지 도구가 적는 자리) 또는 ② 셸에서 ${칸}=… 로 준다.`);
    console.error('       착지 자체가 안 됐으면: node tools/라디오시크릿_착지.js --적용');

 ...
 *   $env:GENERATION_MODEL='claude-opus-5'; node tools/과제생성평가.js --원격   # 벤더 비용을 쓴다(회전 1벌 ≈ $2.25)
 *     · 재개형 — 이미 «성공»한 행은 건너뛴다. 판(시험지_해시·prompt_ver)이 다른 결과 파일에는 이어 붙지 않고 거절한다.
 *     · ⚠ 08-21 까지 이 줄은 「⛔ 지금은 거절」이었다 — #6 이 열리며 사실이 아니게 됐다(2·3회전이 이 통로로 돌았다).
...
if (값('--채점')) {
  ...
  /* 모델이 «실제로 본» 상태 블록 — ⑧상태활용은 이걸 안 보면 원리상 못 매긴다. 정의는
   * lib/과제채점.js 한 벌(채점 화면 tools/과제채점.js 과 같은 것을 본다 — 두 벌이 되면 갈라진다). */
  const { 상태블록 } = require('../lib/과제채점.js');
  ...
  (async () => {
    ...
    /* 🔴 id 를 리터럴로 적지 않는다 — 「Z1~Z3」이라 박아 두면 앵커가 늘어도 화면은 옛 범위를
     * 말하며 조용히 초록이다(채점표 v3 에서 Z4·Z5·A4 가 늘었고 이 줄이 그 자리였다). */
    console.log(`     0점 조건 ${평가.앵커['0점조건'].map((z) => z.anchor_id).join('·')} · 1점 앵커 ${평가.앵커['1점앵커'].map((a) => a.anchor_id).join('·')} · 중간이면 ${평가.앵커.중간사례기준}`);
...
if (값('--판정')) {
  ...
  const 결과 = 읽기(파일);
  const 시험지 = 읽기(경로.시험지);
  /* 판정의 산술은 «결과한벌» 하나다 — 차단기 회귀·왕복시험 A7 과 같은 함수(각자 조립하면
   * 한 곳이 얕아진다 — A7 이 existsSync 로 «미채점 80 = 1벌»을 낸 것이 심문 G2). */
  const 현 = 현행판();
  const 판 = 평가.결과한벌({ 결과, 시험지, 전문, 현행: 현 });
  if (판.무효사유.length) {
    console.log(`[과제생성평가] ❌ 결과 파일 무효(${판.무효사유.length}건 — 한 행만 버리지 않는다 · E2):`);
    for (const s of 판.무효사유.slice(0, 20)) console.log('   · ' + s);
    process.exit(1);
  }
  const 집 = 판.집;
  console.log(`[과제생성평가] 판정 — ${path.relative(ROOT, 파일)} · 채점자 ${결과.동봉.채점자} · ${결과.동봉.시각}`);
  if (판.미채점) console.log(`  ⚠ 미채점 ${판.미채점}행 — 아래 축 숫자는 「깨졌다」가 아니라 「아직 안 쟀다」다(미채점 = 0 으로 접혀 있다)`);
  for (const k of 평가.축키들) {
    const a = 집.축[k];
    console.log(`  ${a.안잼 ? '⏸' : (a.통과 ? '✅' : '❌')} ${k.padEnd(11)} ${a.합}/${a.분모} = ${a.비율 == null ? '—' : a.비율.toFixed(2)}  (${a.안잼 ? '안 쟀다 — 이번 판에서 뺐다(통과로는 안 센다)' : '통과선 ' + 평가.통과선표기(k)})`);
  }

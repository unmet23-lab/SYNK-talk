...
/** 집계 한 줄 — **자기 분모를 밝힌다**(F207 「초록은 분모와 함께 읽는다」 · 2026-08-17).
 *
 * 🔴 **발단은 종료 게이트였다.** `/close` 3-b 가 이 도구를 **인자 없이** 부르는데, 그러면 과녁이
 *   `.env` 의 리허설로 잡혀 「다름 0 · 미측정 0 · 전체 16」 완벽한 초록이 나온다. 같은 순간 운영은
 *   낡음 1 이었다(실측 2026-08-17 · 같은 트리). 그 초록을 받은 세션은 「배포 확인됨」을 적고 닫는다 —
 *   **한 판만 본 초록이 두 판의 초록처럼 읽힌다.** 새는 방향은 언제나 통과다.
 * 🔑 프로즈(스킬 문서)만 고치지 않는 이유: 안 읽으면 안 돈다. **도구가 스스로 말하면 돈다**
 *   (CLAUDE.md 신뢰성 — 장치는 스스로 발화하지 않으면 안 돈다).
 * 🔑 순수 함수인 이유: 회귀가 문장까지 잰다. 같은 판정을 main 안에 적으면 회귀가 못 닿는다.
 *
 * @param 판 `배포빚.판이름()` 의 결과 — **`null` 이면 과녁을 못 갈랐다는 뜻이라 둘 다 「안 본 판」이다**
 *          (아래 filter 가 자연히 그렇게 낸다 — 모름을 통과로 접지 않는다).
 */
function 집계문(판, 수, 판들 = 배포빚.판들) {
  const 부재꼬리 = 수.부재 ? `(그중 라이브에 없음 ${수.부재})` : '';
  const 머리 = `[배포대조] ${판 || '과녁 모름'} — 다름 ${수.다름} · 미측정 ${수.못잼}${부재꼬리} · 전체 ${수.전체}`;
  const 안본 = 판들.filter((p) => p !== 판);
  if (!안본.length) return 머리;
  return `${머리}\n[배포대조] ⚠ 이 실행이 **안 본** 판: ${안본.join(' · ')}`
    + ` — 위 초록은 «${판 || '이 과녁'}» 하나의 초록이다. 두 판을 한 번에: node tools/배포빚.js`;
}
...
async function main() {
  ...
  console.log('\n' + 집계문(배포빚.판이름(ref, 자격증명.운영REF),
    { 다름, 못잼, 부재, 전체: 목록.length }));
...
module.exports = { 정규, 펴기, 평평하게, 함수들, 대조, 게이트판정, 왕복전게이트, 시각뒤처짐, 바이트로잴수있나, 한글앵커, 본체판정, 앵커살아있음, 인자판정, 아는플래그, 집계문 };

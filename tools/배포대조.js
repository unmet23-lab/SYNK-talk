...
async function main() {
  ...
  for (const slug of 목록) {
    ...
    if (!fs.existsSync(디렉터리)) { console.log(`· ${slug}  ⚠ 로컬에 없다`); 못잼++; continue; }

    const res = await fetch(`${API}/${ref}/functions/${encodeURIComponent(slug)}/body`, { headers: 헤더 });
    if (!res.ok) {
      console.log(`· ${slug}  ⚠ 미측정 — 본문 HTTP ${res.status}`);
      못잼++; continue;
    }
    ...
    if (!빠진것.length) console.log(`· ${slug}  ✅ 같다  (파일 ${Object.keys(나갈것).length})`);
    else { console.log(`· ${slug}  🔴 다르다 — 배포본에 없는 판: ${빠진것.join(', ')}`); 다름++; }
  }

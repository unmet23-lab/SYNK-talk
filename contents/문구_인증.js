 ...
 *   `줄들()` 이 병기 줄을 내기 시작한다.
 * ■ ⚠ 화면(`src/인증화면.js`)이 **이 표를 읽는다** — `contents/문구_1차.js` 의 `auth.*`
 *   `source_file` 도 이 파일을 가리킨다(`err.*` 가 `문구_오류.js` 로 간 길 그대로).
 *   `tests/문구_1차.test.js` ⑥ 이 「인용한 원문이 이 파일에 실제로 있나」를 대조하므로,
 *   `ko` 를 고치면 감수 목록의 `source_ko` 도 같은 커밋에서 고친다 — 따로 하면 그 자리에서 빨개진다.
 */

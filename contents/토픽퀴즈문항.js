 ...
 *
 * ■ 🔴 `정답` 은 화면 payload 에 안 싣는다 — 승격 «스냅샷»에는 싣는다 (08-24 실측 정정)
 *   정답 공개·60초 재도전 트리거(설계 §0-5)의 재료라 팩과 radio.quiz_round.task_snapshot 에
 *   있고, 승격이 그 스냅샷을 `engine.submissions.task_snapshot` 으로 나른다(라디오라운드
 *   `스냅` — 그 칸이 없으면 정오 파생이 원리상 불가: `lib/라디오편성.승격요약` 의 유일한
 *   대조 재료다). payload 는 보기 층(options_shown)만 열려 있어 학생 화면엔 안 간다.
 *   C0 🚫is_correct 가 금지하는 것은 정오 «라벨» 저장이지 정답 «원본»이 아니다(정오는
 *   언제나 두 칸을 맞대는 파생값 — 복습 파생층 R3 도 같은 규율 위에 선다).
 *

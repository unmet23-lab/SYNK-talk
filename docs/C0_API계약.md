...
- 🔴 **시도 제한이 이 판의 급소다** — 뒤 4자리는 1만 가지뿐이라 자릿수가 지키는 게 아니다. 뒷자리 실패만 `signup_attempts`를 올리고 **5에 이르면 잠긴다**(해제는 원장). 규격 오류는 게이트를 밟기 전이라 세지 않는다.
...
**director 토큰**으로 부른다. 입력 `{ student_code }` → 출력 `200 { ok, student_code, temp_password, expires_in_minutes: 30 }`.
...
- 함께 일어나는 일: 옛 비밀번호 무효화(난수로 갈아끼움) · `revoked_before=now()`(기존 세션 폐기) · `signup_attempts=0` · `staff_access_log` 1행(**번호는 안 적는다**).
...
> **라이브 실측 32/32** = `tools/인증왕복시험.js`(리허설 · 재실행 가능). 회귀가 못 보는 것만 잰다 — 동봉이 풀렸나 · GoTrue가 계정을 만드나 · 실패가 실제로 세지나 · **만든 계정으로 정말 로그인되나** · 학생 토큰으로 초기화가 막히나 · 평문이 DB·감사표에 없나 · **초기화가 옛 비밀번호도 죽이나** · 임시번호가 1회용인가 · **만료된 번호가 맞아도 막히나**.
>

...
🔴 **`corrected_text`도 `error_tags`도 빈 행은 목록에서 뺀다** — 강사가 판정(`verdict`)만 남긴 골든셋 행이 그 모양으로 설 수 있고, 그대로 주면 화면에 **빈 카드**가 뜬다. 위 「빈 상태」 칸이 금지한 것이 정확히 그것이다. 이것도 실측으로 못박았다(필터를 푼 판에서 빈 행이 목록 맨 앞으로 나왔다).

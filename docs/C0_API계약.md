...
2. 앱이 코드를 **대문자로 정규화**한 뒤 `signInWithPassword({ email: 소문자(코드)+'@synk.invalid', password: 대문자(코드) })` **1회**.
   🔑 정규화 규칙의 정본은 L0 §4-2다. **앱과 발급 도구가 같은 함수를 공유한다** — 두 곳에 적으면 갈라지고, 증상은 「어떤 학생만 로그인이 안 됨」이다.
...
| 앱이 반드시 채운다 | 왜 |
|---|---|
| `idempotency_key` | 오프라인 큐의 재시도를 서버가 셀 수 있게 (UUID v4) |
| `event_type` | 계약 값목록 |
...
| `task_type` | 해당되는 과업이 있으면 필수(퀴즈·제출·발화). `session.abandoned`처럼 과업이 없는 사건은 생략 |
...
2. **§7 왕복 6 통과**(Mock).
3. **c4 판정 3건**(§9) — `intervention.delivered`가 없으면 AI 개입 기록 자체가 계약 밖이다.
4. ~~Supabase 프로젝트~~ ✅ · ~~학생 로그인 방식~~ ✅ (유호님 08-05 · L0 §7).

## 9. 열린 판정

**c4 개정 3건** — 값목록에 **추가**(추가는 하위호환 · §6). 근거는 L0 §3-2·§6-1.
...
**L0 레인에 요청 2건**(그 문서는 지금 다른 세션이 편집 중이라 손대지 않았다):
- §3-2의 payload 버전 표기 `choice.selected.v1` → **`payload.ver`로 정정**(근거 §4-1).
- c3 `보류_유호확정대기.privacy_class` 문구가 낡았다 — L0 §6-2와 코어엔진 A-10 정정으로 **「열을 만들지 않는다」로 이미 해소**됐다. 내용 변경 없이 문구만 정리(c4 동반).

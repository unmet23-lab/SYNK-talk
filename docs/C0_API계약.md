...
🔴 **「첫날」은 「어제가 0」과 다르다.** 어제 있었는데 아무것도 안 낸 학생에게 `yesterday: 0`은 참이고 보여줄 값이다(오늘 1 > 어제 0이 곧 동기). 비우는 것은 **어제라는 시점이 이 학생에게 아직 없는** 경우뿐이다 — 그때 0을 보여주면 없는 과거를 지어내는 것이 된다. 판정은 그 학생의 **첫 사건 시각**이 오늘 이후인가로 한다(서버·몽골 시간대).

...
> **라이브 실측 16/16** = `tools/인증왕복시험.js`(리허설). 회귀가 못 보는 것만 잰다 — 동봉이 풀렸나 · GoTrue가 계정을 만드나 · 실패가 실제로 세지나 · **만든 계정으로 정말 로그인되나**.
...
| 원장 「비밀번호 초기화」 | **다음** — §4-4와 같은 함수에 붙는다. ⛔ 먼저 답할 것: 임시번호 30분 만료를 **무엇이 강제하나**(L0 §4-2-2 참조) |
